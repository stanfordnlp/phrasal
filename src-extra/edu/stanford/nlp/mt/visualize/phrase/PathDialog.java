...
public class PathDialog extends JFrame {
  ...
  static {
    ...
    Map<TextAttribute, Object> fontProps = (Map<TextAttribute, Object>) f
        .getAttributes();
  ...
  private ClickEventListener clickStreamListener = new ClickEventListener() {
    ...
    public void handleClickEvent(ClickEvent e) {
      ...
      if (isRecording && recordingPath.size() == 0) {
        ...
        parent.updateTranslationOnLayout(currentTranslationId,
            recordingPathName, recordingTranslation.toString());
        ...
        if (recordingPath.size() == 0) {
          ...
          recordingTranslation.delete(translationSplits.pop(),
              recordingTranslation.length());
          parent.updateTranslationOnLayout(currentTranslationId,
              recordingPathName, recordingTranslation.toString());
        }
  ...
  private JSplitPane getMainSplitPane() {
    if (mainSplitPane == null) {
      mainSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
          getPathsScrollPane(), getButtonPanel());
  ...
  private void createNewPath(String name) {
    ...
    if (VERBOSE)
      System.err
          .printf(
              "%s: Unable to create path (%s). Does the path name already exist?\n",
  ...
  private void toggleRecording(boolean isOn, String name) {
    if (isOn) {
      ...
      controller.setTranslationForPath(currentTranslationId, name,
          recordingTranslation.toString());
    }
  ...
  private boolean addPathToPanel(String name, boolean isEnabled) {
    PathComponents comps = (pathMap.get(currentTranslationId) != null && pathMap
        .get(currentTranslationId).containsKey(name)) ? pathMap.get(
        currentTranslationId).get(name) : new PathComponents();
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (toks.length != 2 && VERBOSE)
        System.err.printf("%s: Bad radio button command received %s\n", this
            .getClass().getName(), e.getActionCommand());
