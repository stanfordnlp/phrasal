...
my $usage = "\n\nUsage: $0 [-h] -r <ref_file> -s src_file -t <tst_file>\n\n".
    ...
    "  -N score only news stories of MT06 and MT08\n".
    "  -W score only web documents of MT06 and MT08\n".
    "  -B score only broadcast news documents of MT06\n".
    "\n";
 
use vars qw ($opt_r $opt_s $opt_t $opt_d $opt_h $opt_b $opt_n $opt_c $opt_x $opt_N $opt_W $opt_B);
use Getopt::Std;
getopts ('r:s:t:d:hbncx:NWB');
...
sub score_system {
    ...
    foreach $doc (sort keys %eval_docs) {
 				if($opt_N || $opt_W || $opt_B) {
 					my $genre_news = ($doc =~ /^(AFP|CNS|GMW|PDA|PLA|XIN)/);
 					my $genre_web  = ($doc =~ /(^groups\.google\.|\-WL\-|\-NG\-)/);
 					my $genre_bn   = ($doc =~ /^(CCTV4|NTDTV|PHOENIX)/);
 					die "Unrecognized document id: $doc\n" if !$genre_news && !$genre_web && !$genre_bn;
 					my $skip = 0;
 					$skip = 1 if $opt_N && !$genre_news;
 					$skip = 1 if $opt_W && !$genre_web;
 					$skip = 1 if $opt_B && !$genre_bn;
 					print STDERR "document $doc: $skip\n";
 					next if $skip;
 				}
