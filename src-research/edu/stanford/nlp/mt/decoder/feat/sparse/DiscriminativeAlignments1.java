    ...
    for (int i = 0; i < tgtLength; ++i) {
      ...
      if (alignments == null) {
        if (addTargetInsertions) {
          ...
          for(String representation : targetRepresentation(tgtWord)){
            String featureString = FEATURE_NAME_TGT + ":" + representation;
    ...
    for (int i = 0; i < srcLength; ++i) {
      ...
      if (alignments.size() == 0) {
        if (addSourceDeletions) {
          // Thang Jan14: add individual class features
          for(String representation : sourceRepresentation(srcWord)){
            String featureString = FEATURE_NAME_SRC + ":" + representation;
            features.add(new FeatureValue<String>(featureString, 1.0));
