...
public class SSVMScorer implements Scorer<String> {
  ...
  public SSVMScorer(Map<String, Double> manualFeatureWts, int dataPts,
      double wtMultiplier, boolean constrainManualWeights) {
    ...
    for (String key : manualFeatureWts.keySet()) {
      manualWeights[featureIndex.indexOf(key)] = wtMultiplier
          * manualFeatureWts.get(key);
    }
    ...
    ssvm = StructuredSVM.trainableMCSVM(Kernel.factory("linear"), C,
        StructuredSVM.StructLoss.MarginRescale, maxDataPts + 100);
    gssvm = StructuredSVM.trainableMCSVM(Kernel.factory("grbf:1"), C,
        StructuredSVM.StructLoss.MarginRescale, maxDataPts);
    addBiasPt();
  ...
  public void resetSSVM() {
    ssvm = StructuredSVM.trainableMCSVM(Kernel.factory("linear"), ssvm.getC(),
        StructuredSVM.StructLoss.MarginRescale, maxDataPts + 100);
    addBiasPt();
  }
  ...
  public SSVMScorer(Map<String, Double> featureWts) {
    ...
    maxDataPts = 1;
    for (String key : featureWts.keySet()) {
      manualWeights[featureIndex.indexOf(key)] = featureWts.get(key)
          .doubleValue();

    }

    ssvm = StructuredSVM.trainableMCSVM(Kernel.factory("linear"), new double[] {
        DEFAULT_C, DEFAULT_BIAS_C }, StructuredSVM.StructLoss.MarginRescale, 2);
    addBiasPt();
  }
  ...
  private void addBiasPt() {
    ...
    if (constrainManualWeights) {
      for (int i = 0; i < manualWeights.length; i++) {
        ...
        ssvm.expandSubProblem(0, maxDataPts + cPt++, lockVector, emptyVector,
            manualWeights[i], true);
        ssvm.getC()[maxDataPts + cPt] = Double.NEGATIVE_INFINITY;
        displayWeights();
        System.err.printf("Contraining %d <= %e\n", i, manualWeights[i]);
        ssvm.expandSubProblem(0, maxDataPts + cPt++, emptyVector, lockVector,
            -manualWeights[i], true);
        displayWeights();
      }
    }
    ...
    // ssvm.expandSubProblem(0, maxDataPts, biasVector, emptyVector,
    // DEFAULT_BIAS_MARGIN, true);
    ...
    if (!constrainManualWeights) {
      ...
      Map<Integer, Double> vMap = new HashMap<Integer, Double>();
      for (int i = 0; i < manualWeights.length; i++) {
        if (manualWeights[i] != 0)
          vMap.put(i, manualWeights[i]);
      }

      ssvm.getC()[maxDataPts + cPt] = Double.NEGATIVE_INFINITY;
      // ssvm.expandSubProblem(0, maxDataPts+cPt++, new SparseVector(vMap),
      // emptyVector, 0.1, true);
      displayWeights();
      ...
      for (String generativeFeature : generativeFeatures) {
        vMap.clear();
        System.err.printf(
            "Constratining Generative Feature: %s(%d) >= 0 alpha idx: %d\n",
            generativeFeature, featureIndex.indexOf(generativeFeature, true),
            maxDataPts + cPt);

        vMap.put(featureIndex.indexOf(generativeFeature, true), 1.0);
        if (maxIndex < featureIndex.indexOf(generativeFeature, true))
          maxIndex = featureIndex.indexOf(generativeFeature, true);
        SparseVector cVector = new SparseVector(vMap);
        ssvm.getC()[maxDataPts + cPt] = Double.NEGATIVE_INFINITY;
        ssvm.expandSubProblem(0, maxDataPts + cPt++, cVector, emptyVector, 0.0,
            true);
      }
      generativeWts = new boolean[maxIndex + 1];
      /*
       * XXXX for (String generativeFeature : generativeFeatures) {
  ...
  public double getIncrementalScore(Collection<FeatureValue<String>> features) {
    if (!useRBF) {
      ...
      for (FeatureValue<String> feature : features) {
        if (zeroFeatures != null && zeroFeatures.contains(feature.name))
          continue;
        if (nonZeroFeatures != null && !nonZeroFeatures.contains(feature.name))
          continue;
        ...
        if (idx < ssvmWeights.length) {
          ...
          if (idx < generativeWts.length && generativeWts[idx] && wt < 0)
            continue;
  ...
  public SparseVector createVector(Collection<FeatureValue<String>> features,
      boolean addFeatureValues) {
  ...
  public double angleBias() {
    return biasVector.dotProduct(ssvm.getWeights())
        / (l2normDense(ssvm.getWeights()) * biasVector.l2norm());
  }
