...
public class MultinomialDistortionFeaturizer extends
  ...
  private static final double[] paramCache = new double[DistortionModel.Class
      .values().length];
  ...
  public MultinomialDistortionFeaturizer(String... args) {
    ...
    if (!paramsFile.exists())
      throw new RuntimeException(this.getClass().getName()
          + ": Params file does not exits: " + paramsFile.getPath());

    loadParams(paramsFile);

    System.err.println(this.getClass().getName()
        + ": Multinomial (log) parameters");
  ...
  private void loadParams(File paramsFile) {
    ...
    try {
      ...
      while (reader.ready()) {
        ...
        DistortionModel.Class thisClass = DistortionModel.Class
            .valueOf(toks[0]);
