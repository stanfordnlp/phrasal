...
public class DependencyLanguageModelFeaturizer extends
    ...
    RichIncrementalFeaturizer<IString, String>, AlignmentFeaturizer {

  // How many words of left context for POS tagging:
  // public static final String ORDER_PROPERTY = "leftWords";
  // public static final int ORDER =
  // Integer.parseInt(System.getProperty(ORDER_PROPERTY, "3"));

  public static final String TAG_WITH_RIGHT_CONTEXT_PROPERTY = "tagWithRight";
  public static final boolean TAG_WITH_RIGHT_CONTEXT = System
      .getProperty(TAG_WITH_RIGHT_CONTEXT_PROPERTY) != null;

  public static final String CACHE_PARTIAL_PROPERTY = "cachePartial";
  public static final boolean CACHE_PARTIAL = System
      .getProperty(CACHE_PARTIAL_PROPERTY) != null;
  ...
  static final Set<String> allOpts = new HashSet<String>(Arrays.asList(
  ...
  private final boolean localNorm, localInvNorm, lenNorm, mstLenNorm,
      normAndUnnorm; // normalization
  ...
  public DependencyLanguageModelFeaturizer(String... args) throws Exception {
    ...
    if (args.length == 5) {
      ...
      DependencyReader depReader = DependencyReader.createDependencyReader(
          pipe, "CONLL", options);
      System.err.println("Bilingual features. Reading source instances from: "
          + srcFile);
    ...
    if (bilingual && CACHE_PARTIAL) {
      throw new RuntimeException(
          "Can't cache target side dependency structure when dependencies also depend on the source!!");
    } else if (!bilingual && !CACHE_PARTIAL) {
      System.err
          .println("WARNING: depLM without caching is going to be inefficient!");
    }
    ...
    if (!allOpts.containsAll(opts))
      throw new UnsupportedOperationException("Some unknown option in: "
          + optionStr);
   ...
   * 
   */
  private void addMSTFeature(DependencyInstance instance,
    ...
    if (this.reranking && this.mstScore) {
      ...
      if (cached == null) {
        DependencyInstanceFeatures dfeatures = new DependencyInstanceFeatures(
            instance.length(), pipe.getTypes().length);
        pipe.fillFeatureVectors(instance, dfeatures, par);
        Object[][] d = decoder
            .decodeNonProjective(instance, dfeatures, 1, true);
      ...
      features.add(new FeatureValue<String>(depFeatureName + ":mst",
          exactDepScore));
    }
  }
  ...
  private List<FeatureValue<String>> getFeatures(Featurizable<IString, String> f) {
    ...
    for (int i = 0; i < localScores.length; ++i) {
      ...
      if (ds != 0.0)
        features.add(new FeatureValue<String>(depFeatureName + depFeatures[i],
            ds));
    }
    return features;
  }

  private List<FeatureValue<String>> getDelayedFeatures(
      Featurizable<IString, String> f, boolean reranking) {
    ...
    if (reranking && f.done) {
      ...
      // Score last phrase using approximate dependency features (dependencies
      // with loops):
  ...
  private DependencyScores getDependencies(Featurizable<IString, String> f) {
    ...
    DependencyScores currentNode = (f.prior != null) ? (DependencyScores) f.prior
        .getState(this) : null;
...
//        tags[i] = prefixTagger.getBestTag(context, 0);
    ...
    if (f.prior != null) {
      ...
      try {
        DependencyInstance prior_dep = ((DependencyScores) f.prior
            .getState(this)).dep;
    ...
    if (pipe != null) {
      ...
      for (int j = loc; j < loc + sz + (f.done ? 1 : 0); ++j) { // 1 word delay,
        ...
        for (int i = 0; i < j; ++i) {
          ...
          float sL = (float) pipe.getScore(dep, i, j, false, par); // left
                                                                   // modifier
                                                                   // attaches
                                                                   // to right
                                                                   // head
          float sR = (float) pipe.getScore(dep, i, j, true, par); // right
                                                                  // modifier
                                                                  // attaches to
                                                                  // left head
          if (verboseDebug) {
            if (i > 0) {
              ...
              printDep(dep, i, dep.getHead(i), old_sL, j - i, "   old L score"
                  + (sL > old_sL ? " (outperformed)" : ""), false);
            }
            printDep(dep, i, j, sR, j - i, "   R score", true);
            printDep(dep, dep.getHead(j), j, old_sR, j - i, "   old R score"
                + (sR > old_sR ? " (outperformed)" : ""), true);
          }
          if (sL > old_sL) {
            if (i > 0) {
        ...
        if (debug) {
          ...
          if (verboseDebug)
            System.err.printf("m(%d)=%s h(%d)=%s\n", j, dep.getForm(j), hj,
                hj >= 0 ? dep.getForm(hj) : null);
        }
      }
      if (debug) {
    ...
    DependencyScores successorNode = new DependencyScores(dep, loc, sz,
        tagScore, argmaxDepScore, currentNode);
...
//          prefixTagger = new PrefixTagger(maxentTagger, 3, 0); // TODO: 3,1
  ...
  private static void printDep(DependencyInstance dep, int i, int j,
    ...
    System.err.printf("%s: %s/%s %s %s/%s score=%f dist=%d\n", prefix,
        ...
        (attR ? " <- " : " -> "), j < 0 ? "<root>" : (j < len ? dep.getForm(j)
            : "<error>"), j < 0 ? "<root-POS>" : (j < len ? dep.getPOSTag(j)
            : "<error>"), score, dist);
  }
  ...
  public Object clone() throws CloneNotSupportedException {
    ...
    DependencyLanguageModelFeaturizer featurizer = (DependencyLanguageModelFeaturizer) super
        .clone();
  ...
  private String[] getLocalFeatureNames() {
    ...
    if (normAndUnnorm || !localNorm || !localInvNorm)
      names.add(":argmax");
    if (localNorm)
      names.add(":largmax");
    if (localInvNorm)
  ...
  class DependencyScores implements Cloneable {
    ...
    DependencyScores(DependencyInstance dep, int pos, int phraseLen,
        float tagScore, float depScore, DependencyScores prior) {
      this.dep = dep;
      localScores = getLocalFeatures(prior, pos, phraseLen, tagScore, depScore);
      totalScores = (prior == null) ? localScores : ArrayMath.pairwiseAdd(
          localScores, prior.totalScores);
    }

    void updateLocalFromTotal(DependencyScores prior) {
      if (prior != null)
        localScores = ArrayMath
            .pairwiseSubtract(totalScores, prior.totalScores);
      else
        localScores = totalScores;
    }

    private float[] getLocalFeatures(DependencyScores d, int pos,
        int phraseLen, float tagScore, float depScore) {
      float[] scores = new float[depFeatures.length];
      int i = -1;
      if (normAndUnnorm || !localNorm || !localInvNorm)
        scores[++i] = depScore;
      if (localNorm)
        scores[++i] = localNorm(depScore);
      if (localInvNorm)
        scores[++i] = localNorm(-depScore);
      boolean hasPrev = d != null && d.totalScores != null;
      if (lenNorm)
        scores[++i] = lenNorm(hasPrev ? d.totalScores[0] : 0.0f, depScore, pos,
            phraseLen);
      if (posScore)
        scores[++i] = tagScore;
      ++i;
    ...
    private float lenNorm(float oldTotalScore, float localScore, int pos,
        int phraseLen) {
