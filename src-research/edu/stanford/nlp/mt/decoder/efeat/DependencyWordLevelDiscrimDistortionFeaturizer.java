...
/*
 import edu.stanford.nlp.mt.IString;
 */
 ...
 * @param <TK>
 */
public class DependencyWordLevelDiscrimDistortionFeaturizer<TK> implements
  ...
  void initParseFile(String parseFilename) {
    try {
      ...
      e.printStackTrace();
      throw new RuntimeException("loading " + parseFilename
  ...
  public List<FeatureValue<String>> listFeaturize(Featurizable<TK, String> f) {
    ...
    /*
     * if currentDep2Head exists, it means it's read in "reset" from the
     * "parseFilename" file. Let's check if the number of words is the same as
    ...
    for (int fidx = fStartIdx; fidx <= fEndIdx; fidx++) {
      ...
      // do features that has a dependent in the range of this currently
      // translated phrase
      int dep = fidx;
      int head;
      if (currentDep2Head[dep] == null)
        head = -1;
      else
        head = currentDep2Head[dep];

      // if the head is already translated, let's add the feature!!
      if (!WordLevelDiscrimDistortionFeaturizer.wordTranslated(f, dep)) {
        throw new RuntimeException("how can word " + dep
            + " hasn't been translated yet?");
      }

      if (DETAILED_DEBUG)
        System.err.println("DEPS: dep in the range!");
      features.addAll(WordLevelDiscrimDistortionFeaturizer
          .extractDistortionFeatureForPairOfSourceWords(head, dep, f, null,
              model1, "D|"));
      features.addAll(extractDistortionFeatureForDependencyType(head, dep, f,
          model1));

      // then,
      // do features that has a head in the range of this currently translated
      // phrase
      head = fidx;
      if (DETAILED_DEBUG)
        System.err.println("DEPS: head in the range!");
      List<Integer> deps = currentHead2Deps.get(head);
      if (deps != null) {
        for (int oneDep : deps) {
          if (oneDep >= fStartIdx && oneDep <= fEndIdx) {
            // if both side of this link are within this phrase, the feature of
            // this dependency
            // link are actually already added! Don't duplicate!!!
            continue;
          } else {
            features.addAll(WordLevelDiscrimDistortionFeaturizer
                .extractDistortionFeatureForPairOfSourceWords(head, oneDep, f,
                    null, model1, "D|"));
            features.addAll(extractDistortionFeatureForDependencyType(head,
                oneDep, f, model1));
          }
        }
      }
  ...
  private List<FeatureValue<String>> extractDistortionFeatureForDependencyType(
    ...
    List<String> distortionTypes = WordLevelDiscrimDistortionFeaturizer
        .getDistortionTypes(distance);

    for (String distortionType : distortionTypes) {
      StringBuilder sb = new StringBuilder();
      sb.append("DT|") // Dependency Type
          .append(currentDepName[cidx2]).append("-").append(distortionType);
      if (DETAILED_DEBUG)
  ...
  public void reset() {
    ...
    try {
      ...
      if (line == null) {
        System.err
            .printf(
                "DependencyWordLevelDiscrimDistortionFeaturizer: reset lineCount from %d to 1\n",
                lineCount);
        System.err
            .printf(
                "DependencyWordLevelDiscrimDistortionFeaturizer: reread from wordtag file: %s\n",
                parseFilename);
        System.err
            .printf("DependencyWordLevelDiscrimDistortionFeaturizer: set firstIter to false\n");
        parseReader.close();
        try {
          parseReader = new LineNumberReader(new FileReader(parseFilename));
        } catch (Exception e) {
      ...
      if (readerLineNum != lineCount) {
        throw new RuntimeException(
            "DependencyWordLevelDiscrimDistortionFeaturizer: the internal lineCount("
                + lineCount
    ...
    System.err.printf(
        "DependencyWordLevelDiscrimDistortionFeaturizer: line %d\n", lineCount);
    if (line != null) {
      ...
      if (DETAILED_DEBUG) {
        System.err.printf(
            "DependencyWordLevelDiscrimDistortionFeaturizer: %s\n", line);
    ...
    for (String tok : toks) {
      ...
      if (depline.length != 5) {
        throw new RuntimeException("format error! line=[" + line
            + "] when loading file " + parseFilename);
      }
      try {
        ...
        if (depIdx > toks.length + 1 || govIdx > toks.length + 1) {
          throw new RuntimeException("toks.length=" + toks.length + ",depIdx="
              + depIdx + ",govIdx=" + govIdx);
        }
        List<Integer> hdeps = head2deps.get(govIdx);
        if (hdeps == null) {
          hdeps = new ArrayList<Integer>();
        }
        hdeps.add(depIdx);
