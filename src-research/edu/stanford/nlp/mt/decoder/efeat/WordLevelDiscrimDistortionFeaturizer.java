...
/**
 * 
 * implementing word-level discriminative features. ideas similar to
 * "Distortion models for statistical machine translation" (ACL06 Al-Onaizan and
 * Papineni) Difference: used as discriminative features.
 * 
 * @author Pi-Chuan Chang
 * 
 * @param <TK>
 */
public class WordLevelDiscrimDistortionFeaturizer<TK> implements
    IncrementalFeaturizer<TK, String>, AlignmentFeaturizer {
  ...
  void initWordTagFile(String wordTagFilename) {
    try {
      ...
      e.printStackTrace();
      throw new RuntimeException("loading " + wordTagFilename
  ...
  public List<FeatureValue<String>> listFeaturize(Featurizable<TK, String> f) {
    ...
    /*
     * if currentWordTag exists, it means it's read in "reset" from the
     * "wordTagFilename" file. Let's check if the number of words is the same as
    ...
    if (priorPhraseTranslated(f))
      features.addAll(extractConnectionFeaturesWithPreviousPhrase(f));
    if (nextPhraseTranslated(f))
      features.addAll(extractConnectionFeaturesWithNextPhrase(f));
  ...
  public void initialize(List<ConcreteTranslationOption<TK>> options,
    ...
    try {
      ...
      // model1 =
      // IBMModel1.load("/u/nlp/data/gale/scr/DancMTexperiments/feature_experiments/model1/zh_en.model.actual.t1");
      model1 = IBMModel1
          .load("/scr/nlp/data/gale2/acl08dd/resources/model1/zh_en.model.actual.t1");
  ...
  private List<FeatureValue<String>> extractInternalFeatures(
    ...
    for (int curr = start; curr < end; curr++) {
      int next = curr + 1;
      if (DETAILED_DEBUG)
        System.err.printf("word pair %d %d\n", curr, next);

      features.addAll(extractDistortionFeatureForPairOfSourceWords(curr, next,
          f, currentWordTag, model1));
    }
  ...
  /**
   * Return the index of the <code>englishPhrase</code> which has the highest
   * Model 1 score with <code>chWord</code>
   * 
   * @return the index of the word in <code>englishPhrase</code> which has the
   *         highest Model1 score with <code>chWord</code>
   */
  protected static <TK> int getAlignedEnglishWordIndex(TK chWord,
      Sequence<TK> englishPhrase, IBMModel1 model1) {
    double max = Double.MIN_VALUE;
    int max_i = -1;
    if (DETAILED_DEBUG)
      System.err.println("chWord=" + chWord);
    ...
    if (DETAILED_DEBUG)
      System.err.println("enWord=" + englishPhrase.get(max_i));
    return max_i;
  }
  ...
  private static <TK> List<FeatureValue<String>> extractDistortionFeatureForPairOfSourceWords(
      ...
      List<WordTag> currentWordTag, IBMModel1 model1) {
    return extractDistortionFeatureForPairOfSourceWords(cidx1, cidx2, f,
        currentWordTag, model1, "");
  }

  /**
   * extract distortion features for a pair of source words. depends on some
   * global boolean variables like PAIR_OF_SOURCE_WORDS, FIRST_OF_SOURCE_WORDS
   * or SECOND_OF_SOURCE_WORDS, features of different granualarity are included
   **/
  protected static <TK> List<FeatureValue<String>> extractDistortionFeatureForPairOfSourceWords(
    ...
    if (ephrase1_range.length != 2 || ephrase2_range.length != 2) {
      throw new RuntimeException("fword " + cidx1 + " and " + cidx2
          + " should both already been translated, why's the range not int[2]?");
    }
    ...
    int cidx1_AlignedEnglishIndex = ephrase1_range[0]
        + relative_cidx1_AlignedEnglishIndex;
    int cidx2_AlignedEnglishIndex = ephrase2_range[0]
        + relative_cidx2_AlignedEnglishIndex;
    ...
    for (String distortionType : distortionTypes) {
      if (PAIR_OF_SOURCE_WORDS) {
        ...
        if (USE_POS_TAGS) {
          ...
          sb.append(featPrefix).append("POS|")
              .append(currentWordTag.get(cidx1).tag()).append("-")
              .append(currentWordTag.get(cidx2).tag()).append("-")
              .append(distortionType);
          if (DETAILED_DEBUG)
            System.err.println("adding feature: " + sb.toString());
          features.add(new FeatureValue<String>(sb.toString(), 1.0));
      ...
      if (FIRST_OF_SOURCE_WORDS) {
        ...
        if (USE_POS_TAGS) {
          sb = new StringBuilder();
          sb.append(featPrefix).append("POS|").append("SW1:")
              .append(currentWordTag.get(cidx1).tag()).append("-")
              .append(distortionType);
          if (DETAILED_DEBUG)
            System.err.println("adding feature: " + sb.toString());
      ...
      if (SECOND_OF_SOURCE_WORDS) {
        ...
        if (USE_POS_TAGS) {
          sb = new StringBuilder();
          sb.append(featPrefix).append("POS|").append("SW2:")
              .append(currentWordTag.get(cidx2).tag()).append("-")
              .append(distortionType);
          if (DETAILED_DEBUG)
            System.err.println("adding feature: " + sb.toString());
  ...
  public static String getDistortionType(int distance) {
    ...
    if (distance > 1) {
      ...
      throw new RuntimeException(
          "some uncaught case in defining distortionType");
    }
    return distortionType;
  }
  ...
  /**
   * @param wordPosition
   *          the position of the source word we want to check if it has been
   *          translated or not
   **/
  protected static <TK> boolean wordTranslated(Featurizable<TK, String> f,
      int wordPosition) {
    if (wordPosition < 0)
      return false;
  ...
  /**
   * @return wheather the linearly adjacent previous foreign phrase has been
   *         translated or not
   **/
  ...
  /**
   * @return wheather the linearly adjacent next foreign phrase has been
   *         translated or not
   **/
    ...
    try {
      if (USE_POS_TAGS == true) { // wordTagFilename != null) {
        ...
        if (line == null) {
          System.err
              .printf(
                  "WordLevelDiscrimDistortionFeaturizer: reset lineCount from %d to 1\n",
                  lineCount);
          System.err
              .printf(
                  "WordLevelDiscrimDistortionFeaturizer: reread from wordtag file: %s\n",
                  wordTagFilename);
          System.err
              .printf("WordLevelDiscrimDistortionFeaturizer: set firstIter to false\n");
          wordTagReader.close();
          try {
            wordTagReader = new LineNumberReader(
                new FileReader(wordTagFilename));
          } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("loading " + wordTagFilename
                + " generated an exception");
        ...
        if (readerLineNum != lineCount) {
          throw new RuntimeException(
              "WordLevelDiscrimDistortionFeaturizer: the internal lineCount("
                  + lineCount
  ...
  private List<WordTag> parseLineIntoWordTags(String line) {
    ...
    for (String tok : toks) {
      ...
      if (wordtag.length != 2) {
        throw new RuntimeException("format error! line=[" + line
            + "] when loading file " + wordTagFilename);
      }
