...
public class DiscrimDistortionFeaturizer extends
    StatefulFeaturizer<IString, String> implements
    RichIncrementalFeaturizer<IString, String> {
  ...
  // Threadsafe...the reference will be clone()'d, but the object will be set
  // when
  ...
  public DiscrimDistortionFeaturizer(String... args) {
    ...
    if (args.length == 4) {
      System.err.println(this.getClass().getName()
          + ": Using inbound and outbound models");
      useTwoModels = true;
      File outModelFile = new File(args[3]);
      if (!outModelFile.exists())
        throw new RuntimeException(this.getClass().getName()
            + ": Model file does not exist: " + outModelFile.getPath());
  ...
  private DistortionModel loadModel(File modelFile) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Could not load discriminative distortion model: "
              + modelFile.getPath());
  ...
  public List<FeatureValue<String>> listFeaturize(
    ...
    if (useTwoModels) {
      ...
      // assert inBoundScore.first() == outBoundScore.first(); //They need to
      // end at the same place!!
      features.add(new FeatureValue<String>("Out:" + FEATURE_NAME,
          outBoundScore.second()));
    }
  ...
  public void initialize(List<ConcreteTranslationOption<IString>> options,
    ...
    if (!sentenceToId.containsKey(foreign))
      throw new RuntimeException(this.getClass().getName()
          + ": No translation ID for sentence:\n " + foreign.toString());
    ...
    List<String> inDebugDatums = setupModelCache(inModel, inLogProbCache,
        foreign, translationId);

    List<String> outDebugDatums = null;
    if (useTwoModels) {
      outLogProbCache = new double[cacheSize][];
      outDatums = new ArrayList<Datum>();
      outDebugDatums = setupModelCache(outModel, outLogProbCache, foreign,
          translationId);
    }
  ...
  private List<String> setupModelCache(DistortionModel model, double[][] cache,
      Sequence<IString> foreign, final int translationId) {
    ...
    for (int sIdx = 0; sIdx < cache.length; sIdx++) {
      ...
      if (useDelims && sIdx == 0) {
        ...
        tagFeat = model.tagIndex.indexOf(posTags.get(translationId).get(
            realSIdx));
  ...
  public void dump(Featurizable<IString, String> f) {
    ...
    synchronized (System.err) {
      ...
      while (!featurizers.empty()) {
        ...
        int numNulls = (thisF.prior == null) ? 0 : (Integer) thisF
            .getState(this);
