...
public class ArabicVSOkbestFeaturizer extends
    StatefulFeaturizer<IString, String> implements
    ClonedFeaturizer<IString, String>, AlignmentFeaturizer {
  ...
  public ArabicVSOkbestFeaturizer(String... args) {
    ...
    if (!unkFile.exists())
      throw new RuntimeException(String.format("%s: File does not exist (%s)",
          this.getClass().getName(), unkFile.getPath()));
  ...
  /**
   * Returns true if the span of token positions specified by span is completely
   * covered in the partial hypothesis of f. Otherwise, returns false.
   * 
   */
  private static boolean isCovered(int leftSubjectBoundary,
      int rightSubjectBoundary, final int verbIdx,
  ...
  private static int getRightTargetSideBoundary(int sLeft, int sRight,
      final Featurizable<IString, String> f) {
    int maxRightIndex = -1;
  ...
  private static boolean matches(Featurizable<IString, String> f,
    ...
    if (isCovered(subject.first(), subject.second(), thisVerbIdx, f)) {
      int rightSubjTargetBoundary = getRightTargetSideBoundary(subject.first(),
          subject.second(), f);
  ...
  public FeatureValue<String> featurize(Featurizable<IString, String> f) {
    ...
    final int lastVerbIdx = (f.prior == null) ? -1 : (Integer) f.prior
        .getState(this);
    final int thisVerbIdx = (lastVerbIdx + 1 < orderedVerbs.size()) ? orderedVerbs
        .get(lastVerbIdx + 1) : -1;
    if (thisVerbIdx == -1)
    ...
    if (f.hyp.foreignCoverage.get(thisVerbIdx)) {
      ...
      if (subjectAnalyses == null)
        throw new RuntimeException(String.format(
            "%s: No subjects for verb idx (%d)", FEATURE_NAME, thisVerbIdx));
  ...
  public void initialize(List<ConcreteTranslationOption<IString>> options,
    ...
    if (!sentenceToId.containsKey(foreign))
      throw new RuntimeException(String.format(
          "%s: No mapping found for sentence:\n%s\n", FEATURE_NAME,
          foreign.toString()));
    ...
    if (analyses != null) {
      ...
      if (nullAnalRealAccumulator == 0.0)
        defaultNullAnalysisLogProb = Math.log(nullAnalRealAccumulator)
            - Math.log(nullDenom);
      else
        // Some arbitrarily (low) score
        defaultNullAnalysisLogProb = (3.0 * minLogCRFScore)
            - Math.log(nullDenom);

      // Set the other probabilities (for each verb)
      verbMap = new HashMap<Integer, List<Triple<Integer, Integer, Double>>>(
          allVerbs.size());
      ...
      for (int verbIdx : allVerbs) {
        ...
        List<Triple<Integer, Integer, Double>> probList = new ArrayList<Triple<Integer, Integer, Double>>(
            subjects.keySet().size());

        // Check for null analyses
        if (nullNumerator != 0.0) {
          double logProb = Math.log(nullNumerator) - Math.log(denom);
          Triple<Integer, Integer, Double> nullAnal = new Triple<Integer, Integer, Double>(
              -1, -1, logProb);
