...
public class TextCat {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (loadPath == null) {
      ...
      ObjectBank<List<String>> nw = getWebData(Collections.singleton(trainDir
          + "/nw" + suffix + "/c/"));
      ObjectBank<List<String>> wl = getWebData(Collections.singleton(trainDir
          + "/wl" + suffix + "/c/"));
      ObjectBank<List<String>> ng = getWebData(Collections.singleton(trainDir
          + "/ng" + suffix + "/c/"));
      ...
      classifier = (LinearClassifier<String, String>) cf.trainClassifier(
          dataset, dataWeights, prior);
      ...
      classifier = (LinearClassifier<String, String>) IOUtils
          .readObjectFromFile(loadPath);
    }
    ...
    if (labelDir == null) {
      ...
      String suffix = prop.getProperty("suffix", "");
      ObjectBank<List<String>> nwTest = getWebData(Collections
          .singleton(testDir + "/mt06/nw" + suffix + "/"));
      ObjectBank<List<String>> webTest = getWebData(Collections
          .singleton(testDir + "/mt06/ng" + suffix + "/"));
      Map<String, ObjectBank<List<String>>> data = new HashMap<String, ObjectBank<List<String>>>();
      data.put("nw", nwTest);
      ...
      FileSequentialCollection files = new FileSequentialCollection(
          Collections.singleton(labelDir), ".norm", true);
      for (File file : files) {
  ...
  private static Pair<String, Counter<String>> label(List<String> doc,
    ...
    Counter<String> probs = classifier
        .probabilityOf((Datum<String, String>) datum);
  ...
  private static ClassicCounter<String> getFeatures(List<String> doc) {
    ...
    try {
      sentences = ChineseDocumentToSentenceProcessor
          .fromPlainText(concat, true);
    } catch (Exception e) {
