...
public class NewDynamicPhraseTable extends
  ...
  public NewDynamicPhraseTable(
    ...
    Runtime.getRuntime().addShutdownHook(new Thread() {
      ...
      public void run() {
        ...
        commonDB.sync();
        commonDB.close(); // avoid locking on close issue
      }
    });
  }
  ...
  class IndexWrapper extends AbstractList<FIndexSequence> implements
      RandomAccess {

    @Override
    public FIndexSequence get(int idx) {
      // System.out.printf("idx: %d val: %d line: %d offset: %d\n", idx,
      // sortedindex[idx], sortedindex[idx]>>>8, sortedindex[idx]&0xFF);
  ...
  public List<TranslationOption<IString>> getTranslationOptions(
    ...
    if (idx >= sortedindex.length)
      return opts;
    if (!Sequences.startsWith(indexwrapper.get(idx), sequence))
      return opts;
    ...
    List<String> commonTrans = (List<String>) commonDB.getlist(rawSequence
        .toString());
    ...
    if (commonTrans == null) {
      ...
      // extract phrases
      for (; idx < sortedindex.length
          && Sequences.startsWith(indexwrapper.get(idx), sequence); idx++) {
        ...
        // tEquivFStart-MAX_ABSOLUTE_DISTORTION),
        // Math.min(bitext.ecorpus[line].length,
        // tEquivFStart+MAX_ABSOLUTE_DISTORTION));
        for (int tStart = Math.max(0, tEquivFStart - MAX_ABSOLUTE_DISTORTION); tStart < Math
            .min(bitext.ecorpus[line].length, tEquivFStart
                + MAX_ABSOLUTE_DISTORTION); tStart++) {
          for (int tEnd = tStart; tEnd < bitext.ecorpus[line].length
      ...
      sortedTrans = Counters
          .toDescendingMagnitudeSortedListWithCounts(transSet);
      ...
      if (cntNormalizer > 20000) {
        ...
        for (Pair<RawSequence<IString>, Double> trans : sortedTrans) {
          ...
          // trans.first.toString(), trans.second);
        }
        System.err.printf("Cache miss! %.3f sec - ", totalTime / 1000.0);
      } else {
        System.err.printf("Excluded from Cache! %.3f sec - ",
            totalTime / 1000.0);
      }
    } else {
      ...
      System.err.printf("Cache hit! %.3f sec - ",
          (endCacheTotalTime - startCacheCheckTime) / 1000.0);
    }
