...
public class TranslationLayout {
    ...
    while ((phrase = translation.getBestPhrase()) != null) {
      for (int i = 0; i < coverages.size(); i++) {
        ...
        if (testCoverage(phrase, bitSet)) {
          ...
          GridBagConstraints c = buildConstraints(phrase, i, numColumns,
              sourceWordsCovered);
          ...
          // The Phrase hash function is not injective, but we can't do much
          // about collisions
  ...
  private GridBagConstraints buildConstraints(Phrase phrase, int rowId,
      int maxColumn, int sourceWordsCovered) {
    GridBagConstraints c = new GridBagConstraints();
    c.fill = GridBagConstraints.HORIZONTAL;
    c.gridx = (RIGHT_TO_LEFT) ? maxColumn - phrase.getEnd() - 1 : phrase
        .getStart();
    c.gridy = rowId + 1;
