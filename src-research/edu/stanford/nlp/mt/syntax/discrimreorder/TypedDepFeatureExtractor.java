...
public class TypedDepFeatureExtractor implements FeatureExtractor {
  ...
  static {
    OPTS.addAll(Arrays.asList("head_dep_dir"));
  }

  public TypedDepFeatureExtractor(Properties props) throws Exception {
    HEAD_DEP_DIR = Boolean.parseBoolean(props.getProperty("head_dep_dir",
        "false"));
    ...
    if (HEAD_DEP_DIR) {
      ...
      if (example.src_j > 0 && example.src_j < matrix.f.length - 1
        ...
        if (matrix.chGraph != null) {
          ...
          path.append(DepUtils.getPathName(example.src_j - 1,
              example.src_jprime - 1, matrix.pathMap));
        }
        // System.err.printf("%d - %d\n", example.src_j-1,
        // example.src_jprime-1);
