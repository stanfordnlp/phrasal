 ...
 * there's similar class in the edu.stanford.nlp.mt.train package or
 * edu.stanford.nlp.mt.train.transtb. But I think it's good to make a new one in
 * this package so things don't get tangled up.(
 * 
    ...
    for (String al : aStr.split("\\s+")) {
      ...
      if (els.length == 2) {
        ...
        if (0 > fpos || fpos >= f.length)
          throw new IOException("f has index out of bounds (fsize=" + f.length
              + ",esize=" + e.length + ") : " + fpos);
        if (0 > epos || epos >= e.length)
          throw new IOException("e has index out of bounds (esize=" + e.length
              + ",fsize=" + f.length + ") : " + epos);
        fe[fpos][epos] = true;
      } else {
