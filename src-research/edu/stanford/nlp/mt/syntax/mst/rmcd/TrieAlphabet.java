...
public class TrieAlphabet implements Index<String>, Serializable {
   ...
   * 
   * @param mixParams
   *          Weights determining whether keys are dropped. A key j is dropped
   *          if weights[i][j] == 0 for all i.
   */
   ...
   * @return number of descendants with id >= 0 (if this number is zero, current
   *         node can be pruned)
   */
  ...
  private void addToMap(Trie trie, Map<Integer, List<String>> index,
  ...
  private static void evaluateIndex(String trainFile, String testFile,
      Index<String> index) {
    ...
    System.err.printf("Memory usage: %.3f MB time: %.3f sec.\n",
        (endMemUsed - startMemUsed) * 1.0 / 1024 / 1024,
        (endTimeMillis - startTimeMillis) * 1.0 / 1000.0);
...
class Trie implements Serializable {
  ...
  final static int mapType = Integer.parseInt(System
      .getProperty("mapType", "0"));

  private static final int ROOT_ID = -2;
  private static final int UNK_ID = -1;

  private static final Map<String, Trie> roots = new THashMap<String, Trie>(10,
      0.25f);
