...
public class TMGrammar_Memory extends TMGrammar {
  ...
  static int rule_id = 1; // three kinds of rule: regular rule (id>0); oov rule
                          // (id=0), and null rule (id=-1)
  ...
  public TMGrammar_Memory(ArrayList<Model> l_models, String default_ow,
  ...
  public void read_tm_grammar_glue_rules() {
    ...
    add_rule("S ||| [" + Decoder.default_non_terminal + ",1] ||| ["
        + Decoder.default_non_terminal + ",1] ||| 0",
        Symbol.add_terminal_symbol(Decoder.begin_mono_owner));// this does not
                                                              // have any cost
    // TODO: search consider_start_sym (Decoder.java, LMModel.java, and
    // Chart.java)
    // glue_gr.add_rule("S ||| [PHRASE,1] ||| "+Symbol.START_SYM+" [PHRASE,1] ||| 0",
    // begin_mono_owner);//this does not have any cost
    add_rule("S ||| [S,1] [" + Decoder.default_non_terminal + ",2] ||| [S,1] ["
        + Decoder.default_non_terminal + ",2] ||| " + alpha,
        Symbol.add_terminal_symbol(Decoder.begin_mono_owner));
    // glue_gr.add_rule("S ||| [S,1] [PHRASE,2] [PHRASE,3] ||| [S,1] [PHRASE,2] [PHRASE,3] ||| "+alpha,
    // MONO_OWNER);
    print_grammar();
    ensure_grammar_sorted();
  }
  ...
  private Rule add_rule(String line, int owner) {
    ...
    for (int k = 0; k < p_rule.french.length; k++) {
      ...
      if (Symbol.is_nonterminal(p_rule.french[k])) {// TODO
        cur_sym_id = Symbol.add_non_terminal_symbol(TMGrammar_Memory
            .replace_french_non_terminal(Symbol.get_string(p_rule.french[k])));
      }
  ...
  private static String replace_french_non_terminal(String symbol) {
    return symbol.replaceAll(non_terminal_replace_regexp, "");// remove [, ],
                                                              // and numbers
  }
  ...
  public class TrieNode_Memory extends TMGrammar.TrieNode {
    ...
    @Override
    public TrieNode_Memory match_symbol(int sym_id) {// looking for the next
                                                     // layer trinode
                                                     // corresponding to this
                                                     // symbol
      /*
       * if(sym_id==null) Support.write_log_line("Match_symbol: sym is null",
       * Support.ERROR);
       */
  ...
  public class RuleBin_Memory extends TMGrammar.RuleBin {
    ...
    private void add_rule(Rule_Memory rl) {
      if (heap_rules == null) {
        heap_rules = new PriorityQueue(1, Rule_Memory.NegtiveCostComparator);// TODO:
                                                                             // initial
                                                                             // capacity?
        arity = rl.arity;
      }
      if (rl.arity != arity) {
        Support.write_log_line(String
            .format("RuleBin: arity is not matching, old: %d; new: %d", arity,
                rl.arity), Support.ERROR);
        return;
      }
    ...
    private int run_pruning() {
      ...
      if (heap_rules.size() == Decoder.max_n_rules)
        cutoff = (cutoff < heap_rules.peek().est_cost) ? cutoff : heap_rules
            .peek().est_cost + Symbol.EPSILON;// TODO
      return n_pruned++;
    }

    private void print_info(int level) {
      Support.write_log_line(String.format("RuleBin, arity is %d", arity),
          level);
  ...
  public static class Rule_Memory extends Rule {
    ...
    // only called when creating rule in Chart, all others should call the other
    // contructor
    /*
     * the transition cost for phrase model, arity penalty, word penalty are all
     * zero, except the LM cost
     */
    public Rule_Memory(int lhs_in, int fr_in, int owner_in) {
      ...
      arity = 0;
      owner = owner_in;
      tem_estcost += estimate_rule();// estimate lower-bound, and set
                                     // statelesscost
    }
    ...
    public Rule_Memory(int r_id, String line, int owner_in) {
      ...
      if (fds.length != 4) {
        Support.write_log_line("rule line does not have four fds; " + line,
            Support.ERROR);
      }
      lhs = Symbol.add_non_terminal_symbol(TMGrammar_Memory
          .replace_french_non_terminal(fds[0]));

      // french side: note that the french side is not stored at the rule,
      // instead at the rulebin
      arity = 0;
      ...
      for (int i = 0; i < french_tem.length; i++) {
        if (is_non_terminal(french_tem[i]) == true) {
          arity++;
          // french[i]=
          // Symbol.add_non_terminal_symbol(TMGrammar_Memory.replace_french_non_terminal(french_tem[i]));
          french[i] = Symbol.add_non_terminal_symbol(french_tem[i]);// when
                                                                    // storing
                                                                    // hyper-graph,
                                                                    // we need
                                                                    // this
    ...
    protected void print_info(int level) {
      ...
      Support
          .write_log_line(
              " ||| owner: " + owner + "; statelesscost: "
                  + String.format("%.3f", statelesscost)
                  + String.format("; estcost: %.3f", est_cost) + "; arity:"
                  + arity, level);
    }
