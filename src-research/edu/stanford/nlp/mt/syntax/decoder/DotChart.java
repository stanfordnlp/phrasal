...
public class DotChart {
  ...
  public void expand_cell(int i, int j) {
    // there are two kinds of symbols in the foreign side: (1) non-terminal
    // (e.g., X or NP); (2) CN-side terminal
    // therefore, two ways to extend the dot postion
    // (1) if the dot is just to the left of a non-terminal variable, looking
    // for theorems or axioms in the Chart that may apply and extend the dot pos
    for (int k = i + 1; k < j; k++)
      extend_dotitems(i, k, j, false);

    // (2)the dot-item is looking for a CN-side terminal symbol: so we just need
    // a CN-side terminal symbol to advance the dot
    // note: this will deal with the seeding case (translation of terminal cn
    // words)
    // also deal with case in starting dotitems which start from a terminal, see
    // start_dotitems
    int last_word = foreign_sent[j - 1];

    // in seeding case: j=i+1, therefore, it will look for l_dot_bins[i][i]
    if (l_dot_bins[i][j - 1] != null)
      for (DotItem dt : l_dot_bins[i][j - 1].l_dot_items) {// dotitem in
                                                           // dot_bins[i][k]:
                                                           // looking for an
                                                           // item in the right
                                                           // to the dot
        TrieNode child_tnode = dt.tnode.match_symbol(last_word);// match the
                                                                // terminal
        if (child_tnode != null) {
          add_dot_item(child_tnode, i, j, dt.l_ant_super_items, null);// we do
                                                                      // not
                                                                      // have an
                                                                      // ant for
                                                                      // the
  ...
  // note: (i,j) is a non-terminal, this cannot be a cn-side terminal, which
  ...
  // looking for theorems or axioms in the "Chart" that may apply and extend the
  // dot pos
  private void extend_dotitems(int i, int k, int j, boolean start_dotitems) {
    if (l_dot_bins[i][k] == null || p_chart.l_bins[k][j] == null)
      return;

    for (DotItem dt : l_dot_bins[i][k].l_dot_items) {// dotitem in
                                                     // dot_bins[i][k]: looking
                                                     // for an item in the right
                                                     // to the dot
      ArrayList<SuperItem> t_ArrayList = new ArrayList(p_chart.l_bins[k][j]
          .get_sorted_super_items().values());
      // Support.write_log_line(String.format("Add a dotitem with"),
      // Support.DEBUG);
      for (SuperItem s_t : t_ArrayList) {// see if it is match what the dotitem
                                         // is looking for
        TrieNode child_tnode = dt.tnode.match_symbol(s_t.lhs);// match rule and
                                                              // complete part
        if (child_tnode != null) {
          if (start_dotitems == true && child_tnode.is_no_child_trienodes())
            continue;// TODO: byg
          // Support.write_log_line(String.format("Add a dotitem with superitem.lhs= %s",s_t.lhs),
          // Support.DEBUG);
  ...
  private void add_dot_item(TrieNode tnode, int i, int j,
    ...
    // Support.write_log_line(String.format("Add a dotitem in cell (%d, %d), n_dotitem=%d",i,j,p_chart.n_dotitem_added),
    // Support.DEBUG);
  }
  ...
  // #################### DotItem class
  // remember the dot position in which a rule has been applied so far, and
  // remember the old complete items
  public class DotItem {
    int i, j; // start and end position in the chart
    TrieNode tnode = null;// dot_position, point to grammar trie node, this is
                          // the only place that the DotChart points to the
                          // grammar
    ArrayList<SuperItem> l_ant_super_items = null; // pointer to SuperItem in
                                                   // Chart

    public DotItem(int i_in, int j_in, TrieNode tnode_in,
        ArrayList<SuperItem> ant_super_items_in) {
      i = i_in;
      j = j_in;
      tnode = tnode_in;
