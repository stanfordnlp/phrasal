...
public class DEinTextReorderer {
  public static void main(String[] args) throws IOException {
    ...
    Boolean noreorder = Boolean.parseBoolean(props.getProperty("noreorder",
        "false"));
    Boolean betterRange = Boolean.parseBoolean(props.getProperty("betterRange",
        "false"));
    Boolean strictRange = Boolean.parseBoolean(props.getProperty("strictRange",
        "false"));
    ...
    while ((parsedSent = reader.next()) != null) {
      ...
      List<Integer> markedDEIdxs = ExperimentUtils
          .getMarkedDEIndices(parsedSent.yield());
      ...
      for (int deIdx : markedDEIdxs) {
        ...
        if (betterRange)
          ...
          String dnpcpLabel = ExperimentUtils.getNPwithDE_DNPorCPLabel(
              parsedSent, deIdx);
        ...
        if (range.first == -1) {
          throw new RuntimeException(
              "ERROR: Flat tree shouldn't have been marked");
        }
      ...
      for (Map.Entry<SortByEndPair<Integer, Integer>, Integer> e : toOperate
        ...
        if (de.equals("的_AsB") || de.equals("的_AprepB") || de.equals("的_AB")
            || de.equals("的_noB") || de.equals("的_ordered")) {
          continue;
          ...
          parsedSent.pennPrint(System.err);
          System.err.println("(S) " + de + "(" + deIdx + ") in "
              + StringUtils.join(yield, " ") + " is not a valid DE");
