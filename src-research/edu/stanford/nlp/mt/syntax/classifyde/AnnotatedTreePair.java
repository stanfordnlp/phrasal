...
public class AnnotatedTreePair {
  ...
  public int numNPwithDE() {
    if (NPwithDEs.size() != NPwithDEs_deIdx.size()
        ...
        || (parsedNPwithDEs_deIdx != null && NPwithDEs_deIdx_set.size() != parsedNPwithDEs_deIdx
            .size())) {
      System.err.println("NPwithDEs.size=" + NPwithDEs.size());
      System.err.println("NPwithDEs_categories.size="
          + NPwithDEs_categories.size());
      System.err.println("NPwithDEs_deIdx.size=" + NPwithDEs_deIdx.size());
      System.err.println("NPwithDEs_deIdx_set.size="
          + NPwithDEs_deIdx_set.size());
      System.err.println("parsedNPwithDEs_deIdx.size.size="
          + parsedNPwithDEs_deIdx.size());
  ...
  private void computeParsedNPwithDEs() {
    ...
    if (parsedChineseTrees == null || parsedChineseTrees.isEmpty()) {
      System.err
          .println("Warning: NO Chinese parsed trees info present. Use gold trees instead.");
  ...
  public static void annotateNPwithDEs(List<Pair<String, String>> categories,
    ...
    for (int i = 0; i < categories.size(); i++) {
      ...
      if (offset < 0) {
        if (category.second().trim().equals(currentNP)) {
          ...
          if (nextCatNP.equals(nextNP) || nextNP.equals("")) { // if there's no
                                                               // nextNP in
                                                               // TreePair,
                                                               // that's ok too.
            offset = i;
    ...
    if (offset < 0)
      throw new RuntimeException("couldn't find offset in annotateNPwithDEs");
    ...
    if (NPwithDEs == null) {
      ...
      for (Pair<Integer, Integer> deSpan : deSpans) {
        ...
        TreeSet<Integer> nullSpan = treepair_.alignment()
            .mapChineseToEnglish_FillGap(enSpan);
        // merge these 2
          ...
          for (int eidx = enSpan.first(); eidx <= enSpan.last(); eidx++) {
            if (!enSpan.contains(eidx)) {
              ...
              for (int cidx = 1; cidx < treepair_.alignment().matrix_[eidx].length; cidx++) {
                if (treepair_.alignment().matrix_[eidx][cidx] > 0) {
                  notAligned = false;
                  break;
                }
              }
        ...
        Tree chNP = AlignmentUtils.getTreeWithEdges(sentTree, deSpan.first,
            deSpan.second + 1);
        if (chNP == null) {
        ...
        /*
         * Sentence<Word> s = sentTree.yield(); System.err.print(deIdx+"\t");
         * for(int i = deSpan.first; i < deSpan.second+1; i++) { Word w =
         * s.get(i); System.err.print(w+" "); } System.err.print("\t"+sentTree);
         * System.err.println();
         */
  ...
  public static int printAllwithDE(List<AnnotatedTreePair> atps) {
    ...
    // List<Set<Pair<Integer,Integer>>> deSpansList = new
    // ArrayList<Set<Pair<Integer,Integer>>>();
  ...
  public void printTreePairWithDeps() {
    ...
    for (int ti = 0; ti < enTrees.size(); ti++) {
      ...
      /*
       * GrammaticalStructure gsE = new
       * EnglishGrammaticalStructure(enT,puncWordFilter);
       * 
       * Collection<TypedDependency> depsE = gsE.allTypedDependencies();
       * System.out.printf("<h3> English Deps(%d) </h3>\n", ti);
       * System.out.println("<pre>"); for (TypedDependency dep : depsE) {
       * System.out.println(dep); } System.out.println("</pre>");
       */
    }
