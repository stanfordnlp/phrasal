...
class TrainDevTest {
  ...
  public static List<String> splits(int buckets, List<String> splitnames)
      throws IOException {
    List<AnnotatedTreePair> atreepairs = ExperimentUtils
        .readAnnotatedTreePairs(true);
    ...
    for (String key : labelIndices.keySet()) {
      ...
      for (int i = 0; i < split.size(); i++) {
        ...
        if (result.get(index) != null)
          throw new RuntimeException("multiple assignment for index " + index);
    ...
    for (AnnotatedTreePair validSent : atreepairs) {
      ...
      for (int deIdxInSent : validSent.NPwithDEs_deIdx_set) {
        ...
        if (!ExperimentUtils.is6class(label)) {
          ...
          if (set == null)
            throw new RuntimeException("no assignment for index " + npidx);
  ...
  private static List<String> NWayRandomSplit(int numitems, int n,
    ...
    if (splitnames.size() != n)
      throw new RuntimeException("the given splitnames should have size n");
  ...
  /*
   * private static List<String> ThreeWayRandomSplit(int size) { //String[]
   * split = new String[size]; List<String> split = new ArrayList<String>(size);
   * int devSize = size / 6; int testSize = size / 6; int trainSize = size -
   * devSize - testSize; Set<Integer> dev = new HashSet<Integer>(); Set<Integer>
   * test = new HashSet<Integer>(); // fill dev Random r = new Random();
   * while(dev.size() < devSize) { int i = r.nextInt(size); dev.add(i); }
   * while(test.size() < testSize) { int i = r.nextInt(size); if (
   * !dev.contains(i)) { test.add(i); } }
   * 
   * int finalTestSize = 0; int finalDevSize = 0; int finalTrainSize = 0; for
   * (int i = 0; i < size; i++) { if (test.contains(i)) { split.add(i, "test");
   * finalTestSize++; } else if (dev.contains(i)) { split.add(i, "dev");
   * finalDevSize++; } else { split.add(i, "train"); finalTrainSize++; } } if
   * (finalTestSize != testSize) throw new
   * RuntimeException("test size error: "+finalTestSize+" != " +testSize); if
   * (finalDevSize != devSize) throw new RuntimeException("dev size error"); if
   * (finalTrainSize != trainSize) throw new
   * RuntimeException("train size error");
   * 
   * return split; }
   */
}
