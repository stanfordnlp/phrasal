...
class AnalyzeNPsWithDEs {
  ...
  private static String analyzeNPwithDE(int deIdxInSent, AnnotatedTreePair atp,
    ...
    Tree chNPTree = ExperimentUtils.getTreeWithEdges(chTree, np.first,
        np.second + 1);
    if (chNPTree == null) {
      throw new RuntimeException("chNPTree shouldn't be null");
    }

    Tree enNPTree = ExperimentUtils.getTreeWithEdges(atp.enTrees(),
        englishNP.get(0).first, englishNP.get(0).second + 1);
    if (enNPTree == null) {
    ...
    fixWhichWhere(subtranslation, subsource, submatrix, rangeA, rangeB,
        deIndices);
    ...
    if (deIdx > 0 && deIdx < subsource.length - 1) {
      if (rangeA.second == -1) {
        return "other - A not aligned";
      }
      if (rangeB.second == -1) {
        return "other - B not aligned";
      }
    }

    if (deIdx == subsource.length - 1) {
      if (enNPTree != null && enNPTree.value().equals("VP")) {
        return "no B - VP";
      }
      return "no B";
    }
    ...
    if (rangeA.second == -1) {
      return "other - no en range A";
    }
    if (rangeB.second == -1) {
      return "other - no en range B";
    }
    ...
    if (rangeA.second < rangeB.first) {
      ...
      if (checkSecondVP(rangeA, rangeB, enNPTree))
        return "other - VP";

      // For the following cases, we have to first check that
      // DE does not align to something inside (not on edge) rangeA & rangeB
      if (deEidx == -1
          || ((onRangeEdge(deEidx, rangeA) || !inRange(deEidx, rangeA)) && (onRangeEdge(
              deEidx, rangeB) || !inRange(deEidx, rangeB)))) {
        if (deEidx != -1 && subtranslation[deEidx].equals("of"))
          return "A of B";
        if (deEidx != -1 && isPrep(subtranslation[deEidx]))
          return "A prep B";
        if (checkOrderedAdjective(rangeA, rangeB, enNPTree))
          return "A B (adj)";
        if (checkOrderedAdverb(rangeA, rangeB, enNPTree))
          return "A B (adv)";
        String mod = checkOrderedOtherModifier(rangeA, rangeB, enNPTree);
        if (mod != null)
          return "A B (" + mod.toLowerCase() + ")";
        if (checkOrderedNoun(rangeA, rangeB, enNPTree))
          return "A B (n)";
      } else {
        return "other - mixed";
      }
    ...
    if (rangeB.second < rangeA.first) {
      ...
      // check if deIdx aligns to somewhere in between max(rangeB) and
      // min(rangeA)
      for (int eidx = rangeB.second + 1; eidx <= rangeA.first - 1; eidx++) {
        if (submatrix[eidx][deIdx] > 0) {
          String deWord = subtranslation[eidx];
          if (deWord.equals("of") || deWord.equals("to")) {
            return "B " + deWord + " A";
          }
          if (isPrep(deWord)) {
            return "B prep A";
      ...
      if (checkFlippedRelativeClause(rangeA, rangeB, enNPTree))
        return "relative clause";// "flipped (relc)";
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (!dir.exists()) {
      ...
      if (!dir.exists()) {
        ...
        throw new RuntimeException(
            "LDC2006E93 doesn't exist in either of the hard-coded locations.");
      }
    }
    ...
    for (int fileidx = FIDX; fileidx <= FIDX; fileidx++) {
      ...
      if (!ctbdir.exists()) {
        ...
        if (!ctbdir.exists())
          throw new RuntimeException(
              "CTB6.0 doesn't exist in either of the hard-coded locations.");
      }
      String ctbname = String.format("%schtb_%04d.fid", ctbdirname, fileidx);
      ...
      if (!ectbdir.exists()) {
        ...
        if (!ectbdir.exists()) {
          throw new RuntimeException(
              "EnglishChineseTranslationTreebankV1.0 doesn't exist in either of the hard-coded locations.");
        }
      }
      String ename = String.format("%schtb_%03d.mrg.gz", ectbdirname, fileidx);
      ...
      // In this format, every token is as defined in the manual word_alignment
      // file.
      ...
      int npCount = 1;
      if (origtaPW != null) {
        AlignmentUtils.printAlignmentGridHeader(origtaPW);
      }
      if (npgridPW != null) {
      ...
      for (TranslationAlignment ta : alignment_list) {
        ...
        if (enTrees.size() == 0) {
          System.err.printf("i=%d: Can't find tree in PTB: %s\n", fileidx,
              StringUtils.join(ta.translation_, " "));
      ...
      if (origtaPW != null) {
        AlignmentUtils.printAlignmentGridBottom(origtaPW);
        origtaPW.close();
      }
  ...
  private static boolean checkOrderedAdjective(MutablePair<Integer, Integer> rangeA,
    ...
    Tree adj = ExperimentUtils.getTreeWithEdges(enTree, rangeA.first,
        rangeA.second + 1);
    if (adj != null
        && (adj.value().startsWith("ADJP") || adj.value().startsWith("JJ"))) {
      return true;
    }
    ...
    for (int sep = rangeA.first; sep <= rangeA.second; sep++) {
      Tree adj1 = ExperimentUtils.getTreeWithEdges(enTree, rangeA.first,
          sep + 1);
      Tree adj2 = ExperimentUtils.getTreeWithEdges(enTree, sep + 1,
          rangeA.second + 1);
      if (adj1 != null && adj2 != null) {
        String adj1str = adj1.value();
        String adj2str = adj2.value();
        if (adj2str.startsWith("JJ") || adj2str.equals("ADJP")) {
          if (adj1str.startsWith("RB")
              || (adj1str.startsWith("JJ") || (adj1str.startsWith("N")))) {
            return true;
          }
        }
      }
  ...
  private static String checkOrderedOtherModifier(
    ...
    Tree adj = ExperimentUtils.getTreeWithEdges(enTree, rangeA.first,
        rangeA.second + 1);
    if (adj != null
        && (adj.value().startsWith("VBN") || adj.value().startsWith("VBG") || adj
            .value().startsWith("PRP$"))) {
      return adj.value();
    }
    return null;
  }
  ...
  private static boolean checkOrderedNoun(MutablePair<Integer, Integer> rangeA,
    ...
    Tree np = ExperimentUtils.getTreeWithEdges(enTree, rangeA.first,
        rangeA.second + 1);
    if (np != null && (np.value().startsWith("N"))) {
      firstNP = true;
    }
    ...
    if (!firstNP) {
      for (int sep = rangeA.first; sep <= rangeA.second; sep++) {
        Tree n1 = ExperimentUtils.getTreeWithEdges(enTree, rangeA.first,
            sep + 1);
        Tree n2 = ExperimentUtils.getTreeWithEdges(enTree, sep + 1,
            rangeA.second + 1);
        if (n1 != null && n2 != null) {
    ...
    Tree bT = ExperimentUtils.getTreeWithEdges(enTree, rangeB.first,
        rangeB.second + 1);
    if (bT != null && (bT.value().startsWith("N"))) {
      secondNP = true;
    }
    ...
    if (!secondNP)
      for (int sep = rangeB.first; sep <= rangeB.second; sep++) {
        ...
        Tree n1 = ExperimentUtils.getTreeWithEdges(enTree, rangeB.first,
            sep + 1);
        Tree n2 = ExperimentUtils.getTreeWithEdges(enTree, sep + 1,
            rangeB.second + 1);
        if (n1 != null && n2 != null) {
  ...
  private static boolean checkSecondVP(MutablePair<Integer, Integer> rangeA,
    ...
    Tree bT = ExperimentUtils.getTreeWithEdges(enTree, rangeB.first,
        rangeB.second + 1);
    if (bT != null && (bT.value().startsWith("VP"))) {
      return true;
    }
  ...
  private static boolean checkFlippedRelativeClause(
    ...
    Tree relc = ExperimentUtils.getTreeWithEdges(enTree, rangeA.first,
        rangeA.second + 1);
    if (relc != null
        && (relc.value().startsWith("VP") || relc.value().startsWith("SBAR") ||
        ...
        // (NN award))))))
        (relc.value().startsWith("S") && relc.firstChild().value()
            .startsWith("VP")))) {
  ...
  private static void printNPwithDEtoFile(int fileidx, int npidx,
    ...
    npPW.printf("%d\t%d\t%s\t%d\t%s\t%s\n", fileidx, npidx, type,
        englishNP.size(), chStr, enStr);
  }

  private static void fixDeterminerOrOfOrWith(String[] translation,
      String[] source, int[][] matrix) {
    boolean err = false;
    ...
    if (err) {
      throw new RuntimeException("printGridNoNull FAILED.");
    }
    ...
    for (int cidx = 0; cidx < source.length; cidx++) {
      ...
      for (int eidx = translation.length - 1; eidx >= 0; eidx--) {
        if (!isDeterminerOrOfOrWith(translation[eidx])
            && matrix[eidx][cidx] > 0) {
          isNonDT = true;
        } else if (matrix[eidx][cidx] > 0
            && isDeterminerOrOfOrWith(translation[eidx]) && isNonDT) {
          // here it means that source[cidx] has linked to another word later
          // than
  ...
  private static boolean isDeterminerOrOfOrWith(String word) {
    ...
    if (word.equals("the") || word.equals("a") || word.equals("its")
        || word.equals("their") || word.equals("an") || word.equals("with"))
      return true;
    return false;
  }
    ...
    for (int cidx = deIndices.get(0) + 1; cidx < source.length; cidx++) {
      ...
      if (rangeB.second < translation.length && rangeB.second > 0) {
        for (int eidx = 0; eidx < rangeB.second; eidx++) {
          if ((!translation[eidx].equals("which") && !translation[eidx]
              .equals("where")) && matrix[eidx][cidx] > 0) {
            isNonWhichWhere = true;
          }
        }
        if ((translation[rangeB.second].equals("which") || translation[rangeB.second]
            .equals("where"))
            && matrix[rangeB.second][cidx] > 0
            && isNonWhichWhere) {
  ...
  private static boolean isPrep(String word) {
    if (word.equals("with") || word.equals("within") || word.equals("from")
        || word.equals("in") || word.equals("inside") || word.equals("for")
        || word.equals("on") || word.equals("between") || word.equals("by")
        || word.equals("among") || word.equals("at") || word.equals("under"))
      return true;

    return false;
  }

  private static void printGridNoNull(String[] translation, String[] source,
      int[][] matrix, PrintWriter pw) {
    boolean err = false;
    if (translation.length != matrix.length || translation.length == 0) {
      err = true;
    } else if (matrix[0].length != source.length || source.length == 0) {
    ...
    if (err) {
      System.err.println("printGridNoNull FAILED.");
      return;
    }
