    ...
    while ((parsedSent = reader.next()) != null) {
      ...
      for (int deIdx : markedDEIdxs) {
        ...
        if (!rootLabel.equals("NP")) {
          if (!newLeaves.get(deIdx).value().startsWith("的_"))
        ...
        if (!dePOS.value().equals("DEG") && !dePOS.value().equals("DEC")) {
        ...
        if (!de.startsWith("的_"))
          throw new RuntimeException(de + "(" + deIdx + ") in "
      ...
      while (!tPtr.isLeaf() || !q.isEmpty()) {
        ...
        for (Tree c : tPtr.children()) {
          if (!c.isLeaf() && c.label().toString().endsWith("r")) {
            needReorder = true;
      ...
      /*
       * parsedSent.pennPrint(); newTree.pennPrint();
       * System.out.println("---------------------------------------------");
       */
