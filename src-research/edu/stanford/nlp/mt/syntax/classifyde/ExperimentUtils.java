...
public class ExperimentUtils {
  ...
  public static void markRotatingDNPorCPinNP(Tree tree, int deIdx) {
    ...
    /*
     * if (!DNPorCP.label().toString().equals("DNP") &&
     * !DNPorCP.label().toString().equals("CP")) { return; }
     */
  ...
  static Tree processInternalDEReordering(Tree t) {
    ...
    if (moveIdx < 0)
      throw new RuntimeException("no internal 的_ ?");
  ...
  /**
   * From a given tree, get the DE position by checking immediate children of
   * the tree. if there are multiple ones, the last one will be chosen
   */
  ...
  public static Tree maskIrrelevantDEs(Tree tree, int deInTree) {
    ...
    if (!"的".equals(leaves.get(deInTree).value())) {
      ...
      System.err.println("leaves.get(deInTree).value()="
          + leaves.get(deInTree).value());
  ...
  static List<Pair<String, String>>[] readFinalCategories(String categoryFile,
    ...
    /*
     * for(int i = 0; i < categories.length; i++) { int fileid =
     * Integer.parseInt(fileids[i]); int npid = Integer.parseInt(npids[i]); if
     * (maxNP[fileid] < npid) { maxNP[fileid] = npid; } }
     * 
     * for(int i = 1; i <= 325; i++) { result[i] = new List[maxNP[i]+1]; for(int
     * j = 1; j <= maxNP[fileid]; j++) result[i][j] = new ArrayList<Pair<String,
     * String>>(); }
     */
    for (int i = 1; i <= 325; i++) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (fields.length != 4) {
        throw new RuntimeException(
            "finalCategories_all.txt should have 4 fields: " + lines[i]);
      }
  ...
  public static boolean is5class(String cat) {
    if ("A 's B".equals(cat) || "A B".equals(cat) || "A prep B".equals(cat)
        || "B prep A".equals(cat) || "relative clause".equals(cat)) {
      return true;
    }
    if ("no B".equals(cat) || "multi-DEs".equals(cat) || "other".equals(cat)) {
      return false;
    }
    throw new RuntimeException("the category [" + cat
        + "] is not valid in 'is5class'");
  }
  ...
  public static String short6class(String cat) {
    ...
    if ("A B".equals(cat))
      return "AB";
    if ("A prep B".equals(cat))
      return "AprepB";
    if ("B prep A".equals(cat))
      return "BprepA";
    if ("relative clause".equals(cat))
      return "relc";
    if ("no B".equals(cat))
      return "noB";
    throw new RuntimeException("the category [" + cat
        + "] is not valid in 'is6class'");
  }

  static void resultCoarseSummary(
      TwoDimensionalCounter<String, String> confusionMatrix) {
    TwoDimensionalCounter<String, String> cc = new TwoDimensionalCounter<String, String>();

    for (Map.Entry<String, ClassicCounter<String>> k : confusionMatrix
        .entrySet()) {
      String k1 = k.getKey();
  ...
  public static List<AnnotatedTreePair> readAnnotatedTreePairs(
      Boolean useReducedCategories, Boolean useNonOracleTrees)
      throws IOException {
    List<TreePair> treepairs = TransTBUtils
        .readAnnotatedTreePairs(useNonOracleTrees);
    List<AnnotatedTreePair> atps = new ArrayList<AnnotatedTreePair>();

    // Open the hand-annotate file
    // String finalCategoriesFile =
    // "C:\\cygwin\\home\\Pichuan Chang\\javanlp\\projects\\mt\\src\\mt\\classifyde\\data\\finalCategories_all.txt";
