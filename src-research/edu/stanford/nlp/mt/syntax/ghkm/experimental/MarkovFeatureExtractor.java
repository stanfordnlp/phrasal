...
public class MarkovFeatureExtractor extends AbstractFeatureExtractor {
  ...
  public static boolean headPos = Boolean.parseBoolean(System.getProperty(
      "headPos", "false"));
  public static boolean headWord = Boolean.parseBoolean(System.getProperty(
      "headWord", "false"));
  ...
  enum ReorderingType {
    stayLeft, stayRight, moveLeft, moveRight, exitLeft, exitRight, other
  }

  final IntArrayBackoffEstimator hM = new IntArrayBackoffEstimator(10), // head
    ...
    if (h != null && !h.emptySpan()) {
      ...
      for (int i = hIdx - 1; i >= 0; --i) {
        ...
        if (!l.emptySpan()) {
          ...
          if (l2 < h1)
            type = ReorderingType.stayLeft;
          else if (h2 < l1)
            type = ReorderingType.moveRight;
          ...
          if (doTrain) {
            ...
            System.err.printf("p=%f type=%s h=%s l=%s\trule=%s\n", p,
                type.toString(), h.label(), l.label(), rStr);
      ...
      for (int i = hIdx + 1; i < children.length; ++i) {
        ...
        if (!r.emptySpan()) {
          ...
          ReorderingType type = ReorderingType.other;
          if (r2 < h1)
            type = ReorderingType.moveLeft;
          else if (h2 < r1)
            type = ReorderingType.stayRight;
          int[] cat = new int[] { type.ordinal() };
          ...
          if (doTrain) {
            ...
            System.err.printf("p=%f type=%s h=%s r=%s\trule=%s\n", p,
                type.toString(), h.label(), r.label(), rStr);
            sumLogProb += Math.log(p);
            ++normLogProb;
          }
        }
