...
public class Alignment {
  ...
  public int[] num_alignments_infor_for_french;// how many eng words are aligned
                                               // to each french wrd
  public int[] num_alignments_infor_for_english;// how many french words are
                                                // aligned to each eng wrd
  public int[] min_pos_infor_for_french; // the min pos in english that aligned
                                         // to each french wrd
  ...
  public Alignment(String french_str, String english_str, String align_str) {
    ...
    for (int i = 0; i < french_wrds1.length; i++)
      french_wrds[i] = edu.stanford.nlp.mt.syntax.decoder.Symbol
          .add_terminal_symbol(french_wrds1[i]);

    String[] english_wrds1 = english_str.split("\\s+");
    english_wrds = new int[english_wrds1.length];
    for (int i = 0; i < english_wrds1.length; i++)
      english_wrds[i] = edu.stanford.nlp.mt.syntax.decoder.Symbol
          .add_terminal_symbol(english_wrds1[i]);
    ...
    for (int i = 0; i < links.length; i++) {
      ...
      int p_fr = Integer.valueOf(ids[0]);
      int p_eng = Integer.valueOf(ids[1]);
      if (p_fr < 0 || p_fr >= french_wrds.length || p_eng < 0
          || p_eng >= english_wrds.length) {
        System.out
            .println("alignment information error, compared with the number of wrds");
        System.exit(0);
      }
      ...
      min_pos_infor_for_french[p_fr] = min(p_eng,
          min_pos_infor_for_french[p_fr]);
      max_pos_infor_for_french[p_fr] = max(p_eng,
          max_pos_infor_for_french[p_fr]);
      min_pos_infor_for_english[p_eng] = min(p_fr,
          min_pos_infor_for_english[p_eng]);
      max_pos_infor_for_english[p_eng] = max(p_fr,
          max_pos_infor_for_english[p_eng]);
    }
  }
