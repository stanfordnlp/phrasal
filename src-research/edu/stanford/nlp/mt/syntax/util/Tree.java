...
public class Tree {
  ...
  public Tree(String tree_str) {
    ...
    for (int i = 0; i < tree_str.length(); i++) {
      if (tree_str.charAt(i) == '(') {// start a tag
        ...
        // if((tree_str.charAt(pos_space+1)=='(') &&
        // (tag.compareTo("PU")!=0)){//start a new tag, go back to mainloop
        if ((tree_str.charAt(pos_space + 1) == '(')
            && (tree_str.charAt(pos_space + 2) != ')')) {// start a new tag, go
                                                         // back to mainloop
          i = pos_space;// i++ will be always performed
        } else {// the current parent is closed
          int pos_closing = tree_str.indexOf(')', pos_space + 2);// pos of
                                                                 // closing-bracket
                                                                 // ")", skip 2
                                                                 // to avoid
                                                                 // case like
                                                                 // (PU ))
          String terminal_symbol = tree_str.substring(pos_space + 1,
              pos_closing);
          ...
          // check whether another tags are closed, clearly, a closing of tag
          // will only be intialized by the event that a preterminal is closed
          i++;// either skip space or looking for another ")"
          while (i < tree_str.length() && tree_str.charAt(i) == ')') {// end of
                                                                      // a tag
            t_stack.pop();
            i++;// either skip space or looking for another ")"
          }
  ...
  private void print_tree(Node root, BufferedWriter out) {
    if (root.name.compareTo("fake_root") != 0) {// not fake root
      ...
      FileUtility.write_lzf(out, "(" + root.name_after_reorder + " "
  ...
  public void print_tree_statistics(Node root) {
    if (root.name.compareTo("fake_root") != 0) {
      System.out.print("Name: " + root.name + "; " + "Terminal: "
          + root.terminal_symbol + "; " + "Children: " + root.l_children.size()
          + "\n");
    } else {
      System.out.print("\n------Tree Staistics are\n");
    }
  ...
  private void derive_complement_spans(Node root, int total_len) {// total_len:
                                                                  // len of
                                                                  // target
                                                                  // string
    // idea: my complement span is the union of: my parent's complement span +
    // my siblings's span
    ...
    if (root.parent == null) {// root nodes
      ...
      if (root.parent.complement_spans != null)
        v_of_spans.addAll(root.parent.complement_spans);// my parent's
                                                        // complement spans
  ...
  private void derive_span(Node root, Hashtable align_tbl, int[] terminal_pos) {
    ...
    /*
     * if(root.span!=null) System.out.println(root.name+": "
     * +root.span.toString());
     */
  }
  ...
  private void extract_rule(Node root, Hashtable rule_tbl, int len_tgt,
      BufferedWriter out) {
  ...
  public static class Node {
    ...
    int terminal_id = -1; // if this is a pre-terminal, then remember the
                          // terminal id (start from 0)
    Vector span = null;// according to ISI's syntaxMT, this only consider the
                       // first/last word index in the english string, e.g., 1-3
                       // or 10-10
    Vector complement_spans = null;// according to ISI's syntaxMT, it can be :
                                   // 1-3,6-8,10-10
    ...
    /*
     * return value 0: "AND"/"OR" name-matches-stage fail 1: "AND"
     * name-matches-stage susscessful, need to do full-chilren match 2: "OR"
     * name-matches-stage susscessful, means that for children match, we should
     * return true as long as one match the pattern, do not consider the number
     * of chilren
     */
    private int pattern_match(String from, String to) {
      if (from.matches("\\|.+")) {
        ...
        if (from.compareTo("*") == 0 || to.compareTo("*") == 0)// anything will
                                                               // match
          return 1;
    ...
    public boolean is_subsume(Node from, Hashtable tag_tbl) {// check whether
                                                             // the "from" is
                                                             // subsumed by
                                                             // myself, from may
                                                             // be taged by X
      ...
      if (from_str.matches("x\\d+\\:.+")) {
        tag_tbl.put(from_str.substring(0, 2), this);// TODO: assume the d is
                                                    // between [0,9]
        // System.out.println("from_str: " + from_str + " Size: "
        // +tag_tbl.size());
      }
      from_str = from_str.replaceAll("x\\d+\\:", "");// e.g., skip "x0:" in
                                                     // x0:NP
      ...
      if (res_match == 0) {// first, see whether the name match
        res = false;
        // tag_tbl.clear();
      } else if (res_match == 1) {// name match pass, need recursively "AND"
                                  // chilren match
        // System.out.println("AND children match");
        if (from.l_children.size() > 0) {// if from has no children, then return
                                         // true
          if (from.l_children.size() == this.l_children.size()) {
            ...
            for (int i = 0; i < from.l_children.size(); i++) {
              if (((Node) this.l_children.get(i)).is_subsume(
                  (Node) from.l_children.get(i), tag_tbl) == false) {
                res = false;
                // tag_tbl.clear();
                break;
              }
      ...
      } else if (res_match == 2) {// name match pass, need recursively "OR"
                                  // chilren match
        // System.out.println("OR children match");
        boolean t_res = false;
        if (from.l_children.size() == 1) {// In "OR" condition, must have one
                                          // and only one child
          for (int i = 0; i < this.l_children.size(); i++) {
            /*
             * note that we should not clear the tag_tbl if one of the chilren
             * fails, that's why we delete tag_tbl.clear()
             */
            if (((Node) this.l_children.get(i)).is_subsume(
                (Node) from.l_children.get(0), tag_tbl) == true) {// any sucess
              t_res = true;
              break;
            }
          }
    ...
    public boolean set_frontier_flag() {
      if (span == null) {// unaligned source node
        frontier_flag = false;
        return false;
      } else if (complement_spans == null || complement_spans.size() <= 0) {// span
                                                                            // over
                                                                            // all
                                                                            // the
                                                                            // english
        ...
        for (int i = 0; i < complement_spans.size(); i++) {
          ...
          if ((span_start >= ((Integer) t_comp.get(0)).intValue() && span_start <= ((Integer) t_comp
              .get(1)).intValue())
              || (span_end >= ((Integer) t_comp.get(0)).intValue() && span_end <= ((Integer) t_comp
                  .get(1)).intValue())
              || (span_start <= ((Integer) t_comp.get(0)).intValue() && span_end >= ((Integer) t_comp
                  .get(1)).intValue())// subsume
          ) {
            frontier_flag = false;
            return false;
          }
    ...
    private void ctrl_derive_subrule(int[] x_id, String[] str_lhs,
        String[] v_rhs_frags, String[] v_rhs_unaligned) {
    ...
    private void derive_subrule(int[] x_id, String[] str_lhs,
        String[] v_rhs_frags, String[] v_rhs_unaligned) {
      if (this.frontier_flag == true) {// note: pre-terminal can be frontier
                                       // node
        ...
        if (span == null) {
          ...
          add_rhs(v_rhs_unaligned, v_rhs_frags.length, terminal_symbol);// remember
                                                                        // how
                                                                        // many
                                                                        // continuos
                                                                        // span
          ...
          /*
           * TODO: now, we sort the words according to their start span, this
           * may not be good for non-coninous translation problem is due to
           * 1-to-m non-continuous translation
           */
    ...
    private void derive_rule(Hashtable rule_tbl, int len_tgt, BufferedWriter out) {
      // NP ||| (x0:DNP (LCP fake) (DEG fake)) (x1:NP fake) ||| x1 x0 ||| 0 0 0
      // 0 0
      if (frontier_flag == true && l_children.size() > 1) {// only extract rule
                                                           // for frontier node
                                                           // with more than one
                                                           // children
        int[] x_id = new int[1];
        ...
        String[] v_rhs_frags = new String[len_tgt];// the index is the start pos
                                                   // in the target, value is
                                                   // the rhs symbol
        String[] v_rhs_unaligned = new String[len_tgt + 1];// the index remember
                                                           // how many spans
                                                           // should put before
                                                           // it
        ...
        for (int start_pos = 0; start_pos < v_rhs_frags.length; start_pos++) {
          if (v_rhs_frags[start_pos] != null) {
            ...
            for (int n_left = 0; n_left < v_rhs_unaligned.length
                && n_left <= num_comsumed_span; n_left++) {
