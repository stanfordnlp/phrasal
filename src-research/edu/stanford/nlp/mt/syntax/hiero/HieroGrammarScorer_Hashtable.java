...
public class HieroGrammarScorer_Hashtable {
  ...
  public void score_grammar() {
    ...
    while (rules.hasNext()) {
      ...
      if (NUM_FEATS == 4) {
        new_scores[3] = -(float) Math.log10(rl.feat_scores[1] / weight);// weighted
                                                                        // avg
        new_scores[4] = -(float) Math.log10(rl.feat_scores[2] / weight);// weighted
                                                                        // avg
        if (Float.isInfinite(new_scores[3]) || Float.isInfinite(new_scores[4]))
          num_infinite++;
      }
      rl.feat_scores = new_scores;
      // System.out.println("f_sum " + fr_sum + " e_sum " + eng_sum +
      // " old: "+rl.feat_scores + " new " + new_scores);
      rl.print_info();
    }
    System.out.println("invalid is " + num_infinite
        + "; number of unique rules are " + p_gram.size());
    // dump the rules??
