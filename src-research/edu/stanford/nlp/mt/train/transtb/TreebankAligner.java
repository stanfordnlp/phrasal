...
/**
 * Generate a sentence-aligned parallel treebank from CTB and its translation as
 * distributed by LDC.
 * 
...
public class TreebankAligner {
  ...
  // Note: Simpler approach to alignment compared to Pichuan's implementation,
  // since
  ...
  public static void main(String[] args) throws IOException {
    if (args.length == 1)
      ...
      System.err
          .println("Usage: java edu.stanford.nlp.mt.train.transtb.TreebankAligner (outBasename)");
  }
  ...
  public TreebankAligner(String outDir) {
    ...
    if (af.exists()) {
      System.err.println("Target directory exists already : "
          + af.getAbsolutePath());
      System.exit(1);
    }
    if (uf.exists()) {
  ...
  public void align() throws IOException {
    ...
    for (fileidx = 1; fileidx <= 325; fileidx++) {
      ...
      if (aligned) {
        ...
        List<TranslationAlignment> align = autoAligned ? AlignmentUtils
            .readFromSentenceAlignedFiles(aname,
                aname.replace("_zh.snt", "_en.snt")) : AlignmentUtils
            .readFromFile(file);
    ...
    System.err.printf("Good alignments: %d/%d = %f\n", goodAl, totalAl, goodAl
        * 1.0 / totalAl);
  }

  public void printAlignedData(List<TreeYield> c, List<TreeYield> e,
      List<TranslationAlignment> align) {

    PrintStream cStringOut = IOTools.getWriterFromFile(String.format(
        "%s%03d_zh.snt", aDir, fileidx)), cTreeOut = IOTools
        .getWriterFromFile(String.format("%s%03d_zh.trees", aDir, fileidx)), eStringOut = IOTools
        .getWriterFromFile(String.format("%s%03d_en.snt", aDir, fileidx)), eTreeOut = IOTools
        .getWriterFromFile(String.format("%s%03d_en.trees", aDir, fileidx));
    ...
    for (TranslationAlignment ta : align) {
      ...
      Sequence<String> cStr = new SimpleSequence<String>(true,
          cNorm.split("\\s+"));
      Sequence<String> eStr = new SimpleSequence<String>(true,
          eNorm.split("\\s+"));
      // Max match:
  ...
  public void printUnalignedData(List<TreeYield> trees, String extension,
      boolean isChinese) {

    PrintStream stringOut = IOTools.getWriterFromFile(uDir + fileidx
        + extension + ".snt");
    PrintStream treeOut = IOTools.getWriterFromFile(uDir + fileidx + extension
        + ".trees");
  ...
  public void readTrees(String filename, List<TreeYield> trees,
      boolean isChinese) throws IOException {
    Reader reader;
    ...
    Iterator<Tree> i = new TreeTokenizerFactory(new PennTreeReaderFactory())
        .getTokenizer(new BufferedReader(reader));
  ...
  static String normalizeZh(String s) {
    return ChineseUtils.normalize(s, ChineseUtils.ASCII, ChineseUtils.ASCII,
        ChineseUtils.LEAVE);
  }
