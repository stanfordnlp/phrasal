...
/**
 * Annotate Chinese CoNLL2007 with dependency features extracted from Stanford
 * parsed sentences. extracted from parallel treebank.
 * 
...
public class AnnotateChineseCoNLL2007 {
  ...
  static void usage() {
    System.err
        .println("Usage: AnnotateCoNLL2007 (txt) (dep-file) (pcfg-trees) (headfinder) (features) (out-file)");
    System.exit(1);
  }
  ...
  public static void main(String args[]) throws Exception {
    ...
    if (totSent != fTrees.size())
      throw new RuntimeException(String.format(
          "Wrong number of trees: %d != %d\n", totSent, fTrees.size()));

    DependencyReader reader = DependencyReader.createDependencyReader(null,
        "CONLL", null);
      ...
      for (int fi = 0; fi < fYield.size(); ++fi) {
        ...
        for (Map.Entry<Integer, Set<String>> ry : deps.entrySet()) {
          ...
          for (String tp : ry.getValue()) {
            ...
            // Add pairwise features (features that file for specific word
            // pairs):
            if (tp.startsWith("type=")) {
              if (fts.contains("TYPE")) {
                String[] path = tp.replaceAll("^type=", "").split(":");
                String dist = (path.length >= 0) ? getDist(path.length + 1)
                    : "na";
              ...
              if (fts.contains("PD0"))
                feats[fi + 1].add(tp + pos);
              if (fts.contains("PD1"))
                feats[fi + 1].add(tp + getAtt(fi, fid) + pos);
              if (fts.contains("PD2"))
                feats[fi + 1].add(tp + getAttDist(fi, fid) + pos);
              if (tp.equals("P")) {
                // Add Chinese head as feature:
                if (fts.contains("SHW"))
                  feats[fid + 1].add("SHW=" + fSent[fid]);
              }
            }
            if (DEBUG) {
            ...
            if (pt != null) {
              ...
              if (gpt != null) {
                if (fts.contains("POS2"))
                  feats[fi + 1].add("P2=" + t.label() + "^" + pt.label() + "^"
                      + gpt.label());
              }
            }
          }
