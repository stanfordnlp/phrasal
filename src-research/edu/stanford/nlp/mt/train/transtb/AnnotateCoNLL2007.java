  ...
  public static void main(String[] args) throws Exception {
    ...
    while ((dep = reader.getNext()) != null) {
      ...
      if (fail) {
        ...
        System.err
            .printf("WARNING: CoNLL file does not match english sentences (see above).");
      }
              ...
              for (int ei : wa.f2e(fi)) {
                ...
                if (tp.startsWith("type=")) {
                  if (fts.contains("TYPE")) {
                    ...
                    /*
                     * for(int i=0; i<path.length; ++i) { String loc = (i==0) ?
                     * "S" : ((i+1==path.length) ? "E" : "M");
                  ...
                  if (fts.contains("PD0"))
                    feats[ei + 1].add(tp + pos);
                  if (fts.contains("PD1"))
                    feats[ei + 1].add(tp + getAtt(fi, fid) + pos);
                  if (fts.contains("PD2"))
                    feats[ei + 1].add(tp + getAttDist(fi, fid) + pos);
                  if (tp.equals("P")) {
                    // Add Chinese head as feature:
                    if (fts.contains("SHW"))
                      feats[ei + 1].add("SHW=" + fSent[fid]);
                  }
                }
                if (DEBUG) {
              ...
              if (pt != null) {
                ...
                if (gpt != null) {
                  if (fts.contains("POS2"))
                    feats[ei + 1].add("P2=" + t.label() + "^" + pt.label()
                        + "^" + gpt.label());
                }
              }
            }
