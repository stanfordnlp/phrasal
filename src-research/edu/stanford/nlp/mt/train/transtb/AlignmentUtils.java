...
/**
 * This class collects a lot of static functions, most of which are related to
 * processing the alignment grid. Some processes are with respect to the En/Ch
 * trees; some are just looking at the grid. This is like a preprocessing and
 * cleaning when we merge the Chinese and English sentences(trees) and the
 * alignment grid.
 * 
...
public class AlignmentUtils {
  ...
  public static boolean checkDeIsOf(String[] translation, String[] source,
      int[][] matrix, int deIdx) {
    boolean set = false;
    int deEidx = -1;
  ...
  public static boolean checkDeIsPrep(String[] translation, String[] source,
      int[][] matrix, int deIdx) {
    boolean set = false;
    int deEidx = -1;
    for (int eidx = 0; eidx < translation.length; eidx++) {
  ...
  private static boolean isPrep(String word) {
    return word.equals("with") || word.equals("within") || word.equals("from")
  ...
  public static void printGridNoNull(String[] translation, String[] source,
  ...
  public static List<TranslationAlignment> readFromSentenceAlignedFiles(
      String cFile, String eFile) throws IOException {
    List<TranslationAlignment> alignment_list = new ArrayList<TranslationAlignment>();
    ...
    if (eLines.length != cLines.length)
      throw new RuntimeException(String.format(
          "Two files with different number of lines: %s %s\n", cFile, eFile));
  ...
  public static TranslationAlignment fixAlignmentGrid_Scores(
      TranslationAlignment ta, String[] leaves) {
    String regex = "^(\\d+):(\\d+)$";
    ...
    while (needFix >= 0) {
      ...
      int needFixInTranslation = needFix - 1; // in translation_, NULL is not
                                              // there
  ...
  public static TranslationAlignment fixAlignmentGridOnTranslation_Poss_Neg(
      TranslationAlignment ta, String[] leaves) {
    // check if there's "'s" case
    int needFix = -1;
    boolean cannot = false;

    // System.err.println("alignment="+StringUtils.join(ta.translation_,
    // " ")+"<br>");
    // System.err.println("from tree="+StringUtils.join(leaves, " ")+"<br>");
    for (int i = 0; i < ta.translation_.length; i++) {
      String t = ta.translation_[i];
      if (i < leaves.length && !t.equals(leaves[i])) {
        if (t.endsWith("'s") && !t.equals("'s")) {
          needFix = i + 1;
          break;
        } // add one because 0 is NULL
        if (t.equals("cannot")) {
          needFix = i + 1;
          cannot = true;
          break;
      ...
      for (int i = 0; i < newTranslation.length; i++) {
        if (i < needFixInTranslation) {
          ...
          if (cannot) {
            ...
            newTranslation[i] = ta.translation_[i].substring(0,
                ta.translation_[i].length() - 2);
          }
        } else if (i == needFixInTranslation + 1) {
          if (cannot)
    ...
    if (fixed && DEBUG) {
      System.err
          .println("matrix changed on 'fixAlignmentGridOnTranslation_Poss_Neg'");
      System.err.println(StringUtils.join(ta.translation_, " "));
    }
    return ta;
  }
  ...
  public static void checkTranslationAlignmentAndEnTrees(
      TranslationAlignment ta, List<Tree> enTrees) {
    ...
    if (enFromAlignment.length != enFromTrees.size()) {
      ...
      System.out.println("ALGN: " + StringUtils.join(enFromAlignment, " ")
          + "<br>");
      System.out
          .println("TREE: " + StringUtils.join(enFromTrees, " ") + "<br>");
    } else {
      for (int i = 0; i < enFromTrees.size(); i++) {
        if (!enFromTrees.get(i).value().equals(enFromAlignment[i])) {
          ...
          System.out.println("ALGN: " + StringUtils.join(enFromAlignment, " ")
              + "<br>");
          System.out.println("TREE: " + StringUtils.join(enFromTrees, " ")
              + "<br>");
          break;
        }
      }
  ...
  public static void checkTranslationAlignmentAndChTrees(
      TranslationAlignment ta, List<Tree> chTrees) {
    ...
    if (chFromAlignment.length != chFromTrees.size()) {
      System.out.println("Check failed.<br>");
      System.out.println("ALGN: " + StringUtils.join(chFromAlignment, " ")
          + "<br>");
      System.out
          .println("TREE: " + StringUtils.join(chFromTrees, " ") + "<br>");
    } else {
      for (int i = 0; i < chFromTrees.size(); i++) {
        if (!chFromTrees.get(i).value().equals(chFromAlignment[i])) {
          ...
          System.out.println("ALGN: " + StringUtils.join(chFromAlignment, " ")
              + "<br>");
          System.out.println("TREE: " + StringUtils.join(chFromTrees, " ")
              + "<br>");
          break;
        }
      }
  ...
  private static List<List<Integer>> getIndexGroups(String[] leaves,
      String[] source) {
