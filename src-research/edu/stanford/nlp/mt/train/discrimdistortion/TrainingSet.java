...
public class TrainingSet implements Iterable<Datum> {
  ...
  public TrainingSet(final Index<DistortionModel.Feature> featIndex,
      final Index<DistortionModel.Class> classIndex,
      final int numExpectedFeatures) {
    examples = Collections.synchronizedList(new ArrayList<Datum>(
        numExpectedFeatures));
  ...
  public void addDatum(Datum d) throws RuntimeException {
    if (d.numFeatures() != featureIndex.size())
      throw new RuntimeException(String.format(
          "%s: Datum has dimension %d, but training set requires dimension %d",
          this.getClass().getName(), d.numFeatures(), featureIndex.size()));
    examples.add(d);
  }
  ...
  public void addFeatureParameters(DistortionModel.Feature feat,
      DistortionModel.FeatureType type, int offset, int dimension) {
  ...
  public int getFeatureOffset(DistortionModel.Feature f) {
    return (featureOffsets == null) ? -1 : featureOffsets.get(f);
  }

  public int getFeatureDimension(DistortionModel.Feature f) {
    return (featureDimensions == null) ? -1 : featureDimensions.get(f);
  }

  public DistortionModel.FeatureType getFeatureType(DistortionModel.Feature f) {
    return (featureTypes == null) ? null : featureTypes.get(f);
  }

  public Map<DistortionModel.Feature, Integer> getFeatureOffsets() {
    return Collections.unmodifiableMap(featureOffsets);
  }

  public Map<DistortionModel.Feature, Integer> getFeatureDimensions() {
    return Collections.unmodifiableMap(featureDimensions);
  }

  public Map<DistortionModel.Feature, DistortionModel.FeatureType> getFeatureTypes() {
  ...
  public String toString() {
    ...
    for (Map.Entry<DistortionModel.Feature, Integer> feat : featureOffsets
        .entrySet())
      sb.append(String.format("  %s (%d)\n", feat.getKey().toString(),
          feat.getValue()));
