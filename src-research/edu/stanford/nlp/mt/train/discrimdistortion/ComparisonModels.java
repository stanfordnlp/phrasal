...
public class ComparisonModels {
  ...
  private static void trainTestCounts(final String trainPrefix,
    ...
    for (Mode mode : Mode.values()) {
      if (mode == Mode.Train2)
        continue; // Not needed for this model
      ...
      TrainingSet ts = fe.extract(new HashIndex<DistortionModel.Feature>(),
          classIndex, 14000);
      if (mode == Mode.Test)
        testAlignments = ts.getNumExamples();

      for (Datum d : ts) {
        DistortionModel.Class goldClass = DistortionModel
            .discretizeDistortion((int) d.getTarget());
        if (mode == Mode.Train)
      ...
      if (mode == Mode.Train) {
        ...
        for (DistortionModel.Class c : counts.keySet()) {
          String params = String.format("%s %f", c.toString(),
              counts.getCount(c));
  ...
  private static void trainTestMoses(final String trainPrefix,
      final String testPrefix) {
    ...
    int meanNum = 0;
    for (Mode mode : Mode.values()) {
      String filePrefix = (mode == Mode.Train || mode == Mode.Train2) ? trainPrefix
          : testPrefix;
      LineNumberReader algnReader = IOTools.getReaderFromFile(filePrefix
          + algnExtension);

      // Estimate the parameters
      try {
        while (algnReader.ready()) {
          StringTokenizer alignTokenizer = new StringTokenizer(
              algnReader.readLine());
          ...
          while (alignTokenizer.hasMoreTokens()) {
            ...
            if (alignmentMap.containsKey(tIdx))
              System.err
                  .printf(
                      "#WARNING many-to-one alignment at line %d. Are you using the intersect heuristic?\n",
          ...
          for (Map.Entry<Integer, Integer> alignment : alignmentMap.entrySet()) {
            ...
            if (mode == Mode.Train) {
              ...
              logLik += (-1.0 * Math.log(2.0 * b_hat))
                  - (Math.abs(((double) distortion) - u_hat) / b_hat);
          }
        }
  ...
  public static void main(String[] args) {
    if (args.length != 3) {
      System.err
          .println("usage: ComparisonModels [-m|-c] trainPrefix testPrefix");
      System.exit(-1);
    }
