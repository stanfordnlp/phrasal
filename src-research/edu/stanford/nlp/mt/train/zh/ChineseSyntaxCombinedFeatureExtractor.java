...
public class ChineseSyntaxCombinedFeatureExtractor {
  ...
  static {
    REQUIRED_OPTS.addAll(Arrays.asList(F_CORPUS_OPT, E_CORPUS_OPT,
        A_CORPUS_OPT, EXTRACTORS_OPT));
    OPTIONAL_OPTS.addAll(Arrays.asList(FILTER_CORPUS_OPT, FILTER_LIST_OPT,
        REF_PTABLE_OPT, SPLIT_SIZE_OPT, OUTPUT_FILE_OPT, NO_ALIGN_OPT,
        ...
        IBM_LEX_MODEL_OPT, PTABLE_PHI_FILTER_OPT, PTABLE_LEX_FILTER_OPT,
  ...
  public static Sequence<IString>[] getPhrasesFromDevCorpus(String fFilterCorpus) {
    return null;
    /*
     * sourceFilter.addPhrasesFromCorpus(fFilterCorpus, maxPhraseLenF,
     * Integer.MAX_VALUE, false); AlignmentTemplates tmpSet = new
     * AlignmentTemplates();
     * System.err.println("Filtering against corpus: "+fFilterCorpus);
     * filterFromDev = true; try { LineNumberReader fReader =
     * IOTools.getReaderFromFile(fFilterCorpus); for (String fLine; (fLine =
     * fReader.readLine()) != null; ) { Sequence<IString> f = new
     * SimpleSequence<IString>(true,
     * IStrings.toIStringArray(fLine.split("\\s+"))); for(int i=0; i<f.size();
     * ++i) { for(int j=i; j<f.size() && j-i<maxPhraseLenF; ++j) {
     ...
     * tmpSet.addForeignPhraseToIndex(fPhrase); } } } fReader.close(); }
     * catch(IOException e) { e.printStackTrace(); } Sequence<IString>[] phrases
     * = new Sequence[tmpSet.sizeF()]; for(int i=0; i<phrases.length; ++i) {
     * int[] fArray = tmpSet.getF(i); phrases[i] = new
     * SimpleSequence<IString>(true, IStrings.toIStringArray(fArray)); }
     * Collections.shuffle(Arrays.asList(phrases)); return phrases;
     */
  }
  ...
  public void restrictExtractionTo(int start, int end) {
    ...
    /*
     * filterFromDev = true; for(int i=start; i<end && i<list.length; ++i) {
     * Sequence<IString> f = list[i]; if(SHOW_PHRASE_RESTRICTION)
   ...
   * 
   */
  public void extractFromMergedAlignment(String fCorpus, String eCorpus,
      String aCorpus) {
    if (!filterFromDev)
      System.err
  ...
  private void extractPhraseFeatures(SymmetricalWordAlignment sent) {
    ...
    for (int e1 = 0; e1 < esize; ++e1) {
      for (int e2 = e1; e2 < esize; ++e2) {
        ...
        int F1 = f1, F2 = f2;
        // while(F1-1>=0 && f2-F1<maxPhraseLenF-1 && sent.f2e(F1-1).size()==0) {
        // --F1; }
        // while(F2+1<fsize && F2-f1<maxPhraseLenF-1 &&
        // sent.f2e(F2+1).size()==0) { ++F2; }
        while (F1 - 1 >= 0 && sent.f2e(F1 - 1).size() == 0) {
          --F1;
        }
        while (F2 + 1 < fsize && sent.f2e(F2 + 1).size() == 0) {
          ++F2;
        }
        ...
        for (int i = F1; i <= f1; ++i)
          for (int j = f2; j <= F2; ++j) {
            if (j - i < maxPhraseLenF && e2 - e1 < maxPhraseLenE) {
              if (VERBOSE)
                System.err.printf("Add to Al Grid: f(%d-%d) e(%d-%d)\n", i, j,
                    e1, e2);
              addAlTemp(sent, i, j, e1, e2);
            }
            alTemp = new AlignmentTemplateInstance(sent, i, j, e1, e2, 1.0f);
            if (VERBOSE)
              System.err.printf("Add to Full Grid: f(%d-%d) e(%d-%d)\n", i, j,
                  e1, e2);
  ...
  private void addAlTemp(SymmetricalWordAlignment sent, int f1, int f2, int e1,
      int e2) {
  ...
  private boolean checkAlignmentConsistency(SymmetricalWordAlignment sent,
      int f1, int f2, int e1, int e2) {
      ...
      for (AbstractChineseSyntaxFeatureExtractor<String> e : extractors) {
        ...
        if (scores instanceof double[]) { // as dense vector
          ...
          throw new UnsupportedOperationException(
              "AbstractChineseSyntaxFeatureExtractor should return double[] or Counter, not "
                  + scores.getClass());
        }
      }
      assert (oStream != null);
  ...
  // public void setMaxPhraseLenE(int newMaxPhraseLenE) { maxPhraseLenE =
  // newMaxPhraseLenE; }
  // public void setMaxPhraseLenF(int newMaxPhraseLenF) { maxPhraseLenF =
  // newMaxPhraseLenF; }
  ...
  public static boolean multiPassFeatureExtract(Properties prop) {
    ...
    boolean withAlign = !Boolean.parseBoolean(prop.getProperty(NO_ALIGN_OPT,
        "false"));
    String outputFile = prop.getProperty(OUTPUT_FILE_OPT);
    // Split filter list into N chunks:
    if (numSplits > 1) {
      if (!filterFromDev) {
        System.err.println("-" + SPLIT_SIZE_OPT
            + " argument only possible with -" + FILTER_CORPUS_OPT + ", -"
            + FILTER_LIST_OPT + ".");
        return false;
      }
      ...
      while (startLine < fPhrases.length) {
        ChineseSyntaxCombinedFeatureExtractor combined = new ChineseSyntaxCombinedFeatureExtractor(
            prop);
  ...
  public static void usage() {
    System.err
        .print("Usage: java ChineseSyntaxCombinedFeatureExtractor [ARGS]\n"
            + "Mandatory arguments:\n"
