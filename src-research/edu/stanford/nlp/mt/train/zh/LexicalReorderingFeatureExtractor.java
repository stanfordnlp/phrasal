...
public class LexicalReorderingFeatureExtractor extends
    AbstractChineseSyntaxFeatureExtractor<String> {
  ...
  enum ReorderingTypes {
    monotone, discontinuous, swap
  }

  private DirectionTypes directionType;

  private final static int LAPLACE_SMOOTHING = 0;
  ...
  public void extract(AlignmentTemplateInstance alTemp, AlignmentGrid alGrid,
    ...
    int f1 = alTemp.fStartPos(), f2 = alTemp.fEndPos(), e1 = alTemp.eStartPos(), e2 = alTemp
        .eEndPos();
    ...
    if (directionType == DirectionTypes.forward
      ...
      if (ChineseSyntaxCombinedFeatureExtractor.VERBOSE)
        System.err.printf("ADD FWD: (%s) %s\n", type1, alTemp.toString(false));
    ...
    if (directionType == DirectionTypes.backward
      ...
      if (ChineseSyntaxCombinedFeatureExtractor.VERBOSE)
        System.err.printf("ADD BWD: (%s) %s\n", type2, alTemp.toString(false));
  ...
  public void report(AlignmentTemplates alTemps) {
    ...
    for (int idx = 0; idx < alTemps.size(); ++idx) {
      ...
      if (sumP_f != sumN_f) {
        throw new RuntimeException("sumP_f != sumN_f");
      }
      System.out.printf(
          "REPORT:\t%s\t%d\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n", alTemp.f()
              .toString(), sumP_f, (double) countsP_f[0] / sumP_f,
          (double) countsP_f[1] / sumP_f, (double) countsP_f[2] / sumP_f,
          (double) countsN_f[0] / sumN_f, (double) countsN_f[1] / sumN_f,
  ...
  public Object score(AlignmentTemplate alTemp) {
    ...
    int sumN_f = countsN_f[0] + countsN_f[1] + countsN_f[2];
    /*
     * return new int[] { countsP[0]/sumP, countsP[2]/sumP, countsP[1]/sumP,
     * countsN[0]/sumN, countsN[2]/sumN, countsN[1]/sumN };
     */
    if (sumP != sumN) {
      throw new RuntimeException("sumP != sumN");
    }
    if (sumP_f != sumN_f) {
      throw new RuntimeException("sumP_f != sumN_f");
    }
    return new double[] { sumP, sumP_f };
  }

  private static boolean isConnectedTopLeft(AlignmentGrid fullAlGrid, int fi,
      int ei) {
    int cellf = fi - 1;
    int celle = ei - 1;
    if (cellf < 0 || celle < 0 || cellf >= fullAlGrid.fsize()
        || celle >= fullAlGrid.esize())
      return false;
    AlGridCell<AlignmentTemplateInstance> cell = fullAlGrid
        .cellAt(cellf, celle);
    return cell != null && cell.hasBottomRight();
  }

  private static boolean isConnectedTopRight(AlignmentGrid fullAlGrid, int fi,
      int ei) {
    int cellf = fi + 1;
    int celle = ei - 1;
    if (cellf < 0 || celle < 0 || cellf >= fullAlGrid.fsize()
  ...
  private static boolean isConnectedBottomLeft(AlignmentGrid fullAlGrid,
      int fi, int ei) {
    int cellf = fi - 1;
    int celle = ei + 1;
    if (cellf < 0 || celle < 0 || cellf >= fullAlGrid.fsize()
  ...
  private static boolean isConnectedBottomRight(AlignmentGrid fullAlGrid,
      int fi, int ei) {
    int cellf = fi + 1;
    int celle = ei + 1;
    if (cellf < 0 || celle < 0 || cellf >= fullAlGrid.fsize()
  ...
  private static void addCountToArray(ArrayList<Object> list,
      ReorderingTypes type, AlignmentTemplate alTemp) {
    int idx = alTemp.getKey();
    ...
    int[] counts; // = null;
    while (idx >= list.size())
      list.add(new int[] { LAPLACE_SMOOTHING, LAPLACE_SMOOTHING,
          LAPLACE_SMOOTHING });
