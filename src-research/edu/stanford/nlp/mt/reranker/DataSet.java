...
public class DataSet implements Serializable {
  ...
  public String getFromNBest(int sentId, int hypId) {
    if (nbests == null)
      throw new RuntimeException("Sentences from Nbest lists don't exist!");
  ...
  public String[] getFromRefs(int sentId) {
    if (refs == null)
      throw new RuntimeException("Sentences from Ref lists don't exist!");
   ...
   * { List<CompactHypothesisList> sublchl = new
   * ArrayList<CompactHypothesisList>();
   * //System.err.println("DEBUG: getSubset"); for (int i = range[0]; i <=
   * range[1]; i++) { if (i>=exclude[0] && i<=exclude[1]) {
   * //System.err.println("\texcluding "+i); continue; }
   * //System.err.println("\tincluding "+i); if (lchl[i]==null) throw new
   * RuntimeException("nothing in compact hypothesis list "+i+"??");
   * sublchl.add(lchl[i]); } return sublchl; }
   */
   ...
   * List<CompactHypothesisList> sublchl = new
   * ArrayList<CompactHypothesisList>();
   * //System.err.println("DEBUG: getSubset (only incl)"); for (int i =
   * range[0]; i <= range[1]; i++) { if (range.length==4 && i>=range[2] &&
   * i<=range[3]) { continue; } //System.err.println("\t"+i); if (lchl[i]==null)
  ...
  public int[] getTrainRange() {
    int[] trainRange_exclude = { trainRange[0], trainRange[1], devRange[0],
        devRange[1] };
    return trainRange_exclude;
  }

  public int[] getDevRange() {
    int[] trainRange_exclude = { devRange[0], devRange[1], -1, -1 }; // not
                                                                     // exclud
                                                                     // anything
    return trainRange_exclude;
  }
  ...
  static public DataSet load(String dataSetDescriptor) throws IOException {
    ...
    if (n != null) {
      ...
      System.err
          .println("In Dataset.load, setting max #hypotheses to nThresh = "
              + nThresh);
    }
    ...
    try {
      ...
      try {
        ...
        System.out
            .printf("Data set successfully loaded as serialized .gale.DataSet\n");
  ...
  static private DataSet loadSerialized(String filename) throws IOException,
  ...
  static private DataSet loadByTextDescriptor(String filename)
    ...
    if (sentsFn == null) {
      System.err
          .println("Warning: NBestList is not provided in the datadescriptor. The corpus BLEU cannot be evaluated.");
      ds.nbests = null;
    } else {
    ...
    if (sentsFn == null) {
      System.err
          .println("Warning: RefList is not provided in the datadescriptor. The corpus BLEU cannot be evaluated.");
      ds.refs = null;
    } else {
    ...
    if (ds.trainRange != null)
      max = ds.trainRange[1];
    if (ds.devRange != null && ds.devRange[1] > max)
      max = ds.devRange[1];
    if (ds.evalRange != null && ds.evalRange[1] > max)
      max = ds.evalRange[1];
    int min = Integer.MAX_VALUE;
    if (ds.trainRange != null)
      min = ds.trainRange[0];
    if (ds.devRange != null && ds.devRange[0] < min)
      min = ds.devRange[0];
    if (ds.evalRange != null && ds.evalRange[0] < min)
      min = ds.evalRange[0];
    ...
    System.err
        .println("(DEBUG) for FeatureSetBank.load, only load up to (sid) max="
            + max);
    ...
    System.err.println("(DEBUG) for Scores.load, only load up to (sid) max="
        + max);
    ...
    for (FeatureSetBank featureSet : featureSets) {
      if (!scoresDataPts.containsAll(featureSet.getDataPointIndices())) {
        System.err.println("featureSet.getDataPointIndices().size()="
            + featureSet.getDataPointIndices().size());
    ...
    for (int dataPt = min; dataPt <= max; dataPt++) {
      if (dataPt >= ds.devRange[0] && dataPt <= ds.devRange[1]) {
        System.err
            .println("(Training) Making compact hypothesis list for data point "
                + dataPt);
        CompactHypothesisList chl = new CompactHypothesisList(trnFeatureIndex,
  ...
  static public void outputPredictedIndices(
      AbstractOneOfManyClassifier classifier, MyList<CompactHypothesisList> lchl)
      throws Exception {
