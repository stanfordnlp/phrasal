...
public class SSVMLearner extends AbstractOneOfManyClassifier {
  public static final StructuredSVM.StructLoss DEFAULT_STRUCT_LOSS = StructuredSVM.StructLoss.MarginRescale;
  ...
  HashMap<CompactHypothesisList, ArrayList<SparseVector>> svCache = new HashMap<CompactHypothesisList, ArrayList<SparseVector>>();
  ...
  double[] getAllScores(CompactHypothesisList chl) {
    ...
    if (vecs == null) {
      ...
      for (int i = 0; i < fIndices.length && fIndices[i] != null; i++) {
        SparseVector sv = new SparseVector(fIndices[i],
            ArrayUtils.toDouble(fValues[i]));
        vecs.add(sv);
      }
    }
  ...
  private class BestOfManyRLF implements RiskyLabelingFinder {
    public RiskyLabelingRecord find(int dataPointIdx, StructuredSVM pSsvm) {
      if (riskyPsi[dataPointIdx].length == 0) {
        return null;
      }
  ...
  public void learn(List<CompactHypothesisList> lchl) {
    ...
    for (int i = 0; i < lchl.size(); i++) {
      ...
      for (int j = 0, k = 0; j < cntNonNull; j++) {
        ...
        sharedRiskyPsi[k] = new SparseVector(fIndices[j],
            ArrayUtils.toDouble(fValues[j]));
        k++;
      }

      for (int subIdx = 0; subIdx < cntNonNull; subIdx++) {
        if (bleus[subIdx] != bleus[bestBleu])
          continue;
