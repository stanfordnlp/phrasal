...
/**
 * Given two Hiero/Moses system (1-best) output files, performs a statistical
 * significance test to determine whether the system outputs are different or
 * not. Currently only works with Bleu.
 **/
public class SignificanceTester {
  ...
  public static void main(String[] origArgs) throws IOException {
    ...
    if (prop.containsKey("StatsFactory")) {
      try {
        sf = (StatsFactory) Class.forName(prop.getProperty("StatsFactory"))
            .newInstance();
      } catch (Exception e) {
        e.printStackTrace();
    ...
    if (args.length < 3)
      throw new RuntimeException(
          "expecting two system output filenames, and one or more reference filenames");

    System.out.println("System 1 output: " + args[0]);
    System.out.println("System 2 output: " + args[1]);
    for (int i = 0; i < args.length - 2; i++)
      System.out.println("Reference " + i + ": " + args[i + 2]);

    BufferedReader[] rin = new BufferedReader[args.length - 2];

    System.err.println("Reading system 1 output...");
    for (int i = 0; i < rin.length; i++)
      rin[i] = new BufferedReader(new FileReader(args[i + 2]));
    Stats[] system1 = BleuFlatFileReader.read(new BufferedReader(
        new FileReader(args[0])), rin, sf);

    System.err.println("Reading system 2 output...");
    for (int i = 0; i < rin.length; i++)
      rin[i] = new BufferedReader(new FileReader(args[i + 2]));
    Stats[] system2 = BleuFlatFileReader.read(new BufferedReader(
        new FileReader(args[1])), rin, sf);

    if (system1.length != system2.length)
      throw new RuntimeException(
          "Two system output corpora should be of the same length, because they're supposed to be the same output, dummy!");
    ...
    for (int i = 0; i < NUM_ITERS; i++) {
      ...
      if (Math.abs(bleu1.score() - bleu2.score()) >= diff)
        num_gte++;
    }
    ...
    System.out
        .printf(
            "Out of %d samples, %d had a difference greater than or equal to the\n"
    ...
    if (p < GOOD_P_THRESH)
      System.out.printf(
          "Since p < %.2f, you may safely claim that the difference between the two\n"
              + "systems is statistically significant.\n", GOOD_P_THRESH);
    else if (p < MAYBE_P_THRESH)
      System.out
          .printf(
              "Since p >= %.2f, you MAY NOT claim that the difference between the two\n"
      ...
      System.out
          .printf(
              "Since p >= %.2f, you MAY NOT claim that the difference between the two\n"
                  ...
                  + "different under the given metric.", GOOD_P_THRESH,
              MAYBE_P_THRESH);
