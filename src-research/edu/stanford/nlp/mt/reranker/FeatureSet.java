...
public class FeatureSet {
  ...
  Map<Integer, Map<Integer, Map<String, Double>>> dataSetMap = new HashMap<Integer, Map<Integer, Map<String, Double>>>();
  ...
  public static void pruneFeaturesByCount(FeatureSet featSet, int threshold,
      ClassicCounter<String> cnt) {
    /*
     * Counter cnt = new Counter(); for (Map.Entry<Integer,Map<Integer,
     * Map<String, Double>>> e : dataSetMap.entrySet()) { int dataPt =
     * e.getKey(); Map<Integer, Map<String, Double>> v = e.getValue(); for
     * (Map.Entry<Integer, Map<String, Double>> sentE : v.entrySet()) { int
     * hypId = sentE.getKey(); Map<String,Double> feats = sentE.getValue(); for
     * (Map.Entry<String,Double> feat : feats.entrySet()) {
     * cnt.incrementCount(feat.getKey(), feat.getValue()); } } }
     */
    for (Map.Entry<Integer, Map<Integer, Map<String, Double>>> e : featSet.dataSetMap
        .entrySet()) {
      ...
      for (Map.Entry<Integer, Map<String, Double>> sentE : v.entrySet()) {
        ...
        for (Map.Entry<String, Double> feat : feats.entrySet()) {
          ...
          if (cnt.getCount(featName) < threshold
              && !featName.startsWith("moses")) {
  ...
  public static FeatureSet load(String filename) throws IOException {
    ...
    try {
      breader = new BufferedReader(new InputStreamReader(new GZIPInputStream(
    ...
    for (String line; (line = breader.readLine()) != null;) {
      ...
      for (int i = 1; i < fields.length; i++) {
        ...
        double v = (featurePair.length == 1 ? 1.0 : Double
            .parseDouble(featurePair[1]));
        featureSet.setFeature(dataPt, hypId, featurePair[0], v);
        cnt.incrementCount(featurePair[0], v);
      }
    }
    int pruneThresh = Integer.parseInt(System.getProperty(PRUNE_THRESH_PROP,
        DEFAULT_PRUNE_THRESH));
    System.err
        .println("Pruning features with counts lower than " + pruneThresh);
  ...
  public void write(String filename) throws IOException {
    ...
    for (Integer dataPt : new TreeSet<Integer>(dataSetMap.keySet())) {
      pstrm.printf("\n# Data Pt %d\n", dataPt);
      for (Integer hypId : new TreeSet<Integer>(dataSetMap.get(dataPt).keySet())) {
        pstrm.printf("%d,%d", dataPt, hypId);
        for (String feat : new TreeSet<String>(dataSetMap.get(dataPt)
            .get(hypId).keySet())) {
          pstrm.printf(" %s:%f", feat,
              dataSetMap.get(dataPt).get(hypId).get(feat));
        }
        pstrm.println();
      }
    }
  ...
  public void setFeature(int dataPtIdx, int hypothesisIdx, String featureName,
      double featureVal) {
