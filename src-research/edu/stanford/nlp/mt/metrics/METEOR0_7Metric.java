...
public class METEOR0_7Metric {
  ...
  /**
   * Score a list of hypotheses again a single list of references. There is
   * currently no support for multi-references.
   * 
   */
  public static Map<String, Double> score(
    ...
    try {
      ...
      File refFile = File
          .createTempFile(rootName, ".ref.sgm", new File("/tmp"));
      File hypFile = File
          .createTempFile(rootName, ".hyp.sgm", new File("/tmp"));
      File outFile = File.createTempFile(rootName, ".meteor", new File("/tmp"));
      File logFile = File.createTempFile(rootName, ".meteor.log", new File(
          "/tmp"));
      ...
      String cmd = "perl /u/nlp/packages/meteor-0.7/meteor.pl -s machine1 "
          + " -r " + refFile.getAbsolutePath() + " -t "
          + hypFile.getAbsolutePath() + " -plainOutput -outFile "
      ...
      for (String line : IOUtils.slurpFileNoExceptions(outFile).split("\\n")) {
        ...
        if (sid != i)
          throw new IOException(
              String
                  .format(
                      "segment ids somehow got out of sync. Expected id=%d, but found id=%d.",
                      i, sid));
      ...
      // ("Number of scores (%d) does not match the number of sentences (%d)",scores.size(),
      // refsAndHyps.size()));
