...
public class TERtest {
  ...
  public static void main(String[] args) {
    ...
    costfunc._substitute_cost = (Double) paras
        .get(TERpara.OPTIONS.SUBSTITUTE_COST);
    ...
    if (!verifyFormats(in_ref_format, in_hyp_format, formats))
      System.exit(1);
    ...
    BufferedWriter xml_out = openFile(formats, "xml", out_pfx, hyp_fn, ref_fn,
        reflen_fn, caseon);
    BufferedWriter pra_out = openFile(formats, "pra", out_pfx, hyp_fn, ref_fn,
        reflen_fn, caseon);
    BufferedWriter prm_out = openFile(formats, "pra_more", out_pfx, hyp_fn,
        ref_fn, reflen_fn, caseon);
    BufferedWriter ter_out = openFile(formats, "ter", out_pfx, hyp_fn, ref_fn,
        reflen_fn, caseon);
    BufferedWriter sum_out = openFile(formats, "sum", out_pfx, hyp_fn, ref_fn,
        reflen_fn, caseon);
    BufferedWriter nbt_out = openFile(formats, "sum_nbest", out_pfx, hyp_fn,
        ref_fn, reflen_fn, caseon);

    // 6. compute TER
    Pattern id_rank = Pattern.compile("^\\s*(.*):([^ ]*)\\s*$",
      ...
      if (refsegs.containsKey(id_nrank)) {
        ...
        if (reflensegs != null) {
          ...
          if (reflenseglist == null)
            System.out
                .println("Warning: NO reference length can be found for hyp: "
                    + refids);
        }
        ...
        TERalignment result = score_all_refs((String) hyps.get(0),
            (ArrayList) refsegs.get(id_nrank), reflenseglist, refids, refspan,
            hypspan, costfunc);
        ...
        try {
          if (ter_out != null)
            ter_out.write(id + " " + result.numEdits + " " + result.numWords
                + " " + result.score() + "\n");
          if (xml_out != null)
            TERsgml
                .writeXMLAlignment(xml_out, result, id, (in_ref_format == 1));
          if (pra_out != null)
    ...
    System.out.println("Number of calls to beam search: " + ter.numBeamCalls());
    System.out.println("Number of segments scored: " + ter.numSegsScored());
    System.out.println("Number of shifts tried: " + ter.numShiftsTried());
  }

  public static BufferedWriter openFile(ArrayList formats, String type,
      String out_pfx, String hyp_fn, String ref_fn, String reflen_fn,
      boolean caseon) {
      ...
      try {
        ...
        if (type.equals("xml"))
          ...
          bw.write("Hypothesis File: " + hyp_fn + "\nReference File: " + ref_fn
              + "\n" + "Ave-Reference File: "
              + ((reflen_fn == "") ? ref_fn : reflen_fn) + "\n");
          bw.write(String
              .format(
                  "%1$-19s | %2$-4s | %3$-4s | %4$-4s | %5$-4s | %6$-4s | %7$-6s | %8$-8s | %9$-8s\n",
                  "Sent Id", "Ins", "Del", "Sub", "Shft", "WdSh", "NumEr",
                  "NumWd", "TER"));
          bw.write("-------------------------------------------------------------------------------------\n");
  ...
  public static void closeFile(BufferedWriter bw, String type) {
    ...
    if (bw != null) {
      try {
        if (type.equals("xml"))
          ...
          bw.write(String
              .format(
                  "%1$-19s | %2$-4d | %3$-4d | %4$-4d | %5$-4d | %6$-4d | %7$-6.1f | %8$-8.3f | %9$-8.3f\n",
                  "TOTAL", tot_ins, tot_del, tot_sub, tot_sft, tot_wsf,
                  tot_err, tot_wds, tot_err * 100.0 / tot_wds));
        }
        bw.close();
      } catch (IOException ioe) {
        System.out.println(ioe);
  ...
  public static void writeSummary(BufferedWriter sum, TERalignment result,
      String id) {
    try {
      result.scoreDetails();
      ...
      sum.write(String
          .format(
              "%1$-19s | %2$4d | %3$4d | %4$4d | %5$4d | %6$4d | %7$6.1f | %8$8.3f | %9$8.3f\n",
              id, result.numIns, result.numDel, result.numSub, result.numSft,
              result.numWsf, result.numEdits, result.numWords,
              result.score() * 100.0));
  ...
  public static void writeNbestSum(BufferedWriter nbt, TERalignment result,
      String id) {
    try {
      result.scoreDetails();

      nbt.write(String.format(
          "%1$-19s %2$4d %3$4d %4$4d %5$4d %6$4d %7$6.1f %8$8.3f %9$8.3f\n",
          id, result.numIns, result.numDel, result.numSub, result.numSft,
          result.numWsf, result.numEdits, result.numWords,
          result.score() * 100.0));
  ...
  public static boolean verifyFormats(int in_ref_format, int in_hyp_format,
      ArrayList out_formats) {
    if (in_ref_format != in_hyp_format) {
      System.out
          .println("** Error: Both hypothesis and reference have to be in the SAME format");
      return false;
    } else if (in_ref_format == 1 && out_formats.indexOf("xml") > -1) {
      System.out
          .println("** Warning: XML ouput may not have correct doc id for Trans format inputs");
      return true;
    } else
      return true;
  }

  public static TERalignment score_all_refs(String hyp, List refs,
      List reflens, List refids, String refspan, String hypspan,
      TERcost costfunc) {
    double totwords = 0;
    String ref;
    String refid = "";
    ...
    if (has_span && refs.size() > 1) {
      System.out
          .println("Error, translation spans should only be used with SINGLE reference");
      System.exit(1);
    }
