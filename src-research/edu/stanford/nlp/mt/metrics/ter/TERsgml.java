...
public class TERsgml {
  ...
  /********************************************************
   * Name: parse Desc: It tries to parse the file with the name given first, as
   * XML then, as SGML if we failed to parse it as XML last, return document if
   * successful or null.
   ********************************************************/
    ...
    try {
      ...
      builder.setErrorHandler(new org.xml.sax.ErrorHandler() {
        ...
        public void fatalError(SAXParseException sxe) throws SAXException {
          /*
           * System.out.println("** SAXException" + ", line " +
           * sxe.getLineNumber() + ", uri " + sxe.getSystemId());
        ...
        public void error(SAXParseException spe) throws SAXParseException {
          System.out.println("** SAXParseException" + ", line "
        ...
        public void warning(SAXParseException warn) throws SAXParseException {
          System.out.println("** Warning" + ", line " + warn.getLineNumber()
  ...
  /********************************************************
   * Name: loadSegs Desc: Loads the segments from the document objects. This
   * document object assumes the existence of sysid field, which is ususally in
   * reference file. Segment text and sysids are put into the given hashmaps.
   ********************************************************/
  ...
  /********************************************************
   * Name: loadSegs Desc: Loads the segments from the document objects. This
   * document object do NOT assumes the existence of sysid field. Segment text
   * and sysids are put into the given hashmaps.
   ********************************************************/
  ...
  /********************************************************
   * Name: writeXMLHeader Desc: Writes the header information to the given XML
   * file.
   ********************************************************/
  public static void writeXMLHeader(BufferedWriter xml, String hyp_fn,
      String ref_fn, boolean caseon) {
    ...
    try {
      ...
      xml.write("<system title=\"Byblos\" hyp_fname=\"" + hyp_fn
          + "\" ref_fname=\"" + ref_fn + "\" creation_date=\""
  ...
  /********************************************************
   * Name: writeXMLAlignment Desc: Writes the detailed alignment for each word
   * of the segments.
   ********************************************************/
  public static void writeXMLAlignment(BufferedWriter xml, TERalignment result,
      String id, boolean istrans) {
    ...
    if (cur_ids != null) {
      ...
      if (newDoc(cur_ids))
        writeXMLDoc(xml, cur_ids);
      else if (newSeg(cur_ids))
        writeXMLSeg(xml, cur_ids);
      try {
        xml.write("      <hyp id=\"" + cur_rank + "\" refid=\""
            + result.bestRef + "\" wrd_cnt=\"" + result.numWords
            + "\" num_errs=\"" + result.numEdits + "\">");
        writeXMLAlignmentDetails(xml, result.ref, result.aftershift,
            result.alignment, result.allshifts);
        xml.write("      </hyp>");
      } catch (IOException ioe) {
        System.out.println(ioe);
        return;
  ...
  /********************************************************
   * Name: writeXMLFooter Desc: Closes the tags for the given XML file.
   ********************************************************/
  ...
  /********************************************************
   * Name: join Desc: Joins the elements of the given array list with the given
   * delimeter.
   ********************************************************/
  public static String join(String delim, ArrayList al) {
    String ret = "";
    if (al != null) {
  ...
  /********************************************************
   * Name: static Desc: Initializes the hashmaps for the tags and attributes.
   ********************************************************/
  static {
    last_doc = "";
    last_seg = "";
  ...
  /********************************************************
   * Name: newDoc Desc: Test if the given id is from a new document.
   ********************************************************/
  ...
  /********************************************************
   * Name: newSeg Desc: Test if the given id is from a new Segment.
   ********************************************************/
  ...
  /********************************************************
   * Name: parseID Desc: Extracts the docid, segid and rank from the given id.
   ********************************************************/
  private static HashMap parseID(String id, boolean istrans) {
  ...
  /********************************************************
   * Name: writeXMLDoc Desc: Writes the tags for the new document.
   ********************************************************/
  private static void writeXMLDoc(BufferedWriter xml, HashMap cur_ids) {
    try {
      if (!is_first_doc) {
        xml.newLine();
        xml.write("    </seg>");
  ...
  /********************************************************
   * Name: writeXMLSeg Desc: Writes the tags for the new segment.
   ********************************************************/
  private static void writeXMLSeg(BufferedWriter xml, HashMap cur_ids) {
    try {
      if (!is_first_seg) {
        xml.newLine();
        xml.write("    </seg>");
  ...
  /********************************************************
   * Name: writeXMLAlignmentDetails Desc: Writes the alignment information for
   * the given segments. They can be I, S, D and ' '.
   ********************************************************/
  private static void writeXMLAlignmentDetails(BufferedWriter xml,
      Comparable[] ref, Comparable[] hyp, char[] alignment, TERshift[] allshifts) {
      ...
      for (int i = 0; i < alignment.length; ++i) {
        ...
        if (alignment[i] != 'D') {
          if (slen > 0) {
            ...
            // if(val != null) System.out.println("shiftstr: " + ref_idx + "," +
            // hyp_idx + "-" + ind_in + ":" + shift_in_str);
          }
        }
        switch (alignment[i]) {
        case ' ':
          try {
            ...
            if (!ref[ref_idx].equals(hyp[hyp_idx]))
              System.out.println("* Error: unmatch found, " + ref[ref_idx]
                  + " vs. " + hyp[hyp_idx]);
            xml.write("        ");
            xml.write("\"" + ref[ref_idx] + "\",\"" + hyp[hyp_idx] + "\","
                + "C,");
            // System.out.println("hyp[" + hyp_idx + "] ref[" + ref_idx +
            // "]:"+shift_in_str + ","+ ref[ref_idx] + "," + hyp[hyp_idx]);
            if (shift_in_str.equalsIgnoreCase(""))
              ...
              if (val != null) {
                ArrayList al = (ArrayList) val;
                // System.out.println(hyp_idx + "," + ref_idx + ":" + al.get(0)
                // + "-" + shift_dists.get((Integer)al.get(0) - 1));
        ...
        case 'I':
          try {
            // System.out.println("hyp[hyp_idx-1]: " +
            // ((hyp_idx>0)?hyp[hyp_idx-1]:"") + "," + hyp_idx + ", "+
            // hyp.length);
            if (i == 0)
              xml.newLine();
            xml.write("        ");
  ...
  /********************************************************
   * Name: addDomElement Desc: Adds a new element into the document.
   ********************************************************/
  private void addDomElement(Element parent, Element child, HashMap content) {
  ...
  /********************************************************
   * Name: getAttr Desc: Extracts the attributes from the string.
   ********************************************************/
  private void getAttr(String inner, HashMap content) {
    while (inner != "") {
  ...
  /********************************************************
   * Name: getNTag Desc: Parse the line to extract the properties of the tag. It
   * returns int to indicate different type of tags found: 0 - nothing is found.
   * 1 - start tag in the form of <tag> is found. 2 - end tag in the form of
   * </tag> is found. 3 - both start and end tags are found <tag/> is found.
   ********************************************************/
  private int getNTag(String line, TERtag tag) {
  ...
  /********************************************************
   * Name: buildDomLine Desc: Builds the document elements from a given line is
   * possible.
   ********************************************************/
  private void buildDomLine(String line) {
    ...
    if (cur_element == null && (found == 1 || found == 3)
      ...
      case 2:
        ...
        if (tag.name.equalsIgnoreCase(cur_element.getTagName())) {
          ...
          System.out.println("** Warning: found mis-matching tags in line: "
              + line + " - " + tag.name + " vs " + cur_element.getTagName());
          document = null;
          return;
        }
        break;
  ...
  /********************************************************
   * Name: buildDom Desc: Constructs the document from a text file if possible.
   ********************************************************/
  private void buildDom(String fn) {
    is_sgml = false;
    start_tag_i = Pattern.compile("^\\s*\\<([^> ]*)\\s*([^>]*)\\>(.*)$",
        Pattern.CASE_INSENSITIVE);
    end_tag_i = Pattern.compile("^\\s*\\</([^> ]*)\\s*([^>]*)\\>(.*)$",
        Pattern.CASE_INSENSITIVE);
    start_end_tag_i = Pattern.compile("^\\s*\\<([^> ]*)\\s*([^>]*)\\\\>(.*)$",
        Pattern.CASE_INSENSITIVE);
    attr_i = Pattern.compile("^\\s*(\\S+)=(\\S+)\\s*(.*)$",
        Pattern.CASE_INSENSITIVE);
    no_tag_i = Pattern.compile("^\\s*([^\\<]*)(.*)\\s*$",
        Pattern.CASE_INSENSITIVE);
  ...
  /********************************************************
   * Name: loadSegsElement Desc: Load segs from element nodes of the document.
   * This function decides how to deal with different tags.
   ********************************************************/
      ...
      if (attrs != null) {
        for (int i = 0; i < attrs.getLength(); ++i) {
          ...
          case ID:
            ...
            id = "[" + doc_id + "]["
                + String.format("%1$04d", Integer.valueOf(seg_id)) + "]";
            break;
          case SEGID:
            seg_id = attr.getNodeValue().trim();
            break;
