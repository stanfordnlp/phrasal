 ...
 * @param <FV>
 */
public class AgendaMultiTranslationMetricMax<TK, FV> implements MultiTranslationMetricMax<TK, FV> {
	...
	public List<ScoredFeaturizedTranslation<TK, FV>> maximize(
		...
		while (!agenda.isEmpty()) {
			...
			if (mts.depth+1 == nbestLists.size()) {
				if (DEBUG) {
					System.err.printf("Recombinations: %.3f %% (%d/%d)\n", recombinations*100.0/hypothesesGenerated, recombinations, hypothesesGenerated);
				}
				return mts.selected();
			}
			...
			System.out.printf("depth: %d, agenda size: %d\n", mts.depth, agenda.size());
		}
		
		throw new RuntimeException("Search error - agenda exhaused without reaching goal");
	}
