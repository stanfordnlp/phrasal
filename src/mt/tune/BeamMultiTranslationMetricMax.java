 ...
 * @param <FV>
 */
public class BeamMultiTranslationMetricMax<TK, FV> implements MultiTranslationMetricMax<TK, FV> {
	...
	public List<ScoredFeaturizedTranslation<TK, FV>> maximize(
		...
		for (int translationId = 0; translationId < nbestLists.size(); translationId++) {
			if (DEBUG) {
				System.err.printf("Doing translation id: %d Incoming States: %d Best Score: %.5f\n", translationId, stateList.size(), bestScore);
			}
			beam = new TreeBeam<MultiTranslationState<TK, FV>>(beamSize, new MetricBasedRecombinationFilter<TK,FV>(metric));
			for (MultiTranslationState<TK, FV> mts : stateList) {
				for (ScoredFeaturizedTranslation<TK,FV> featurizedTrans : nbestLists.get(translationId)) {
