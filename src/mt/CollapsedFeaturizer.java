	...
	public FeatureValue<FV> featurize(Featurizable<TK,FV> f) {
		...
		for (int i = 0; i < sz; i++) {
			...
			if (singleFeatureValue != null) {
				value += getIndividualWeight(singleFeatureValue.name)*featurizerWts[i]*singleFeatureValue.value;
			}
			...
			if (listFeatureValues != null) {
				for (FeatureValue<FV> featureValue : listFeatureValues) {
					value += getIndividualWeight(featureValue.name)*featurizerWts[i]*featureValue.value;
				}
			}
		}
