 ...
 *
 */
public class RotationalBoundryFeaturizer implements IncrementalFeaturizer<IString, String>{
	...
	public FeatureValue<String> featurize(Featurizable<IString, String> f) { 
		...
		if (foreignSwapPos != -1) {  		
  		...
  		englishSwapCue = EnglishSwapCues.contains(tAInitialWord) ? tAInitialWord : EnglishSwapCues.contains(tBFinalWord) ? tBFinalWord : noCue;
  		foreignSwapCue = ForeignSwapCues.contains(fBInitialWord) ? fBInitialWord : ForeignSwapCues.contains(fAFinalWord) ? fAFinalWord : noCue;
  		...
  		if (DEBUG) {
  			...
  			System.err.printf(
  					...
  					englishSwapCue, tAInitialWord, tBFinalWord, foreignSwapCue, fBInitialWord, fAFinalWord);
  		}
		} else {
			type = "cmp:";
			IString tAInitialWord = f.translatedPhrase.get(0);
			...
			IString tBFinalWord = (f.translationPosition > 0 ? f.partialTranslation.get(f.translationPosition-1) : START_SEQ);
			IString fBFinalWord = (f.foreignPosition > 0 ? f.foreignSentence.get(f.foreignPosition-1) : START_SEQ);
			// we have no swap
			englishSwapCue = EnglishSwapCues.contains(tAInitialWord) ? tAInitialWord : EnglishSwapCues.contains(tBFinalWord) ? tBFinalWord : noCue;  	
			foreignSwapCue = ForeignSwapCues.contains(fAInitialWord) ? fAInitialWord : ForeignSwapCues.contains(fBFinalWord) ? fBFinalWord : noCue;	
		}
		
				
		if (englishSwapCue == noCue && foreignSwapCue == noCue) return null;

		String featureString = FEATURE_PREFIX+type+englishSwapCue.toString()+":"+foreignSwapCue.toString();
		if (DEBUG) {
