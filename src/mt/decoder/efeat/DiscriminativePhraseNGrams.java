 ...
 *
 */
public class DiscriminativePhraseNGrams implements IncrementalFeaturizer<IString, String> {
	...
	public List<FeatureValue<String>> listFeaturize(
		...
		for (int history = 0; history < size && f != null; history++) {
			if (history != 0) sbuf.append(";");
			if (type == Type.s || type == Type.st) sbuf.append(f.foreignPhrase.toString("_"));
			if (type == Type.st) sbuf.append("=>");
			if (type == Type.t || type == Type.st) sbuf.append(f.translatedPhrase.toString("_"));
			f = f.prior;
