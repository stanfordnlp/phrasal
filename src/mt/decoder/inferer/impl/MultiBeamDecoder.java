...
public class MultiBeamDecoder<TK, FV> extends AbstractBeamInferer<TK, FV> {
  ...
  @SuppressWarnings("unchecked")
    protected Beam<Hypothesis<TK,FV>> decode(Sequence<TK> foreign, int translationId, RecombinationHistory<Hypothesis<TK,FV>> recombinationHistory,
    ...
    for (int i = 0; i < beams.length; i++) {
      ...
      */
      ...
      for (Hypothesis<TK, FV> hyp : beams[i]) { hypPos++;
        //System.err.printf("\nExpanding hyp: %s\n", hyp);
        ...
        /*
          if (localOptionsApplied == 0 && !hyp.isDone()) {
          System.err.printf("Beam: %d - search warning\n", i);
          System.err.printf("Can't find any options to apply to hyp: %s\n", (hyp.featurizable != null ? hyp.featurizable.partialTranslation : ""));
          System.err.printf("Foreign coverage: %s\n", hyp.foreignCoverage);
          System.err.printf("Stored options\n");
          for (ConcreteTranslationOption<TK> option : options) {
          if (option.foreignCoverage.intersects(hyp.foreignCoverage)) continue;
          int distortion = (hyp.translationOpt == null ? option.foreignPos
          : hyp.translationOpt
          .linearDistortion(option));
          if (distortion > maxDistortion) continue;
          System.err.printf("%s->%s coverage: %s distortion: %d\n", 
          option.abstractOption.foreign, option.abstractOption.translation, option.foreignCoverage, 
          distortion);
          }
          } */
      }
      ...
      if (bestHyp.featurizable != null) {
        ...
        for (Hypothesis<TK,FV> hyp : trace) {
          ...
          if (hyp.translationOpt != null) {
            System.err.printf("\tPhrase: %s(%d) => %s(%d) | constilation = %s ",
                              ...
                              hyp.translationOpt.abstractOption.constilation
              );
          }
          System.err.printf("\tCoverage: %s\n", hyp.foreignCoverage);
