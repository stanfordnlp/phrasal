...
public class TreeBeam<S extends State<S>> implements Beam<S> {
	...
	public static final boolean DEBUG = Boolean.parseBoolean(System.getProperty(DEBUG_OPT, "false"));
	...
	public S put(S hypothesis) { hypothesisInsertions++;
		...
		if (status == RecombinationHash.Status.COMBINABLE) { recombinations++;
			return hypothesis;
		}
		
		if (status == RecombinationHash.Status.UPDATED) { recombinations++;
