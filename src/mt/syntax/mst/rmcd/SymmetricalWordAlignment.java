...
public class SymmetricalWordAlignment extends AbstractWordAlignment {
  ...
  public void init(String[] fStr, String[] eStr, String aStr) throws IOException {
    f = new SimpleSequence<IString>(true, IStrings.toIStringArray(preproc(fStr)));
    e = new SimpleSequence<IString>(true, IStrings.toIStringArray(preproc(eStr)));
    initAlignment();
    if(aStr == null) {
    ...
    for(int i=0; i<f2e.length; ++i)
      this.f2e[i] = ArrayUtils.toPrimitive(f2e[i].toArray(new Integer[f2e[i].size()]));
    for(int i=0; i<e2f.length; ++i)
      this.e2f[i] = ArrayUtils.toPrimitive(e2f[i].toArray(new Integer[e2f[i].size()]));
  }
