...
public class HieroExtractor {
	...
	public static void main(String[] args) {		
		...
		BufferedReader t_reader_zh = FileUtility.getReadFileStream("C:\\data_disk\\java_work_space\\SyntaxMT\\mt.syntax.train.hiero\\aligned.zh","UTF8");
		//BufferedReader t_reader_tree = FileUtility.getReadFileStream(args[1].trim(),"UTF8");
		...
		while((line_align=FileUtility.read_line_lzf(t_reader_align))!=null){
			...
			ArrayList p_rules_and_phrases = process_a_sentence( line_align, line_fr, line_en);
		...
		System.out.println("total lines: " + n_line + "; total ini phrases  " + g_num_init_phrases + "; total rules and phrases  " + g_num_rules_and_phrases );
	...
	private static ArrayList process_a_sentence(String line_align, String line_fr, String line_en){	
		//#### create alignment datastructure
		Alignment align = new Alignment(line_fr, line_en, line_align);
		
		//#### compute weights
		if(fweights_table!=null)fweights = compute_lexical_weights(align, fweights_table, false, false);
		if(eweights_table!=null) eweights = compute_lexical_weights(align, eweights_table, true, false);//transpose
		
		
		//#### exphrase phrases
		int[] actual_max_init_phrase_size = new int[1];
		ArrayList l_init_phrases = extract_phrases(align, max_init_phrase_size, actual_max_init_phrase_size);//extract regular flat phrases;
		...
		if(use_tight_phrase==false)	loosen_phrases();
		
		if(remove_overlap_phrases==true) remove_overlap_phrases();
		...
		//#### extract rules: the l_init_phrases is a list of phrases from a specific training sentence-pair
		ArrayList l_rules_and_phrases = extract_rules(align, l_init_phrases, max_final_phrase_size, min_sub_phrase_size, max_num_non_terminals, actual_max_init_phrase_size[0]);			
	...
	private static float[] compute_lexical_weights(Alignment align, HashMap weights_maxtrix, Boolean transpose, Boolean swap){
	...
	private static ArrayList extract_phrases(Alignment align, int max_init_phrase_size, int[] actual_max_init_phrase_size){
		...
		for(i1=0; i1< align.french_wrds.length; i1++){
			...
			j2 = -1;
			for(i2=i1; i2<Alignment.min(i1+max_init_phrase_size, align.french_wrds.length); i2++){
				...
				if(j2-j1+1>max_init_phrase_size) break; //go to next i1, since adding more wrds in french will increase the eng span
				...
				actual_max_init_phrase_size[0] = Alignment.max(actual_max_init_phrase_size[0], i2-i1+1);
			}
		}
	...
	private static ArrayList extract_rules(Alignment align, ArrayList l_init_phrases, int max_final_phrase_size, int min_sub_phrase_size, int max_num_non_terminals, int actual_max_init_phrase_size){
		if(l_init_phrases.size()==0){
	...
	private static Rule make_and_score_rule(Alignment align, int[] phrase, ArrayList item){
		//ignore ??
		if(item.size()==1 && !(item.get(0) instanceof Integer))	return null; //bug: this may skip the flat phrase
	...
	private static Rule score_rule(Alignment align, Rule r_in, int[] fpos, int[] epos){
