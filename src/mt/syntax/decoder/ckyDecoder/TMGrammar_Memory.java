...
public class TMGrammar_Memory extends TMGrammar {
	...
	*/
	...
	public void read_tm_grammar_glue_rules(){
		...
		add_rule("S ||| ["+Decoder.default_non_terminal+",1] ||| ["+Decoder.default_non_terminal+",1] ||| 0", Decoder.begin_mono_owner);//this does not have any cost	
		...
		add_rule("S ||| [S,1] ["+Decoder.default_non_terminal+",2] ||| [S,1] ["+Decoder.default_non_terminal+",2] ||| "+alpha, Decoder.mono_owner);		
	...
	private Rule add_rule(String line, String owner){
		return add_rule(line, Symbol.add_terminal_symbol(owner));
	}
	...
	private Rule add_rule(String line, int owner){
		...
		int lhs = Symbol.add_non_terminal_symbol(TMGrammar_Memory.replace_french_non_terminal(fds[0]));
			
		//french side: note that the french side is not stored at the rule, instead at the rulebin
		int arity=0;
		String[] french_tem= fds[1].split("\\s+");
		int[] french = new int[french_tem.length];			
		...
		if(pos.rule_bin==null){
			...
			pos.rule_bin.french = french;
			pos.rule_bin.arity=arity;
			num_rule_bin++;
		}
...
//	TODO: we assume all the Chinese training text is lowercased, and all the non-terminal symbols are in [A-Z]+
	private static boolean is_non_terminal(String symbol){
	...
	public class RuleBin_Memory extends TMGrammar.RuleBin {
		...
		private double cutoff=Chart.IMPOSSIBLE;
		...
		private int run_pruning(){
			...
			if(heap_rules.size()==Decoder.max_n_rules)
				cutoff= (cutoff<heap_rules.peek().est_cost) ? cutoff : heap_rules.peek().est_cost+Chart.EPSILON;//TODO
			return n_pruned++;
		}
	...
	public static class Rule_Memory extends Rule {
		...
		public Rule_Memory(int lhs_in, int[] eng_in, int owner_in, float[] feat_scores_in, int arity_in){
			super(lhs_in, eng_in,  owner_in,  feat_scores_in, arity_in);			
