...
public class HyperGraph {
	//implement HyperGraph data structure, and k-best extraction algorithm over a hyper-graph
	//a hyper-graph is indentified by its goal item (or target vertex)
	...
	*state: lhs and edge ngram*/
	public static class Item implements Comparable 
	{
		...
		public int lhs; //this is the symbol like: NP, VP,  terminal word, and so on
		public HashMap  tbl_states; //remember the state required by each model, for example, edge-ngrams for LM model
		private String signature=null;
		public boolean is_dead=false;
		
		public double est_total_cost=0.0; //this is maintained for pruning purpose, it includes the bonus cost
		...
		private PriorityQueue<DerivationState> heap_cands=null;// in the paper, it is called cand[v]		
		private HashMap  derivation_tbl = null;//rememeber which DerivationState has been explored
		private HashMap  nbest_str_tbl =null;//!!!!!maintaining this tbl at each item (instead of at goal item only)is critical to get fast unique nbest extraction
		...
		public void add_deduction_in_item(Deduction dt){
			...
			l_deductions.add(dt);
			if(best_deduction==null || best_deduction.best_cost>dt.best_cost)//TODO: hiero does not have this?
				best_deduction=dt;			
		}
...
//			TODO: if tem.size is too large, this may cause unnecessary computation, we comment the segment to accormodate the unique nbest extraction			
		...
		public String get_signature(){
			...
			res.append(lhs);
			for(Integer st_name : Chart.l_model_state_names){
				int[] st = (int[])tbl_states.get(st_name);
				if(st!=null){
					res.append("|");					
...
//	#################### Deduction class
	...
	{	//should remember two costs: best_total_cost, and transition cost
		...
		public double best_cost=Chart.IMPOSSIBLE;//the 1-best cost of all possible derivation: best costs of ant items + non_stateless_transition_cost + r.statelesscost
		private Rule rule;
...
//########################################## kbest extraction algorithm ##########################	
	...
	public static void lazy_k_best_extract(Item goal_item, ArrayList<Model> l_models, int global_n, boolean extract_unique_nbest, int sent_id, BufferedWriter out, boolean extract_nbest_tree, boolean add_combined_score){		
	...
	private static class DerivationState implements Comparable 
	{
		...
		private void compute_cost(Rule rl, ArrayList<Item> ants_items, double[] model_cost, ArrayList l_models){
			...
			for(int k=0; k< l_models.size(); k++){
				...
				model_cost[k] += ((Double)tem_tbl.get(Chart.TRANSITION_COST_SYM_ID)).doubleValue();
			}
		}
