...
public class Decoder {
	...
	public static void main(String[] args) {	
		...
		//####operations order: read config, init lm, init sym tbl, read lm, read tm
		//##### read config file
		...
		init_lm_grammar();//inside, it will init sym tbl (add global symbols)
		...
		//#### init chart static symbols, after the symbol table has been initialized
		Chart.add_static_symbols();
		...
		if(num_parallel_decoders==1){
			decode_a_file(test_file,nbest_file,0);
		...
		while((cn_sent=FileUtility.read_line_lzf(t_reader_test))!=null){
			...
			if(sent_id!=0 && 
			   ...
			   sent_id%num_per_thread==0){
				...
				ParallelDecoder pdecodder = new ParallelDecoder(cur_test_file, cur_nbest_file, start_sent_id);
				l_parallel_decode_threads[n_decoder_to_commit-1]=pdecodder;
				l_parallel_test_files[n_decoder_to_commit-1]=cur_test_file;
				l_parallel_nbest_files[n_decoder_to_commit-1]=cur_nbest_file;
				System.out.println("##############start thread "+n_decoder_to_commit);
				pdecodder.start();
	...
	public static class ParallelDecoder extends Thread {
		 ...
		 int start_sent_id; //start sent id
		  public ParallelDecoder( String test_file_in, String nbest_file_in, int start_sent_id_in){
		    ...
		    public void run() {		    	
		    	decode_a_file(test_file, nbest_file, start_sent_id);
		    }
	}
	
	
	//TODO: log file
	public static void decode_a_file(String test_file, String nbest_file, int start_sent_id){
		BufferedReader t_reader_test = FileUtility.getReadFileStream(test_file,"UTF-8");
		BufferedWriter t_writer_nbest = FileUtility.getWriteFileStream(nbest_file,"UTF-8");
		...
		while((cn_sent=FileUtility.read_line_lzf(t_reader_test))!=null){
			...
			translate(p_tm_grammars,p_l_models,cn_sent.split("\\s+"),l_default_nonterminals,t_writer_nbest, sent_id, topN);
			sent_id++;
			//if(sent_id>0)break;
		}
		FileUtility.close_read_file(t_reader_test);
...
//		###### Initialized the TM grammar
	...
	static void translate(TMGrammar[] grs,ArrayList<Model> l_models, String[] sentence, ArrayList<Integer> l_default_nonterminals, BufferedWriter t_writer_nbest, int sent_id, int topN){
		long start = System.currentTimeMillis();
		...
		Item goal_item =  chart.expand();
		
		System.out.println("after expand, time: " + (System.currentTimeMillis()-start)/1000);
		HyperGraph.lazy_k_best_extract(goal_item,l_models, topN, use_unique_nbest, sent_id,t_writer_nbest, use_tree_nbest,add_combined_cost);
		System.out.println("after kbest, time: " + (System.currentTimeMillis()-start)/1000);
		
		//debug
		/*
		String ref_sent = "export of high-tech products in guangdong in first two months this year reached 3.76 billion us dollars";
		String[] ref_wrds = ref_sent.split("\\s+");
		int[] ref_sent_numeric = new int[ref_wrds.length];
		for(int i=0; i< ref_wrds.length; i++){
			ref_sent_numeric[i]=Symbol.add_terminal_symbol(ref_wrds[i]);//TODO			
		}
		OracleExtractor.oracle_decode(g_lm_order, sentence.length, ref_sent_numeric, chart, goal_item);
		*/
		//debug end
