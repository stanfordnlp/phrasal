...
//	state name definition
	...
	static public void init_sym_tbl_from_file(String fname, boolean u_own_tbl){
		...
		if(tbl_id_2_str.size()>=lm_end_sym_id-lm_start_sym_id){	System.out.println("Error: read symbol tbl, tlb is too big");System.exit(0);
		}
	...
	static public int add_terminal_symbol(String str){
		if(use_my_own_tbl==false)
			...
			if(res_id!=null){//already have this symbol
				if(is_nonterminal(res_id)==true){System.out.println("Error, terminal symbol mix with non-terminal, Sym: " + str + "; id: " + res_id);System.exit(0);}
				return res_id;
			}else{
...
//	####### following funcitons used for TM only
	static public int add_non_terminal_symbol(String str){
		Integer res_id = (Integer)str_2_num_tbl.get(str);
		if(res_id!=null){//already have this symbol
			if(is_nonterminal(res_id)==false){System.out.println("Error, NONTSym: " + str + "; id: " + res_id);System.exit(0);}
			return res_id;
		}else{
