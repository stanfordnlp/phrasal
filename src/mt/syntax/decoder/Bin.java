...
import mt.syntax.decoder.HyperGraph.Deduction;
...
{	
	...
	public void complete_cell_cube_prune(int i, int j, ArrayList<SuperItem> l_super_items, RuleBin rb){//combinations: rules, antecent items
		...
		while(heap_cands.size()>0){
			...
			if(((Double)cur_state.tbl_states.get(Symbol.TOTAL_COST_SYM_ID)).doubleValue()>cut_off_cost+Decoder.fuzz1){
			...
			for(int k=0; k<cur_state.ranks.length; k++){
				...
				if(((Double)tbl_states2.get(Symbol.TOTAL_COST_SYM_ID)).doubleValue()<cut_off_cost+Decoder.fuzz2){
	...
	private static class CubePruneState implements Comparable {
		...
		public int compareTo(Object another) throws ClassCastException {
		    ...
		    if((Double)this.tbl_states.get(Symbol.TOTAL_COST_SYM_ID) < (Double)((CubePruneState)another).tbl_states.get(Symbol.TOTAL_COST_SYM_ID))
		    	return -1;
		    else if((Double)this.tbl_states.get(Symbol.TOTAL_COST_SYM_ID) == (Double)((CubePruneState)another).tbl_states.get(Symbol.TOTAL_COST_SYM_ID))
		    	return 0;
		    else
		    	return 1; 
		}
	...
	public Item add_deduction_in_bin(HashMap  tbl_states, Rule rl, int i, int j,  ArrayList<Item> ants){
		...
		double total_cost=((Double)tbl_states.get(Symbol.TOTAL_COST_SYM_ID)).doubleValue();//total_cost=ADDITIVE_COST+TRANSITION_COST+bonus
		double transition_cost=((Double)tbl_states.get(Symbol.TRANSITION_COST_SYM_ID)).doubleValue();//non-stateless transition cost 
		double additive_cost=((Double)tbl_states.get(Symbol.ADDITIVE_COST_SYM_ID)).doubleValue();
	...
	private boolean add_deduction( Item new_item){
		...
		cut_off_cost = find_min(best_item_cost+Decoder.relative_threshold, Symbol.IMPOSSIBLE_COST);			
		run_pruning();
		return res;
	}
...
//	this function is called only there is no such item in the tbl
	...
	private void run_pruning(){
		...
		if(heap_items.size()-dead_items==Decoder.max_n_items){//TODO:??	
			cut_off_cost = find_min(cut_off_cost, heap_items.peek().est_total_cost+Symbol.EPSILON);
		}			
	}
