 ...
 * <li> HypothesisScores: a gzipped file that has the hypothesis scores. The format of the file is:
 * sentId,hypId score. For example:<br>
 * <pre>
 * 100,0 0.25
 * 100,1 0.13
 * ...
 * </pre>
 * <li> FeatureSets: one or more gzipped files that have features for each hypothesis.
 * <li> NBestList: this is optional. If provided, the Reranker can access the original sentences,
 * thus can output predicted sentences for later use.
 * </ul>
 * Note: HypothesisScores and FeatureSets can be generated from {@link LegacyFeatureExtractor}.
 * <h3>Useful properties</h3>
 * <ul>
 * <li> -DfeatureIndex=featureIndex.txt
 * <li> -DpredictedIndices=predictedIndices.txt
 * <li> -Dpredicted=predictedSentences.txt : you have to specify "NBestList" to output the predicted sentences.
 * </ul>
 *
...
public class NBest {
  ...
  static public void outputPredictedIndices(AbstractOneOfManyClassifier classifier, DataSet dataSet, String results) throws Exception {
    PrintWriter pr = new PrintWriter(new BufferedWriter(new FileWriter(results, false)));
  ...
  static public void outputPredictedSents(AbstractOneOfManyClassifier classifier, DataSet dataSet, String results) throws Exception {
    if (dataSet.nbests==null) {
      System.err.println("Warning: NBestList wasn't provided. Therefore predicted sentences cannot be output.");
      return;
    }
  ...
  static public void outputPredictedSents(AbstractOneOfManyClassifier classifier, MyList<CompactHypothesisList> lchl, int range[], PrintWriter pr, DataSet dataSet) throws Exception {
    // makes the mapping
    int mapping[] = new int[lchl.size()];
    int ctr = 0;
    for(int i = range[0]; i <= range[1]; i++) {
  ...
  static double computeCorpusBleu(int[] indices, int offset, DataSet dataSet) {
    ...
    for (int i = 0; i < indices.length; i++) {
      System.err.println("DEBUG: calling dataSet.getFromNBest("+i+"+"+offset+", "+indices[i]);
      String[] sentence = dataSet.getFromNBest(i+offset, indices[i]).split("\\s+");
  ...
  static double computeCorpusBleu(int[] indices, int[] range_exclude, DataSet dataSet) {
    // make the range first..
    int mapping[] = new int[indices.length];
    int ctr = 0;
    System.err.printf("DEBUG: %d %d %d %d\n", range_exclude[0], range_exclude[1], range_exclude[2], range_exclude[3]);
    ...
    Scorer scorer = new Bleu();
    for (int i = 0; i < indices.length; i++) {
      String[] sentence = dataSet.getFromNBest(mapping[i], indices[i]).split("\\s+");
  ...
  static double computeCorpusTER(int[] indices, int[] range_exclude, DataSet dataSet) {
    // make the range first..
    int mapping[] = new int[indices.length];
    int ctr = 0;
    for(int i = range_exclude[0]; i <= range_exclude[1]; i++) {
  ...
  static public void displayEvaluationStats(AbstractOneOfManyClassifier classifier,
                                            MyList<CompactHypothesisList> examples,
                                            int offset,
                                            DataSet dataSet) {
    ...
    double predictedBleu_tieLast = computeCorpusBleu(bestChoices_tieLast, offset, dataSet);
    double predictedTER_tieLast = computeCorpusTER(bestChoices_tieLast, offset, dataSet);
    double predictedCL_tieLast = classifier.getLogLikelihood(examples, bestChoices_tieLast);

    double predictedBleu_tieFirst = computeCorpusBleu(bestChoices_tieFirst, offset, dataSet);
    double predictedTER_tieFirst = computeCorpusTER(bestChoices_tieFirst, offset, dataSet);
    double predictedCL_tieFirst = classifier.getLogLikelihood(examples, bestChoices_tieFirst);
    ...
    System.out.printf("\tPredicted Sentences corpus BLEU score (if tied, choose first): %.4f\n", predictedBleu_tieFirst);
    System.out.printf("\tPredicted Sentences corpus TER score (if tied, choose first): %.4f\n", predictedTER_tieFirst);
    System.out.printf("\tPredicted Sentences CL (if tied, choose first): %.4f\n", predictedCL_tieFirst);

    System.out.printf("\tPredicted Sentences corpus BLEU score (if tied, choose last) : %.4f\n", predictedBleu_tieLast);
    System.out.printf("\tPredicted Sentences corpus TER score (if tied, choose last) : %.4f\n", predictedTER_tieLast);
    System.out.printf("\tPredicted Sentences CL (if tied, choose last) : %.4f\n", predictedCL_tieLast);

    System.out.printf("\tOracle corpus BLEU score (choosing the best sentences by the scores provided): %.4f\n", oracleBleu);
    System.out.printf("\tOracle corpus TER score (choosing the best sentences by the scores provided): %.4f\n", oracleTER);
    System.out.printf("\tOracle CL (choosing the best sentences by the scores provided): %.4f\n", oracleCL);

    System.out.printf("\tWorst corpus BLEU score (choosing the worst sentences by the scores provided): %.4f\n", worstBleu);
    System.out.printf("\tWorst corpus TER score (choosing the worst sentences by the scores provided): %.4f\n", worstTER);
    System.out.printf("\tWorst CL (choosing the worst sentences by the scores provided): %.4f\n", worstCL);
  ...
  static public void displayEvaluationStats(AbstractOneOfManyClassifier classifier,
    ...
    if (classifier instanceof MCLPairLearner) {
      MCLPairLearner mpl = (MCLPairLearner)classifier;
      int[] stats = mpl.getAccuracy(examples, range, dataSet);
      System.out.println("Pairwise Stats:");
      System.out.println("Correct="+stats[0]);
      System.out.println("All="+stats[1]);
      System.out.println("Acc="+(double)stats[0]/stats[1]);
    ...
    */
    ...
    double predictedBleu_tieLast = computeCorpusBleu(bestChoices_tieLast, range, dataSet);
    double predictedTER_tieLast = computeCorpusTER(bestChoices_tieLast, range, dataSet);
    double predictedCL_tieLast = classifier.getLogLikelihood(examples, bestChoices_tieLast);

    double predictedBleu_tieFirst = computeCorpusBleu(bestChoices_tieFirst, range, dataSet);
    double predictedTER_tieFirst = computeCorpusTER(bestChoices_tieFirst, range, dataSet);
    double predictedCL_tieFirst = classifier.getLogLikelihood(examples, bestChoices_tieFirst);
  ...
  /**
   * @param args first argument: datadescriptor
   * @throws Exception
   */
  public static void main(String[] args) throws Exception {
    ...
    int[] range = dataSet.getTrainRange();
    AbstractOneOfManyClassifier classifier = AbstractOneOfManyClassifier.factory();
    System.err.println("Training classifier on train range: "+range[0]+"-"+range[1]+
                       ", excluding the dev range "+range[2]+"-"+range[3]+
                       " if overlapped.");
    ...
    if (!classifier.isLogLinear()) {
      System.out.printf("Wrapping %s using LogLinearProbalisticWrapper", classifier.getClass().getName());
      ...
      System.out.printf("Log Likelihood - Original: %f Trained: %f  %% Improvement: %.2f %%\n", origLL, newLL, 100 * (origLL - newLL) / origLL);
    }

    // If appropriate, write out feature/feature-weight pairs 
    // sorted by feature-weight magnitude
    String featureIndexFn = System.getProperty(FEATURE_INDEX_FILENAME_PROP, DEFAULT_FEATURE_INDEX_FILENAME);
    ...
    if (dataSet.nbests==null || dataSet.refs==null) {
      System.out.println("Warning: NBestList and/or RefList are not present. Therefore corpus BLEU cannot be evaluted.");
    } else {
      //displayEvaluationStats(classifier, dataSet.getTrainingSet(), dataSet.getOffset()+trainRange[0], dataSet);
      displayEvaluationStats(classifier, dataSet.getTrainingSet(), dataSet.getTrainRange(), dataSet);
    }
    ...
    if (dataSet.nbests==null || dataSet.refs==null) {
      ...
      displayEvaluationStats(classifier, dataSet.getDevSet(), dataSet.getDevRange(), dataSet);
    }

    // output results for evaluation...
    String predictedIndicesFn = System.getProperty(PREDICTED_INDICES_FILENAME_PROP, DEFAULT_PREDICTED_INDICES_FILENAME);
    if (predictedIndicesFn != null) {
      outputPredictedIndices(classifier, dataSet, predictedIndicesFn);
      //outputPredictedIndices(classifier, dataSet.getDevSet(), predictedIndicesFn);
    }

    // output results for evaluation...
    String predictedFn = System.getProperty(PREDICTED_FILENAME_PROP, DEFAULT_PREDICTED_FILENAME);
    if (predictedFn != null) {
