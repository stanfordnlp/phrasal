...
/**
 * Given two {@link Tree} or {@link GrammaticalStructure} and alignments, this class includes some utility methods
 * for extracting some information from them.
 *
...
public class DependencyUtils {
  ...
  public static ClassicCounter<String> extractEnZhPathFeatures(Tree enT, GrammaticalStructure chGs, Alignment align, boolean EnZh) {
    return extractEnZhPathFeatures(enT, chGs, align, EnZh, false);
  }

  /**
   *
   * @param enT the {@link Tree} of the English sentence
   * @param chGs the {@link GrammaticalStructure} for the Chinese sentence
   * @param align the {@link Alignment} between the two
   * @param EnZh If set to true, the method will extract features mapping from English to Chinese. If set to false, it will be the other way around.
   * @param retricted if true, features will only be extracted when the alignment is one-to-one. (We restrict it this way to get higher precision)
   * @return a {@link ClassicCounter} that has the information of what features are extracted
   */
  public static ClassicCounter<String> extractEnZhPathFeatures(Tree enT, GrammaticalStructure chGs, Alignment align, boolean EnZh, boolean retricted) {
  ...
  private static ClassicCounter<String> extractEnZhPathFeatures(GrammaticalStructure enGs, GrammaticalStructure chGs, Alignment align, boolean EnZh, boolean retricted) {
    ...
    for (SemanticGraphEdge edge : edges) {
      ...
      // if govIdx and depIdx is bigger than 100, this happens because GIZA++ has
      // 100-word limit.
      // TODO: use William's aligner
      if (govIdx > 100 || depIdx > 100) {
        System.err.println("warning: alignments for sentences longer than 100 words were cropped by GIZA++");
      }
      ...
      for (int chGovI : chGovIdx) {
        ...
        for (int chDepI : chDepIdx) {
          ...
          List<SemanticGraphEdge> paths = chGraph.getShortestPathEdges(list.get(chGovI), list.get(chDepI));
          int startI = chGovI;
          StringBuilder sb = new StringBuilder();
          if (EnZh) sb.append("EnZh"); else sb.append("ZhEn");
  ...
  @Deprecated
  private static ClassicCounter<String> extractPathFeatures(Tree enT, GrammaticalStructure chGs, Alignment align, int lengLimit) {
    return extractPathFeatures(enT, chGs, align, lengLimit, false);
  }

  @Deprecated
  private static ClassicCounter<String> extractPathFeatures(Tree enT, GrammaticalStructure chGs, Alignment align, int lengLimit, boolean retrictedAlign) {
    Filter<String> puncWordFilter;
    puncWordFilter = Filters.acceptFilter();
    TreebankLanguagePack tlp = new PennTreebankLanguagePack();
    GrammaticalStructureFactory gsf = tlp.grammaticalStructureFactory(puncWordFilter);
  ...
  public static ClassicCounter<String> extractPathFeatures(GrammaticalStructure enGs, GrammaticalStructure chGs, Alignment align, int lengLimit) {
  ...
  private static ClassicCounter<String> extractPathFeatures(GrammaticalStructure enGs, GrammaticalStructure chGs, Alignment align, int lengLimit, boolean restrictedAlign) {
    ...
    for(int i = 0; i < chlist.size(); i++) {
      for (int j = i; j < chlist.size(); j++) { // Note: we actually consider j==i as well --
                                                // in the case that English words are reduced (aligned)
                                                // to one Chinese word, we can see what kind of
                                                // dependencies are reduced most
        List<SemanticGraphEdge> chPaths = chGraph.getShortestPathEdges(chlist.get(i), chlist.get(j));
        // skip if chPaths doesn't exist or is too long
        if (chPaths == null) continue;
        if (chPaths.size() > lengLimit) continue;
        // if path do exist:
        ...
        for (SemanticGraphEdge p : chPaths) {
          ...
          if(govIdx==start) {
            ...
            throw new RuntimeException("either governoor or dependent should be the same as 'start'!!");
          }
        }
        ...
        for (int enIdxI = 0; enIdxI < enIs.size(); enIdxI++) {
          for (int enIdxJ = 0; enIdxJ < enJs.size(); enIdxJ++) {
            ...
            if (i==j) { // if chinese indices i==j, we're looking at the same list
              if (enI >= enJ) continue;
            }
            List<SemanticGraphEdge> enPaths = enGraph.getShortestPathEdges(enlist.get(enI), enlist.get(enJ));
            if (enPaths == null) continue;
            if (enPaths.size() > lengLimit) continue;
            ...
            for (SemanticGraphEdge p : enPaths) {
              ...
              if(govIdx==startE) {
                ...
                throw new RuntimeException("either governoor or dependent should be the same as 'startE'!!");
              }
            }
