...
public class DumpPossibleWordCoOccurrences {
	static public void main(String[] args) throws IOException {
		...
		PhraseTable<IString> ppt = new PharaohPhraseTable<String>(null, null, phraseTableFilename);
		
		LineNumberReader reader = new LineNumberReader(new FileReader(sourceTextFilename));
		
		Map<IString, List<Integer>> wordSents = new HashMap<IString, List<Integer>>();
		
		for (String line = reader.readLine(); line != null; line = reader.readLine()) {
			if (reader.getLineNumber() % 10 == 0) err.print(".");
			Sequence<IString> tokens = new SimpleSequence<IString>(IStrings.toIStringArray(line.split("\\s+")));
			...
			for (int i = 0; i < tokensSz; i++) {
				...
				for (int j = i+1; j <= jMax; j++) {
					...
					List<TranslationOption<IString>> opts = ppt.getTranslationOptions(phrase);
			...
			Integer sentId = reader.getLineNumber()-1; // use a shared 'Integer' object for all words
		...
		for (String word : sortedWords) {
			out.printf("%s %s\n", word, wordSents.get(new IString(word)).toString().replaceAll("[^0-9 ]", ""));
		}
	}
}
