...
public class FindMetricMax {
	...
	static public void main(String[] args) throws IOException {
		...
		if (metricStr.startsWith(BLEU_METRIC_OPT+":")) {
			...
			metric = new BLEUMetric<IString,String>(Metrics.readReferences(fileNames));
		} else {
			throw new RuntimeException(String.format("Unrecognized metric: '%s'", metric));
		}
		
		MultiTranslationMetricMax<IString,String> searchAlgorithm;
		
		if (AGENDA_OPT.equals(searchAlgorithmStr)) {
			searchAlgorithm = new AgendaMultiTranslationMetricMax<IString, String>(metric);
		} else if (BEAM_OPT.equals(searchAlgorithmStr) || searchAlgorithmStr.matches(BEAM_OPT+":.*")) {
			String[] fields = searchAlgorithmStr.split(":");
			if (fields.length == 1) {
				searchAlgorithm = new BeamMultiTranslationMetricMax<IString, String>(metric);
			} else if (fields.length == 2) {
				int beamSize = Integer.parseInt(fields[1]);
				searchAlgorithm = new BeamMultiTranslationMetricMax<IString, String>(metric, beamSize);
			} else {
				throw new RuntimeException(String.format("%s search only accepts one parameter.", BEAM_OPT));
			}
		} else if (HILLCLIMBING_OPT.equals(searchAlgorithmStr)) {
			searchAlgorithm = new HillClimbingMultiTranslationMetricMax<IString, String>(metric);
		} else if (GREEDY_SEARCH_OPT.equals(searchAlgorithmStr)) {
			searchAlgorithm = new GreedyMultiTranslationMetricMax<IString, String>(metric);
		} else if (TOP_LIST_OPT.equals(searchAlgorithmStr)) {
			searchAlgorithm = new ListTopMultiTranslationMetricMax<IString, String>();
		} else {
			throw new RuntimeException(String.format("Unrecognized search algorithm: '%s'", searchAlgorithmStr));
		}
		
		NBestListContainer<IString, String> nbestLists = new MosesNBestList(nbestListFilename);
		List<ScoredFeaturizedTranslation<IString,String>> maxFeaturizedTranslations = searchAlgorithm.maximize(nbestLists);
