...
/**
 * Compare two system outputs (baseline and contrastive), and display sentences with biggest BLEU score difference.
 *
...
public class BLEUSorter {
  ...
  static public void main(String args[]) throws IOException {
		...
		List<List<Sequence<IString>>> referencesList = Metrics.readReferencesFromRoot(args[0]);

    BLEUMetric<IString,String> bleu1 = new BLEUMetric<IString,String>(referencesList, true);
    BLEUMetric<IString,String> bleu2 = new BLEUMetric<IString,String>(referencesList, true);
    BLEUMetric<IString,String>.BLEUIncrementalMetric incMetric1 = bleu1.getIncrementalMetric();
    BLEUMetric<IString,String>.BLEUIncrementalMetric incMetric2 = bleu2.getIncrementalMetric();
    ...
    List<Integer> sentIds = sortSentencesByScore(hyps1, hyps2, incMetric1, incMetric2);

    for(int sentId : sentIds) {
      double score1 = 100*incMetric1.computeLocalSmoothScore(hyps1.get(sentId), sentId);
      double score2 = 100*incMetric2.computeLocalSmoothScore(hyps2.get(sentId), sentId);
      int sz1 = hyps1.get(sentId).size();
      int sz2 = hyps2.get(sentId).size();
      int avgSz = (sz1+sz2)/2;
      if(avgSz >= minLength && avgSz <= maxLength)
        System.out.printf("%d\t%.3f\t%.3f\t(%d)\t%.3f\t(%d)\t%s\n", sentId, score2-score1,
           score1, sz1, score2, sz2, info.get(sentId));
    }
	}
  ...
  static List<Integer> sortSentencesByScore(List<Sequence<IString>> hyps1, List<Sequence<IString>> hyps2,
    ...
    List<Pair<Double,Integer>> scores = new ArrayList<Pair<Double,Integer>>();
     ...
     for(int sentId=0; sentId < hyps1.size(); ++sentId) {
       double score1 = incMetric1.computeLocalSmoothScore(hyps1.get(sentId), sentId);
       double score2 = incMetric2.computeLocalSmoothScore(hyps2.get(sentId), sentId);
       boolean add;
       if(score1 == 0.0) {
          add = (score2 > 0.0);
       } else {
  ...
  static List<Sequence<IString>> readHypotheses(String fileName, BLEUMetric<IString,String>.BLEUIncrementalMetric incMetric) throws IOException {
