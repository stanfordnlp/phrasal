...
public class DynamicPhraseTable<FV> extends AbstractPhraseGenerator<IString,FV> {
	...
	public List<TranslationOption<IString>> getTranslationOptions(
		...
		for (byte[] byteOpts : listByteOpts) {
				DataInputStream distrm = new DataInputStream(new ByteArrayInputStream(byteOpts));
				...
				RawSequence<IString> transSeq = new RawSequence<IString>(IStrings.toIStringArray(trans.split("\\s+")));
	...
	static public void extractDB(String fCorpus, String eCorpus, String tcFile, IBMModel1 model1F2E, IBMModel1 model1E2F)
		...
		if (!tmpFPhrases.open(fileTmpFPhrases, BDB.OWRITER | BDB.OTRUNC | BDB.OCREAT ))
			throw new RuntimeException();


		int lineno = -1;
		long phrasesExtracted = 0;
		for (String fLine = fReader.readLine(), eLine = eReader.readLine();
				 fLine != null && eLine != null;
    ...
    while((key = fcur.key()) != null){	
			...
			for (String lowCntKey : lowCnt) {
				System.err.println("Skipping: "+fphrase+" ||| "+lowCntKey+" ||| "+pF2E.getCount(lowCntKey));
        ...
        float pcFgE = (float)Math.log(pE2F.getCount(trans)); // pcFgE
				RawSequence<IString> fphraseSeq = new RawSequence<IString>(IStrings.toIStringArray(fphrase.split("\\s+")));
				RawSequence<IString> transSeq = new RawSequence<IString>(IStrings.toIStringArray(trans.split("\\s+")));
	...
	public static void main(String[] args) throws Exception {
		...
		extractDB(args[0].split(":")[0], args[1].split(":")[0], args[2],
					model1F2E, model1E2F);
	}
}
