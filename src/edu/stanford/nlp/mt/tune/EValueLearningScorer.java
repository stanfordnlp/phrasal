...
public class EValueLearningScorer implements Scorer<String> {
  ...
  public EValueLearningScorer(Map<String, Double> manualFeatureWts,
      double lrate, double momentumTerm) {
    this.lrate = lrate;
    ...
    for (Map.Entry<String, Double> stringDoubleEntry : manualFeatureWts
        .entrySet()) {
      manualWeights[featureIndex.indexOf(stringDoubleEntry.getKey())] = stringDoubleEntry
          .getValue() / 100.0;
    }
  ...
  public double objectiveValue(
      List<Collection<FeatureValue<String>>> featureVectors, double[] l) {
    double Z = 0;
  ...
  static public ClassicCounter<String> dEl(Scorer<String> scorer,
      List<Collection<FeatureValue<String>>> featureVectors, double[] l) {
    ...
    // System.out.printf("Z:%e (vectors: %d)\n", Z, featureVectors.size());
    {
      ...
      for (Collection<FeatureValue<String>> featureVector : featureVectors) {
        ...
        for (FeatureValue<String> feature : summarizedFeatureVector(featureVector)) {
          ...
          // feature.name, delta, p, feature.value,
          // eF.getCount(feature.name),l[lIdx]);
  ...
  public void wtUpdate(List<List<FeatureValue<String>>> featureVectors,
      double[] l, int iter) {
    double Z = 0.0;
    ClassicCounter<String> eF = new ClassicCounter<String>();
    double[] n = new double[l.length];
    ...
    for (int i = 0; i < wts.length; i++) {
      d[i] = -wts[i] * (alpha); // l2 - reg, @ 1e-4 w/1000 dt pts this is like a
                                // Gaussian prior with sigma = 10
    }
    ...
    for (int i = 0; i < wts.length; i++) {
      deltaWts[i] = (momentumTerm + acclTerm) * deltaWts[i]
          + (1.0 - momentumTerm) * d[i];
      if (DEBUG) {
        if (wts[i] != 0) {
          System.err.printf("w: %e (md %e, d: %e) %s\n", wts[i], deltaWts[i],
              d[i], featureIndex.get(i));
        }
      }
    }
  ...
  public static List<FeatureValue<String>> summarizedFeatureVector(
      Collection<FeatureValue<String>> featureValues) {
    ClassicCounter<String> sumValues = new ClassicCounter<String>();
    List<FeatureValue<String>> fVector = new ArrayList<FeatureValue<String>>(
        featureValues.size());
    ...
    for (String featureName : sumValues.keySet()) {
      fVector.add(new FeatureValue<String>(featureName, sumValues
          .getCount(featureName)));
    }
    return fVector;
  }
