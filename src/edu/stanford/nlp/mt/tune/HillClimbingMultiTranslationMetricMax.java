 ...
 * @param <FV>
 */
public class HillClimbingMultiTranslationMetricMax<TK, FV> implements
    MultiTranslationMetricMax<TK, FV> {
  public static final String DEBUG_PROPERTY = "HillClimbingMultiTranslationMetricMaxDebug";
  public static final boolean DEBUG = true; // XXX
                                            // Boolean.parseBoolean(System.getProperty(DEBUG_PROPERTY,
                                            // "false"));
  ...
  public HillClimbingMultiTranslationMetricMax(EvaluationMetric<TK, FV> metric,
      EvaluationMetric<TK, FV> subMetric) {
    this.metric = metric;
    this.greedyMetrixMax = new GreedyMultiTranslationMetricMax<TK, FV>(metric);
  ...
  public List<ScoredFeaturizedTranslation<TK, FV>> maximize(
    ...
    List<ScoredFeaturizedTranslation<TK, FV>> selected = greedyMetrixMax
        .maximize(nbest);
    List<List<ScoredFeaturizedTranslation<TK, FV>>> nbestLists = nbest
        .nbestLists();

    IncrementalEvaluationMetric<TK, FV> incrementalMetric = metric
        .getIncrementalMetric();
    IncrementalEvaluationMetric<TK, FV> incrementalSubMetric = (subMetric != null ? subMetric
        .getIncrementalMetric() : null);
    for (ScoredFeaturizedTranslation<TK, FV> featurizedTranslation : selected) {
      incrementalMetric.add(featurizedTranslation);
      if (incrementalSubMetric != null)
        incrementalSubMetric.add(featurizedTranslation);
    }
    ...
    for (int changes = nbestListsSize; changes != 0 && iter < 25; iter++) { // XXX
      ...
      for (int i = 0; i < nbestListsSize; i++) {
        ...
        if (incrementalSubMetric != null)
          incrementalSubMetric.replace(i, bestFTrans);
      ...
      if (DEBUG) {
        ...
        System.err.printf("%d: score: %.5f changes: %d\n", iter,
            incrementalMetric.score(), changes);
        /*
