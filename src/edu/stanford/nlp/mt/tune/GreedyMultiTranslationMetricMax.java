 ...
 * @param <FV>
 */
public class GreedyMultiTranslationMetricMax<TK, FV> implements
  ...
  public List<ScoredFeaturizedTranslation<TK, FV>> maximize(
    ...
    for (List<ScoredFeaturizedTranslation<TK, FV>> nbestList : nbestLists) {
      ...
      if (!nbestList.isEmpty())
        incrementalMetric.add(nbestList.get(0));
      for (ScoredFeaturizedTranslation<TK, FV> featurizedTranslation : nbestList) {
        incrementalMetric.replace(incrementalMetric.size() - 1,
            featurizedTranslation);
