 ...
 * 
 */
public class DynamicIntegerArrayIndex implements Iterable<int[]>,
    IntegerArrayIndex {
  ...
  /*
   * public boolean contains(int[] key) { int pos = findPos(key); return pos >=
   * 0; }
   */

  /*
   * @SuppressWarnings("unused") public synchronized int commonRepIndexOf(int[]
   * key, boolean add) {//s int pos = findPos(key); if (pos >= 0) return
   * values[pos]; if (!add) return -1; return add(key, -pos-1,true); }
   */
  ...
  public int indexOf(int[] key, boolean add) {
    if (locked) {
      // if (add)
      // throw new
      // UnsupportedOperationException("Can't add key; index is locked.");
