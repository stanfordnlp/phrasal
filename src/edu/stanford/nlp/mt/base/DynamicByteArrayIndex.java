 ...
 */
public class DynamicByteArrayIndex implements Iterable<int[]>,
    IntegerArrayIndex {
  ...
  private int add(byte[] key, int pos, boolean sharedRep) {
    ...
    if (!sharedRep) {
      ...
      keys[pos] = CommonDynamiIntegerArrayIndex
          .getB(CommonDynamiIntegerArrayIndex.indexOf(key, true));
      values[pos] = maxIndex++;
    }
    reverseIndex[values[pos]] = pos;
  ...
  private int indexOf(byte[] key, boolean add) {
    ...
    return add(key, -pos - 1, false); /*
                                       * if (pos != sanityIndex.indexOf(key,
                                       * true)) { System.err.printf("%d != %d",
                                       * pos, sanityIndex.indexOf(key));
                                       * System.exit(-1); }
                                       */
  }
  ...
  public static void main(String[] args) {
    ...
    byte[] c = compress(a);
    int[] d = decompress(c);
    System.out.printf("sz: %d -> %d array: %s\n", c.length * Integer.SIZE,
        d.length * Byte.SIZE, Arrays.toString(d));
  }
