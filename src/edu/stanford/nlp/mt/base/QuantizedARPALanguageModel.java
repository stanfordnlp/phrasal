...
public class QuantizedARPALanguageModel extends ARPALanguageModel {
  ...
  protected void init(String filename) throws IOException {
    ...
    // read header:
    String line = "";
    while (!line.startsWith("qARPA "))
      line = readLineNonNull(reader);
      ...
      while (!(inline = readLineNonNull(reader)).equals("")) {
        ...
        if (prob > Character.MAX_VALUE) {
          throw new RuntimeException(String.format(
              "Token (%d/%d) not a byte at line: \"%s\"", prob,
              (int) Character.MAX_VALUE, inline));
        }
    ...
    System.err
        .printf(
            "Done loading quantized arpa lm: %s (order: %d) (mem used: %d MiB time: %.3f s)\n",
  ...
  protected double scoreR(Sequence<IString> sequence) {
    ...
    if (index >= 0) { // found a match
      // mg2008: the "& 0xFF" is needed because Java doesn't allow
      // "unsigned byte"
      double p = probClusters[order][qprobs[order][index] & 0xFF]
          * log10LogConstant;
      if (verbose)
        System.err.printf(
            "q-scoreR: seq: %s logp: %f (order=%d probCenter=%d)\n",
    ...
    if (index >= 0)
      bow = bowClusters[order2][qbows[order2][index] & 0xFF] * log10LogConstant;
  ...
  public boolean releventPrefix(Sequence<IString> prefix) {
    if (prefix.size() > tables.length - 1)
      return false;
    int[] prefixInts = Sequences.toIntArray(prefix);
