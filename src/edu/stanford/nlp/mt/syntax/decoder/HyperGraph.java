...
public class HyperGraph {
  ...
  // #################### item class ##########################
  /*
   * in and/or graph, Item is a "or" node/vertexit remembers all possible
   * deduction that leads to the same symbol (e.g., NP)state: lhs and edge ngram
   */
  public static class Item implements Comparable {
    public int i, j;
    public ArrayList<Deduction> l_deductions = null;// each deduction is a "and"
                                                    // node
    public Deduction best_deduction = null;
    public int lhs; // this is the symbol like: NP, VP, and so on
    public HashMap tbl_states; // remember the state required by each model, for
                               // example, edge-ngrams for LM model
    ...
    ArrayList l_nbest = new ArrayList();// ArrayList of DerivationState, in the
                                        // paper is: D(^) [v]
    private PriorityQueue<DerivationState> heap_cands = null;// remember
                                                             // frontier states,
                                                             // in the paper, it
                                                             // is called
                                                             // cand[v]
    private HashMap derivation_tbl = null;// rememeber which DerivationState has
                                          // been explored
    private HashMap nbest_str_tbl = null;// maintaining this tbl at each item
                                         // (instead of at goal item only) is
                                         // critical to fastly get unique nbest
                                         // extraction

    public Item(int i_in, int j_in, int lhs_in, HashMap states_in,
        Deduction init_deduction, double est_total_cost_in) {
      i = i_in;
      j = j_in;
      lhs = lhs_in;
    ...
    public Item(int i_in, int j_in, int lhs_in,
        ArrayList<Deduction> l_deductions_in, Deduction best_deduction_in,
        HashMap states_in) {
      i = i_in;
      j = j_in;
      lhs = lhs_in;
    ...
    public void add_deduction_in_item(Deduction dt) {
      ...
      l_deductions.add(dt);
      if (best_deduction == null || best_deduction.best_cost > dt.best_cost)
        best_deduction = dt;
    }

    public void add_deductions_in_item(ArrayList<Deduction> l_dt) {
      for (Deduction dt : l_dt)
        add_deduction_in_item(dt);
    }

    // ########### nbest ##############################
    private DerivationState lazy_k_best_extract(int next_n,
        boolean extract_unique_nbest, boolean extract_nbest_tree) {
      ...
      while (l_nbest.size() < next_n) {
        if (heap_cands.size() > 0) {
          res = heap_cands.poll();
          // derivation_tbl.remove(res.get_signature());//TODO: should remove?
          // note that two state may be tied because the cost is the same
          ...
          lazy_next(res, extract_unique_nbest, extract_nbest_tree);// always
                                                                   // extend the
                                                                   // last, add
                                                                   // all new
                                                                   // hyp into
          ...
          // debug: sanity check
          t_added++;
          if (extract_unique_nbest == false && t_added > 1) {// this is possible
                                                             // only when
                                                             // extracting
                                                             // unique nbest
            Support.write_log_line(
                "In lazy_k_best_extract, add more than one time, nex_n is "
                    + next_n, Support.ERROR);
            System.exit(0);
          }
        } else {
          break;
    ...
    private void lazy_next(DerivationState last, boolean extract_unique_nbest,
        boolean extract_nbest_tree) {
      if (last.p_edge.l_ant_items == null)
        return;
      for (int i = 0; i < last.p_edge.l_ant_items.size(); i++) {// slide the ant
                                                                // item
        ...
        String new_sig = DerivationState.get_signature(last.p_edge, new_ranks,
            last.deduction_pos);
        ...
        it.lazy_k_best_extract(new_ranks[i], extract_unique_nbest,
            extract_nbest_tree);
        if (new_ranks[i] <= it.l_nbest.size()// exist the new_ranks[i]
                                             // derivation
        /* && "t" is not in heap_cands */) {// already checked before, check
                                            // this condition
          double cost = last.cost
              - ((DerivationState) it.l_nbest.get(last.ranks[i] - 1)).cost
              + ((DerivationState) it.l_nbest.get(new_ranks[i] - 1)).cost;
          DerivationState t = new DerivationState(last.p_edge, new_ranks, cost,
              last.deduction_pos);
          heap_cands.add(t);
    ...
    // get my best derivation, add 1best for all my children, used by
    // get_candidates
    private DerivationState get_best_derivation(Deduction hyper_edge,
        int deduct_pos, boolean extract_unique_nbest, boolean extract_nbest_tree) {
      int[] ranks;
      double cost = 0;
      if (hyper_edge.l_ant_items == null) {// axiom
        ranks = null;
        cost = hyper_edge.best_cost;// this Deduction only have one single
        ...
        for (int i = 0; i < hyper_edge.l_ant_items.size(); i++) {
          ...
          child_it.lazy_k_best_extract(ranks[i], extract_unique_nbest,
              extract_nbest_tree);
        }
        cost = hyper_edge.best_cost;// TODO???????????????
      }
      DerivationState t = new DerivationState(hyper_edge, ranks, cost,
          deduct_pos);
      return t;
    }

    // this is the seeding function, for example, it will get down to the leaf,
    // and sort the terminals
    // get a 1best from each deduction, and add them into the heap_cands
    private void get_candidates(boolean extract_unique_nbest,
        boolean extract_nbest_tree) {
      ...
      if (l_deductions == null) {
        System.out
            .println("Error, l_deductions is null in get_candidates, must be wrong");
        System.exit(0);
      }
      int pos = 0;
      for (Deduction hyper_edge : l_deductions) {
        DerivationState t = get_best_derivation(hyper_edge, pos,
            extract_unique_nbest, extract_nbest_tree);
        // why duplicate, e.g., 1 2 + 1 0 == 2 1 + 0 1 , but here we should not
        // get duplicate
        ...
        if (derivation_tbl.containsKey(t.get_signature()) == false) {
          ...
          System.out
              .println("Error: get duplicate derivation in get_candidates, this should not happen");
      ...
      // TODO: if tem.size is too large, this may cause unnecessary computation,
      // we comment the segment to accormodate the unique nbest extraction
      /*
       * if(tem.size()>global_n){ heap_cands=new
       * PriorityQueue<DerivationState>(); for(int i=1; i<=global_n; i++)
       * heap_cands.add(tem.poll()); }else heap_cands=tem;
       */
    }
    ...
    public void print_info(int level) {
      Support.write_log_line(
          String.format("lhs: %s; cost: %.3f", lhs, best_deduction.best_cost),
          level);
    }
    ...
    public String get_signature() {
      ...
      for (Integer st_name : Symbol.l_model_state_names) {
        int[] st = (int[]) tbl_states.get(st_name);// TODO assume the state is
                                                   // an int[] array
        if (st != null) {
          res.append(SIG_SEP);
          for (int i = 0; i < st.length; i++) {
            if (true/*
                     * st[i]!=Symbol.NULL_RIGHT_LM_STATE_SYM_ID &&
                     * st[i]!=Symbol.NULL_LEFT_LM_STATE_SYM_ID &&
                     * st[i]!=Symbol.LM_STATE_OVERLAP_SYM_ID
      ...
      // Support.write_log_line(String.format("Signature is %s", res),
      // Support.INFO);
      return signature;
    }
    ...
    public static String get_string_from_state_tbl(HashMap tbl) {
      ...
      for (int t = 0; t < Symbol.l_model_state_names.size(); t++) {
        ...
        int[] st = (int[]) tbl.get(st_name);// TODO assume the state is an int[]
                                            // array
        if (st != null) {
          res.append(Symbol.get_string(st));
          if (t < Symbol.l_model_state_names.size() - 1)
            res.append(SIG_SEP);
        } else {
          System.out.println("state is null");
          System.exit(0);
    ...
    public static HashMap get_state_tbl_from_string(String state_str) {
      ...
      for (int i = 0; i < Symbol.l_model_state_names.size(); i++) {
        ...
        res.put(st_name, Symbol.get_terminal_ids(state_wrds));// TODO assume the
                                                              // state is an
                                                              // int[] array
      }
      return res;
  ...
  // #########################################
  /*
   * in and/or graph, this is a and node, or a hyper-arc (without including the
   * head vertex/item)
   */
  public static class Deduction { // should remember two costs: best_total_cost,
                                  // and transition cost
                                  // double
                                  // non_stateless_transition_cost=0.0;//this
                                  // remember the non-stateless cost assocated
                                  // with the rule and the previous items (which
                                  // have same states)
    public double best_cost = Symbol.IMPOSSIBLE_COST;// the 1-best cost of all
                                                     // possible derivation:
                                                     // best costs of ant items
                                                     // +
                                                     // non_stateless_transition_cost
                                                     // + r.statelesscost
    private Rule rule;
    // if(l_ant_items==null), then this shoud be the terminal rule
    private ArrayList<Item> l_ant_items = null; // ant items. In comparison, in
                                                // a derivation, the parent
                                                // should be the sub-derivation
                                                // of the tail of the hyper-arc

    public Deduction(Rule rl, double total_cost, double non_stateless_cost,
        ArrayList<Item> ant_items) {
    ...
    public ArrayList<Item> get_ant_items() {
      return l_ant_items;
    }
  }
  ...
  // ##########################
  public void lazy_k_best_extract(ArrayList<Model> l_models, int global_n,
    ...
    int next_n = 0;
    while (true) {
      DerivationState cur = goal_item.lazy_k_best_extract(++next_n,
          extract_unique_nbest, extract_nbest_tree);// global_n is not used at
                                                    // all
      if (cur == null || goal_item.l_nbest.size() < next_n // do not have more
                                                           // hypthesis
          || goal_item.l_nbest.size() > global_n)
        break;

      // get individual model cost
      Item true_item = cur.p_edge.l_ant_items.get(0);// goal_item only has one
                                                     // ant item
      double[] model_cost = new double[l_models.size()];
      FileUtility.write_lzf(out2, sent_id + " ||| ");
      String str_hyp_numeric = ((DerivationState) true_item.l_nbest
          .get(cur.ranks[0] - 1)).get_hyp(extract_nbest_tree, model_cost,
          l_models);
      String str_hyp_str = convert_hyp_2_string(cur, l_models, str_hyp_numeric,
          extract_nbest_tree, add_combined_score, model_cost, true_item);

      // print the str
      FileUtility.write_lzf(out2, str_hyp_str);
      FileUtility.write_lzf(out2, "\n");
      // Support.write_log_line(String.format("nbest: %d;",
      // goal_item.l_nbest.size()) , Support.INFO);
    }
    // g_time_kbest_extract += System.currentTimeMillis()-start;
    // Support.write_log_line("time_kbest_extract: "+
    // Chart.g_time_kbest_extract, Support.INFO);
  }

  private static String convert_hyp_2_string(DerivationState cur,
      ArrayList<Model> l_models, String str_hyp_numeric,
      boolean extract_nbest_tree, boolean add_combined_score,
      double[] model_cost, Item true_item) {
    ...
    for (; t < tem.length; t++) {
      ...
      if (extract_nbest_tree == true
          && (tem[t].startsWith("(") || tem[t].endsWith(")"))) {// tree tag
        if (tem[t].startsWith("(") == true) {
          ...
          // System.out.println("tem is :" + tem[t] + "; the len is: "+
          // tem[t].length());
          // note: it may have more than two ")", e.g., "3499))"
          int first_bracket_pos = tem[t].indexOf(")");// TODO: assume the
                                                      // tag/terminal does not
                                                      // have ")"
          String tag = Symbol.get_string(Integer.valueOf(tem[t].substring(0,
              first_bracket_pos)));
          if (tag.compareTo("PHRASE") == 0)// TODO
            tag = "X";
    ...
    if (model_cost != null) {
      ...
      for (int k = 0; k < model_cost.length; k++) {
        model_cost[k] += l_models.get(k).finaltransition(true_item.tbl_states);// add
                                                                               // final
                                                                               // tranition
      ...
      if (Math.abs(cur.cost - tem_sum) > 1e-2) {
        Support.write_log_line("In nbest extraction, Cost does not match",
            Support.ERROR);
        System.exit(0);
      }
    }
  ...
  /*
   * each Item will maintain a list of this, each of which corresponds to a
   * deduction and its children's ranks remember the ranks of a deduction node
   * used for kbest extraction
   */
  private static class DerivationState implements Comparable {
    Deduction p_edge;// in the paper, it is "e"

    // **lesson: once we define this as a static variable, which cause big
    // trouble
    int deduction_pos; // this is my position in my parent's Item.l_deductions,
    ...
    private static String get_signature(Deduction p_edge2, int[] ranks2, int pos) {
      StringBuffer res = new StringBuffer();
      // res.apend(p_edge2.toString());//Wrong: this may not be unique to
      // identify a Deduction (as it represent the class name and hashcode which
      // my be equal for different objects)
      res.append(pos);
      if (ranks2 != null)
    ...
    // compute model_cost[]
    private void compute_cost(Rule rl, ArrayList<Item> ants_items,
        double[] model_cost, ArrayList l_models) {
      if (model_cost == null)
        return;
      ...
      for (int k = 0; k < l_models.size(); k++) {
        ...
        HashMap tem_tbl = m.transition(rl, ants_states, -1, -1, -1);// TODO: i,
                                                                    // j., j1
        model_cost[k] += ((Double) tem_tbl.get(Symbol.TRANSITION_COST_SYM_ID))
            .doubleValue();
      }
    }

    // if want to get model cost, then have to set model_cost and l_models
    private String get_hyp(boolean tree_format, double[] model_cost,
        ArrayList l_models) {
      StringBuffer res = new StringBuffer();
      Rule rl = p_edge.rule;
      if (rl == null) {// deductions under "goal item" does not have rule
        ...
        for (int id = 0; id < p_edge.l_ant_items.size(); id++) {
          ...
          res.append(((DerivationState) child.l_nbest.get(ranks[id] - 1))
              .get_hyp(tree_format, null, null));
      ...
      for (int c = 0; c < rl.english.length; c++) {
        if (Symbol.is_nonterminal(rl.english[c]) == true) {
          ...
          Item child = p_edge.l_ant_items.get(id);
          res.append(((DerivationState) child.l_nbest.get(ranks[id] - 1))
              .get_hyp(tree_format, model_cost, l_models));
        } else {
