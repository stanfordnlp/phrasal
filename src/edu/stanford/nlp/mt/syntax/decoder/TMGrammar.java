...
public abstract class TMGrammar {
  ...
  public TMGrammar(ArrayList<Model> l_models, String default_ow,
  ...
  // if the span covered by the chart bin is greather than the limit, then
  // return false
  ...
  public static int get_eng_non_terminal_id(String symbol) {
    ...
    /*
     * String new_str = symbol.replaceAll("[\\[\\]\\,]+", "");//TODO int res =
     * new Integer(new_str.substring(new_str.length()-1,
     * new_str.length())).intValue()-1;//TODO: assume only one integer, start
     * from one
     */
    int res = Integer.parseInt(symbol.substring(symbol.length() - 2,
        symbol.length() - 1)) - 1;// TODO: assume only one integer, start from
                                  // one
    // Chart.g_time_check_nonterminal += Support.current_time()-start;
    return res;
    // return (new Integer(new_str.substring(new_str.length()-1,
    // new_str.length()))).intValue();//TODO: assume only one integer, start
    // from zero
  }

  // TODO: we assume all the Chinese training text is lowercased, and all the
  // non-terminal symbols are in [A-Z]+
  public static boolean is_non_terminal(String symbol) {
    ...
    if (symbol.length() <= 4)// [x,?]
      ...
      String non_terminal = symbol.substring(1, symbol.length() - 3); // e.g.,
                                                                      // [PHRASE,2]
  ...
  public abstract static class Rule {
    ...
    public int arity = 0;// TODO: disk-grammar does not have this information,
                         // so, arity-penalty feature is not supported in
                         // disk-grammar

    // public String str;
    /*
     * this remember all the stateless cost: sum of cost of all stateless models
     * (e..g, phrase model, word-penalty, phrase-penalty). The LM model cost is
     * not included here. this will be set by Grmmar.estimate_rule
     */
    ...
    public String convert_to_string() {
      ...
      res.append("[");
      res.append(Symbol.get_string(lhs));
      res.append("] ||| ");
      res.append(Symbol.get_string(french));
      res.append(" ||| ");
