...
public class LMGrammar_JAVA_GENERAL extends LMGrammar {
  /*
   * a backoff node is a hashtable, it may include: (1) probabilititis for next
   * words (2) pointers to a next-layer backoff node (hashtable) (3) backoff
   * weight for this node (4) suffix/prefix flag to indicate that there is
   * ngrams start from this suffix
   */
  private LMHash root = null;
  private int g_n_bow_nodes = 0;
  private int g_n_suffix_nodes = 0;
  static private float MIN_LOG_P = -9999.0f; // ngram prob must be smaller than
                                             // this number
  static private double SUFFIX_ONLY = MIN_LOG_P * 3; // ngram prob must be
  ...
  public LMGrammar_JAVA_GENERAL(int order, boolean is_add_suffix_infor,
    ...
    /*
     * //debug LMHash[] t_arrays = new LMHash[10000000];
     * System.out.println("##### mem used (kb): " + Support.getMemoryUse());
     * System.out.println("##### time used (seconds): " +
     * (System.currentTimeMillis()-start_loading_time)/1000); for(int i=0;
     * i<10000000;i++){ LMHash t_h = new LMHash(5); double j=0.1f; t_h.put(i,
     * j);
     * 
     * //System.out.println("ele is " + t_h.get(i)); t_arrays[i]=t_h;
     * if(i%1000000==0){ System.out.println(i +" ##### mem used (kb): " +
     * Support.getMemoryUse()); System.out.println("##### time used (seconds): "
  ...
  /*
   * note: the mismatch between srilm and our java implemtation is in: when unk
   * words used as context, in java it will be replaced with "<unk>", but srilm
   * will not, therefore thelm cost by srilm may be smaller than by java, this
   * happens only when the LM file have "<unk>" in backoff state
   */
  ...
  protected double get_prob_specific(int[] ngram_wrds_in, int order,
    ...
    if (ngram_wrds[ngram_wrds.length - 1] == Symbol.UNK_SYM_ID) {// TODO: wrong
      ...
      for (int i = ngram_wrds.length - 2; i >= 0; i--) {// reverse search, start
                                                        ...
                                                        // word
        LMHash next_layer = (LMHash) pos.get(ngram_wrds[i]
        ...
        if (next_layer != null) {// have context/bow node
          ...
          if (prob2 != null) {// reset, if backoff, will at least back off to
                              // here
            prob = prob2;
            bow_sum = 0;
          } else {
  ...
  public int[] get_right_equi_state(int[] original_state_in, int order,
    ...
    if (Decoder.use_right_euqivalent_state == false
        || original_state_in.length != g_order - 1)
    ...
    int[] original_state = replace_with_unk(original_state_in);// we do not put
                                                               // this statement
                                                               // at the beging
                                                               // to match the
                                                               // SRILM
    ...
    for (int i = 1; i <= original_state.length; i++) {// forward search
      int[] cur_wrds = Support.sub_int_array(original_state, i - 1,
          original_state.length);
  ...
  public int[] get_left_equi_state(int[] original_state_wrds_in, int order,
    ...
    int[] original_state_wrds = replace_with_unk(original_state_wrds_in);// we
                                                                         // do
                                                                         // not
                                                                         // put
                                                                         // this
    ...
    for (int i = original_state_wrds.length; i > 0; i--) {// BACKWORD search
      ...
      if (have_suffix(cur_wrds) == false) {
        ...
        if (last_wrd == Symbol.UNK_SYM_ID) {
          ...
          res_est_cost += -get_prob(cur_wrds, cur_wrds.length, false);// est
                                                                      // cost
        } else {
          ...
          /*
           * //TODO: for simplicity, we may just need
           * BACKOFF_LEFT_LM_STATE_SYM_ID?? int[] backoff_history =
           * Support.sub_int_array(cur_wrds, 0, cur_wrds.length-1);//ignore last
           * wrd double[] bow = new double[1]; boolean finalized_backoff =
           * check_backoff_weight(backoff_history, bow, 0);//backoff weight is
           * already added outside this function? if(finalized_backoff==true){
           * res_equi_state[i-1]=Symbol.NULL_LEFT_LM_STATE_SYM_ID;//no state, no
           * bow, no est_cost }else{
           * res_equi_state[i-1]=Symbol.BACKOFF_LEFT_LM_STATE_SYM_ID; }
           */
        }
      } else {// have suffix
  ...
  private boolean have_suffix(int[] words) {
    LMHash pos = root;
    for (int i = words.length - 2; i >= 0; i--) {// reverse search, start from
  ...
  protected int replace_with_unk(int in) {
    if (root.containsKey(in) == true || in == Symbol.NULL_RIGHT_LM_STATE_SYM_ID
  ...
  private void add_rule(String line, int order, boolean is_add_suffix_infor,
      boolean is_add_prefix_infor) {// format: prob \t ngram \t backoff-weight
    num_rule_read++;
    ...
    if (is_add_prefix_infor == true) {
      ...
      for (int i = 1; i < order - 1; i++) {// ignore the last prefix
        ...
        for (int j = i; j >= 1; j--) {// reverse search: [1,i]
          ...
          LMHash next_layer = (LMHash) pos.get(cur_sym_id
              + Symbol.lm_end_sym_id);
          if (next_layer != null) {
            pos = next_layer;
          } else {
        ...
        pos.put(Symbol.LM_HAVE_PREFIX_SYM_ID, 1);// only the last node should
                                                 // have this flag
      }
    }
    ...
    if (wrds.length == order + 2) { // have bow weight to add
      pos = root;
      for (int i = order; i >= 1; i--) {// reverse search, start from the last
                                        // word
        int cur_sym_id = Symbol.add_terminal_symbol(wrds[i]);
        LMHash next_layer = (LMHash) pos.get(cur_sym_id + Symbol.lm_end_sym_id);
        if (next_layer != null) {
        ...
        if (i == 1) {// force to override the backoff weight
          ...
          if (pos.containsKey(Symbol.BACKOFF_WGHT_SYM_ID) == false)
            pos.put(Symbol.BACKOFF_WGHT_SYM_ID, NON_EXIST_WEIGHT);// indicate it
                                                                  // is a
                                                                  // backoof
                                                                  // node, to
                                                                  // distinguish
  ...
  /*
   * ###################### not used private boolean have_suffix_old(int[]
   * words){ LMHash pos=root; int i=words.length-1; for(; i>=0; i--){//reverse
   * search LMHash next_layer=(LMHash) pos.get(words[i]+Symbol.lm_end_sym_id);
   * if(next_layer!=null){ pos=next_layer; }else{ break; } } if(i==-1 &&
   * pos.containsKey(Symbol.LM_HAVE_SUFFIX_SYM_ID)) return true; else return
   * false; }
   */
  ...
  public static class LMHash // 4bytes
  {
    ...
    int size = 0;// 8 bytes?
    int[] key_array;// pointer itself is 4 bytes?, when it is newed, then add 10
                    // more bytes, and the int itself
    Object[] val_array;// pointer itself is 4 bytes?, when it is newed, then add
                       // 10 more bytes, and the object itself
