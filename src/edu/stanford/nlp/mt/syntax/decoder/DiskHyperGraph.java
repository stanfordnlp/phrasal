...
public class DiskHyperGraph {
  ...
  HashMap tbl_item_2_id = new HashMap();// map item to id, used for saving
                                        // hypergraph
  HashMap tbl_id_2_item = new HashMap();// map id to item, used for reading
                                        // hypergraph
  int cur_item_id = 1;
  ...
  static int NULL_RULE_ID = -1;// three kinds of rule: regular rule (id>0); oov
                               // rule (id=0), and null rule (id=-1)
  static int OOV_RULE_ID = 0;

  static String RULE_TBL_SEP = " -LZF- ";

  // for writting hyper-graph: (1) saving each hyper-graph; (2) remember each
  // regualar rule used; (3) dump the rule jointly (in case parallel decoding)
