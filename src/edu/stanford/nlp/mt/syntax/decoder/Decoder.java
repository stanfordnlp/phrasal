...
public class Decoder {
	...
	public static void main(String[] args) {
		...
		if(num_parallel_decoders==1){
			...
			if(save_disk_hg == true){d_hg = new DiskHyperGraph(); d_hg.init_write(nbest_file+".hg.items");}
		...
		while((cn_sent=FileUtility.read_line_lzf(t_reader_test))!=null){
			...
			if(sent_id!=0 &&
			   ...
			   sent_id%num_per_thread==0){
				...
				if(save_disk_hg==true) { dhg = new  DiskHyperGraph(); dhg.init_write(cur_nbest_file+".hg.items");}
				ParallelDecoder pdecoder = new ParallelDecoder(cur_test_file, cur_nbest_file, start_sent_id, dhg);
	...
	public static class ParallelDecoder extends Thread {
		 ...
		 DiskHyperGraph dpg;
		  public ParallelDecoder( String test_file_in, String nbest_file_in, int start_sent_id_in, DiskHyperGraph dpg_in){
	...
	public static void decode_a_file(String test_file, String nbest_file, int start_sent_id, DiskHyperGraph dhg){
		BufferedReader t_reader_test = FileUtility.getReadFileStream(test_file,"UTF-8");
		BufferedWriter t_writer_nbest = FileUtility.getWriteFileStream(nbest_file,"UTF-8");
		...
		while((cn_sent=FileUtility.read_line_lzf(t_reader_test))!=null){
			...
			translate(p_tm_grammars,p_l_models,cn_sent.split("\\s+"),l_default_nonterminals, t_writer_nbest, sent_id, topN, dhg);
			sent_id++;
			//if(sent_id>0)break;
		}
		FileUtility.close_read_file(t_reader_test);
	...
	static void translate(TMGrammar[] grs,ArrayList<Model> l_models, String[] sentence, ArrayList<Integer> l_default_nonterminals, BufferedWriter t_writer_nbest, int sent_id, int topN, DiskHyperGraph dhg){
		long start = System.currentTimeMillis();
		...
		if(dhg!=null) dhg.save_hyper_graph(p_hyper_graph, sent_id);
	}
