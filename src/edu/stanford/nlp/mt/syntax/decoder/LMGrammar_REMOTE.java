...
public class LMGrammar_REMOTE extends LMGrammar {
  ...
  public LMGrammar_REMOTE(int order, String remote_symbol_tbl,
      String f_server_lists, int num_servers) {
    super(order);
    ...
    if (num_servers == 1) {
      ...
      /*
       * //debug begin System.out.println("begin to call msrlm");
       * System.out.println("prob is " +
       * ((LMClient_Single)p_lm_client).get_prob_msrlm("i am ok", 3));
       * System.out.println("prob is " +
       * ((LMClient_Single)p_lm_client).get_prob_msrlm("china is with", 3));
       * System.out.println("prob is " +
       * ((LMClient_Single)p_lm_client).get_prob_msrlm("china is a", 3));
       * System.out.println("prob is " +
       * ((LMClient_Single)p_lm_client).get_prob_msrlm("the chinese government",
       * 3));
       * 
       * System.exit(0); //end
       */
    ...
    // read the symbol tbl here (note that the file may already contains the
    // global symbols)
    Symbol.init_sym_tbl_from_file(remote_symbol_tbl, true);// it will also add
                                                           // the global
                                                           // nonterminal symbol
  }
  ...
  private void read_lm_server_lists(String f_server_lists, int num_servers,
      String[] l_lm_server_hosts, int[] l_lm_server_ports,
      double[] l_lm_server_weights) {
    ...
    while ((line = FileUtility.read_line_lzf(t_reader)) != null) {
      ...
      count++;
      System.out.println("lm server: " + "lm_file: " + lm_file + "; host: "
          + host + "; port: " + port + "; weight: " + weight);
    }
    ...
    while ((line = FileUtility.read_line_lzf(t_reader_config)) != null) {
      ...
      if (line.indexOf("=") != -1) {// parameters
        ...
        if (fds[0].compareTo("lm_file") == 0) {
          ...
          System.out.println(String.format("interpolation_weightt: %s",
              interpolation_weight));
        } else {
          Support.write_log_line("Warning: not used config line: " + line,
              Support.ERROR);
          // System.exit(0);
        }
      }
  ...
  // format: lm_file host port weight
  private void read_lm_server_lists_old(String f_server_lists, int num_servers,
      String[] l_lm_server_hosts, int[] l_lm_server_ports,
      double[] l_lm_server_weights) {
    BufferedReader t_reader = FileUtility.getReadFileStream(f_server_lists,
        "UTF-8");
  ...
  public void write_vocab_map_srilm(String fname) {
    System.out
        .println("Error: call write_vocab_map_srilm in remote, must exit");
    System.exit(0);
  }
  ...
  protected double get_prob_backoff_state_specific(int[] ngram_wrds, int order,
    ...
    System.out
        .println("Error: call get_prob_backoff_state_specific in LMGrammar_REMOTE, must exit");
    System.exit(0);
    return -1;
  ...
  @Override
  public int[] get_left_equi_state(int[] original_state, int order,
      double[] cost) {
    return original_state;
    /*
     * if(Decoder.use_left_euqivalent_state==false) return original_state;
  ...
  public int[] get_right_equi_state(int[] original_state, int order,
    ...
    return original_state;
    /*
     * if(Decoder.use_right_euqivalent_state==false) return original_state;
  ...
  public int replace_with_unk(int in) {
    System.out
        .println("Error: call replace_with_unk in LMGrammar_REMOTE, must exit");
    System.exit(0);
    return 0;
  }
