...
public class KBestParseForest {
  ...
  public KBestParseForest(int end, int K) {
    this.K = K;
  ...
  public boolean add(int s, int r, int t, int type, int dir, int comp,
      double score, FeatureVector fv, ParseForestItem p1, ParseForestItem p2) {

    boolean added = false;

    if (chart[s][t][dir][comp][0] == null) {
      for (int i = 0; i < K; i++)
        chart[s][t][dir][comp][i] = new ParseForestItem(s, r, t, type, dir,
            comp, Double.NEGATIVE_INFINITY, null, null, null);
    }
  ...
  public FeatureVector getFeatureVector(ParseForestItem pfi) {
    ...
    return cat(pfi.fv,
        cat(getFeatureVector(pfi.left), getFeatureVector(pfi.right)));
  }
  ...
  public String getDepString(ParseForestItem pfi) {
    ...
    if (pfi.comp == 0) {
      ...
      return (pfi.t + "|" + pfi.s + ":" + pfi.type + " " + (getDepString(pfi.left)
          + " " + getDepString(pfi.right)).trim()).trim();
    }
  }
  ...
  public int[][] getKBestPairs(ParseForestItem[] items1,
      ParseForestItem[] items2) {
    ...
    if (items1 == null || items2 == null || items1[0] == null
        || items2[0] == null)
      return result;

    BinaryHeap heap = new BinaryHeap(K + 1);
    int n = 0;
    ValueIndexPair vip = new ValueIndexPair(items1[0].prob + items2[0].prob, 0,
        0);
    ...
    while (n < K) {
      ...
      if (!beenPushed[vip.i1 + 1][vip.i2]) {
        heap.add(new ValueIndexPair(items1[vip.i1 + 1].prob
            + items2[vip.i2].prob, vip.i1 + 1, vip.i2));
        beenPushed[vip.i1 + 1][vip.i2] = true;
      }
      if (!beenPushed[vip.i1][vip.i2 + 1]) {
        heap.add(new ValueIndexPair(items1[vip.i1].prob
            + items2[vip.i2 + 1].prob, vip.i1, vip.i2 + 1));
...
class BinaryHeap {
  ...
  public BinaryHeap(int def_cap) {
    theArray = new ValueIndexPair[def_cap + 1];
