...
public class Rule {
  ...
  // Whether to print syntactic constituents in the rhs of each rule (e.g.,
  // x0:NP instead of just x0)
  public static final String SYNCAT_RHS_PROPERTY = "SynCatRHS";
  public static final boolean SYNCAT_RHS = Boolean.parseBoolean(System
      .getProperty(SYNCAT_RHS_PROPERTY, "false"));
  ...
  // Note: it would probably be safe to move from char to byte (i.e., 16 to 8
  // bits)
  ...
  /**
   * Because of unaligned foreign words, there may be more than one minimal rule
   * extractible from a given tree node; this function returns all of them. To
   * Avoid combinatorial explosion, this function returns just one minimal rule
   * if the number of unaligned foreign words appearing in the RHS of the rule
   * is greater than a given threshold.
   * 
   * @param uRHS
   *          Set of indices of unaligned words in RHS
   */
  ...
  private void printXrsLHS(StringBuffer buf, MutableInteger arrayIdx,
      MutableInteger varIdx) {
  ...
  private void printJoshuaRHS(StringBuffer buf, MutableInteger arrayIdx,
      Map<Integer, Integer> idxToVar) {
    int sz = lhsStruct[arrayIdx.intValue()];
    if (sz == 0)
      return;
    for (int i = 0; i < sz; ++i) {
      ...
      if (isNT) {
        buf.append("[").append(tok).append(",").append(idxToVar.get(arrayPos))
            .append("]");
      } else if (isLex) {
        buf.append(tok);
      }
  ...
  public boolean equals(Object object) {
    ...
    Rule r = (Rule) object;
    return (Arrays.equals(lhsStruct, r.lhsStruct)
        && Arrays.equals(lhsLabels, r.lhsLabels)
        && Arrays.equals(rhsLabels, r.rhsLabels) && Arrays.equals(rhs2lhs,
        r.rhs2lhs));
  }

  @Override
  public int hashCode() {
    return Arrays.hashCode(new int[] { Arrays.hashCode(lhsStruct),
        Arrays.hashCode(lhsLabels), Arrays.hashCode(rhsLabels),
  ...
  public int[] getTreeLHSIntArray() {
    ...
    System.arraycopy(lhsLabels, 0, array, lhsStruct.length + 1,
        lhsLabels.length);
    return array;
  }
