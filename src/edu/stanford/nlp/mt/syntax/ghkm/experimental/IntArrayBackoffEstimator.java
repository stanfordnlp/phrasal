...
/**
 * Interpolated backoff ML estimation of multinomials using Witten-Bell
 * discounting. Currently used to smooth lexical probabilities of sytnax-based
 * models in edu.stanford.nlp.mt.syntax.train (similarly to Michael Collins's
 * thesis, p. 65).
 * 
...
public class IntArrayBackoffEstimator {
  ...
  final DynamicIntegerArrayPrefixIndex index = new DynamicIntegerArrayPrefixIndex(),
  ...
  /**
   * Returns the interpolated (backoff) probability of idx given cIdx. Note that
   * backoff goes from right to left, i.e., 0-5, 0-4, 0-3, etc.
   */
  ...
  /**
   * Returns the interpolated (backoff) log-probability of idx given cIdx. Note
   * that backoff goes from right to left, i.e., 0-5, 0-4, 0-3, etc.
   */
  public double logProbabilityOf(int[] idx, int[] cIdx) {
    ...
    for (int i = 0; i <= cIdx.length; ++i) {
      ...
      double logpMLE = (nCount > 0.0 && ji < counts.size()) ? Math.log(counts
          .get(ji) / nCount) : Double.NEGATIVE_INFINITY;
      double old_logp = logp;
      logp += Math.log(1 - lambda);
      double logLambda = lambda > 0 ? Math.log(lambda)
          : Double.NEGATIVE_INFINITY;
      logp = ArrayMath.logSum(new double[] { logpMLE + logLambda, logp });
      if (DEBUG) {
        System.err.printf(
            "p%d = p_mle%d*lambda + p%d*(1-lambda) = %f*%f + %f*%f = %f\n",
            i + 1, i, i, Math.exp(logpMLE), lambda, Math.exp(old_logp),
            1 - lambda, Math.exp(logp));
      }
    }
    assert (logp == logp);
    return logp;
  ...
  /**
   * Provides sample for estimating p(idx | cIdx). Increase counts of c(idx,
   * cIdx) and c(cIdx) and all relevant backoff counts. Note that backoff goes
   * from right to left, i.e., 0-5, 0-4, 0-3, etc.
   * 
   * @param idx
   *          event whose probability we need to estimate.
   * @param cIdx
   *          context
   */
  ...
  public void addTuningSample(int[] idx, int[] cIdx) {
    IntPair p = new IntPair(index.indexOf(idx, true),
        nIndex.indexOf(cIdx, true));
  ...
  /**
   * Simple test case: train and test 3gram LM, and optionally tune backoff
   * coefficients to maximize log-likelihood of a dev set.
   */
  public static void main(String[] args) {
    ...
    default:
      System.err
          .println("Usage: BackoffProbabilityDistribution <train_corpus> [<tune_corpus>] <test_corpus> <backoff_constant>");
      System.exit(1);
    }
    ...
    for (String line : ObjectBank.getLineIterator(trainStr)) {
      ...
      for (int i = tokens.length - 3; i >= 0; --i)
        mle.addTrainingSample(new int[] { tokens[i] }, new int[] {
    ...
    if (tuneStr != null) {
      ...
      for (String line : ObjectBank.getLineIterator(tuneStr)) {
        int[] tokens = toReverseIntArray(line);
        for (int i = tokens.length - 3; i >= 0; --i)
          mle.addTuningSample(new int[] { tokens[i] }, new int[] {
              tokens[i + 1], tokens[i + 2] });
      }
    ...
    for (String line : ObjectBank.getLineIterator(testStr)) {
      ...
      for (int i = tokens.length - 3; i >= 0; --i) {
        ...
        System.out.printf("p(%s | %s %s) = ", IString.getString(tokens[i]),
            IString.getString(tokens[i + 2]), IString.getString(tokens[i + 1]));
        System.out.print((float) Math.exp(logp));
        System.out.println("\tconfidence in bigram prefix: "
            + (float) mle.getLambda(context));
        totalLogp += logp;
      }
  ...
  /**
   * Returns the partial dervatives of the interpolated (backoff) probability of
   * idx given cIdx.
   * 
   * Derivative:
   * 
   ...
   * 
   * f(3) = l_3 * mle3 + (1-l_3) * f(2) f(2) = l_2 * mle2 + (1-l_2) * f(1) f(1)
   * = l_1 * mle1 + (1-l_1) * mleD
   * 
   * df(3)/dl_3 = mle3 df(3)/dl_2 = mle2 df(3)/dl_1 = mle1 - mleD
   * 
  ...
  double[] diffLogProbabilityOf(int[] idx, int[] cIdx) {
    ...
    for (int i = 0; i <= cIdx.length; ++i) {
      ...
      double mlei = (nCount > 0.0 && ji < counts.size()) ? counts.get(ji)
          / nCount : 0.0;
      dlogp[i] += mlei;
  ...
  /*
   * static void printArray(String id, int[] a) { String[] strs =
   * IStrings.toStringArray(a);
