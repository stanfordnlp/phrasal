...
public class FillInNP {
  ...
  /**
   * Takes two files (NPs.txt and all, currently in ~pichuan/javanlp/) and make
   * a new list of NPs. Then I annotate the "new" ones myself and get a new
   * file: project/mt/src/mt/translationtreebank/data/finalCategories_all.txt
   */
  public static void main(String[] args) throws Exception {
    ...
    for (int fileid : nps_NPs.keySet()) {
      ...
      for (int npid : np_NPs.keySet()) {
        ...
        System.err.println("NP:\t" + fileid + "\t" + npid + "\t" + np.type
            + "\t" + np.ch + "\t" + np.en);
      }
    }
    ...
    for (int fileid : nps_all.keySet()) {
      ...
      if (np_NPs == null) {
        ...
        for (int npid : np_all.keySet()) {
          ...
          System.out.println("new" + fileid + "\t" + npid + "\t" + np.type
              + "\t" + np.ch + "\t" + np.en);
        }
      } else {
        ...
        for (int npid : np_all.keySet()) {
          ...
          if (npN != null && npN.ch.equals(np.ch)) {
            System.out.println("NPs" + "\t" + fileid + "\t" + npid + "\t"
                + npN.type + "\t" + npN.ch + "\t" + npN.en);
            System.err.println("matched! ptr_np=" + ptr_np + ", npid=" + npid
                + ", " + npN.ch);
            npN = null; // matched away
          } else {
            System.out.println("new" + "\t" + fileid + "\t" + npid + "\t"
                + np.type + "\t" + np.ch + "\t" + np.en);
            if (npN == null)
