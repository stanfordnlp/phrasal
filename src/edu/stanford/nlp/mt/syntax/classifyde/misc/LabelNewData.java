...
class LabelNewData {
      ...
      if (sentNPs != null) {
        for (Map.Entry<String, NPdata> e : sentNPs.entrySet()) {
          ...
          if (npdata.chNP.equals(chNP)) {
            if (!npdata.entrans.equals(entrans))
              System.err.println("ch matched but not en: " + line
                  + "\n-----------------");
            if (!npdata.autocat.equals(autocat))
              System.err.println("autocat not matched: " + line
                  + "\n-----------------");
    ...
    for (Map.Entry<String, Map<String, NPdata>> e : newNPs.entrySet()) {
      ...
      for (Map.Entry<String, NPdata> e2 : sentNPs.entrySet()) {
        ...
        if (npdata.finalcat == null) {
          // try to fix it first
          if (npdata.autocat.startsWith("A B")) {
            npdata.handcat = "";
            npdata.finalcat = "A B";
          }
          if (npdata.autocat.startsWith("no B")) {
            npdata.handcat = "";
            npdata.finalcat = "no B";
          }
          if (npdata.autocat.startsWith("other")) {
            npdata.handcat = "";
            npdata.finalcat = "other";
          }
          if (npdata.autocat.startsWith("relative clause")) {
            npdata.handcat = "";
            npdata.finalcat = "relative clause";
          }
          if (npdata.autocat.startsWith("B of A")) {
            npdata.handcat = "";
            npdata.finalcat = "B of A";
          }
          if (npdata.autocat.startsWith("B prep A")) {
            npdata.handcat = "";
            npdata.finalcat = "B prep A";
          }
        }
  ...
  static void print(String sentid, String npid, NPdata npdata) {
    ...
    System.out.print("\t");
    if (npdata.handcat != null)
      System.out.print(npdata.handcat);
    else
      System.out.print("");
    System.out.print("\t");
    if (npdata.finalcat != null)
      System.out.print(npdata.finalcat);
    else
      System.out.print("");
    System.out.print("\t");
