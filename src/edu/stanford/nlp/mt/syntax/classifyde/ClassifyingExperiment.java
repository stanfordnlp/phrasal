...
class ClassifyingExperiment {
  ...
  public void run(Properties props, List<String> trainDevTest, boolean verbose)
      throws Exception {
    ...
    ExperimentUtils.TOPICALITY_SENT_WINDOW_SIZE = Integer
        .parseInt(windowSizeStr);
    System.err.println("ExperimentUtils.TOPICALITY_SENT_WINDOW_SIZE="
        + ExperimentUtils.TOPICALITY_SENT_WINDOW_SIZE);

    String writeClassifier = props.getProperty("writeClassifier", null);

    String featurizerStr = props.getProperty("featurizer",
        "mt.classifyde.FullInformationFeaturizer");
    Featurizer featurizer = (Featurizer) Class.forName(featurizerStr)
        .newInstance();
    ...
    for (int tpidx = 0; tpidx < atreepairs.size(); tpidx++) {
      ...
      while (prevTpIdx >= 0
          && tpidx - prevTpIdx <= ExperimentUtils.TOPICALITY_SENT_WINDOW_SIZE) {
        ArrayList<Label> prevSent = atreepairs.get(prevTpIdx).chParsedTrees()
            .get(0).yield();
        for (Label w : prevSent) {
      ...
      for (int deIdxInSent : validSent.NPwithDEs_deIdx_set) {
        Counter<String> featureList = featurizer.extractFeatures(deIdxInSent,
            validSent, props, cachedWords);
        ...
        if (sixclass && !ExperimentUtils.is6class(label))
          notAdd = true;
        if (!sixclass && !ExperimentUtils.is5class(label))
          notAdd = true;
        ...
        Datum<String, String> d = new RVFDatum<String, String>(featureList,
            label);
        ...
        if (notAdd || "n/a".equals(trainDevTest.get(npid))) {
          ...
          throw new RuntimeException("trainDevTest error, line: "
              + trainDevTest.get(npid));
        }
    ...
    if (npid != trainDevTest.size()) {
      ...
      System.err
          .println("#np doesn't match trainDevTest. Is this partial test?");
    }
    ...
    if (verbose) {
      ...
      System.out.println(((Dataset<String, String>) trainDataset)
          .toSummaryStatistics());
      System.out.println();
      ...
      TwoDimensionalCounter<String, String> confusionMatrix = getConfusionMatrix(
          otherData, classifier);
      evaluateOnSet(confusionMatrix);
      System.out.println();
    }
  }

  public static void displayEval(
      TwoDimensionalCounter<String, String> trainStats,
      TwoDimensionalCounter<String, String> devStats,
      TwoDimensionalCounter<String, String> testStats) {
  ...
  public static TwoDimensionalCounter<String, String> getConfusionMatrix(
      List<Datum<String, String>> data, LinearClassifier<String, String> lc) {
    return getConfusionMatrix(data, lc, null);
  }

  public static TwoDimensionalCounter<String, String> getConfusionMatrix(
      List<Datum<String, String>> data, LinearClassifier<String, String> lc,
      PrintWriter decisionPW) {
    ...
    for (Datum<String, String> d : data) {
      ...
      if (decisionPW != null)
        decisionPW.printf("%s\t%s\n", d.label(), predictedClass);
    }
    return confusionMatrix;
  }

  static void evaluateOnSet(
      TwoDimensionalCounter<String, String> confusionMatrix) {
    ...
    for (String k2 : secondKeySet) {
      String normK2 = k2;
      if (normK2.equals("relative clause"))
        normK2 = "relc";
      else
        normK2 = normK2.replaceAll(" ", "");
