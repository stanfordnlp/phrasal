...
public class WordFeatureExtractor implements FeatureExtractor {
  ...
  static {
    OPTS.addAll(Arrays.asList("src_j", "tgt_i", "src_j2", "src_j_tgt",
        "src_j_src_j2", "window"));
  }
  ...
  public WordFeatureExtractor(Properties props) throws Exception {
    ...
    SRCJ_SRCJ2 = Boolean.parseBoolean(props
        .getProperty("src_j_src_j2", "false"));
  ...
  public List<String> extractFeatures(AlignmentMatrix matrix,
    ...
    if (SRCJ)
      for (int d = -WINDOW; d <= WINDOW; d++) {
        ...
        feature.append(d).append(":")
            .append(matrix.getSourceWord(example.src_j + d));
    ...
    if (TGT)
      for (int d = -WINDOW; d <= WINDOW; d++) {
        ...
        feature.append(d).append(":")
            .append(matrix.getTargetWord(example.tgt_i + d));
        features.add(feature.toString());
      }
    ...
    if (SRCJ2)
      for (int d = -WINDOW; d <= WINDOW; d++) {
        ...
        feature.append(d).append(":")
            .append(matrix.getSourceWord(example.src_jprime + d));
        features.add(feature.toString());
      }
    ...
    if (SRCJ_TGT) {
      ...
      feature.append(matrix.getSourceWord(example.src_j)).append("-")
          .append(matrix.getTargetWord(example.tgt_i));
      features.add(feature.toString());
    }
    ...
    if (SRCJ_SRCJ2) {
      ...
      feature.append(matrix.getSourceWord(example.src_j)).append("-")
          .append(matrix.getSourceWord(example.src_jprime));
      features.add(feature.toString());
    }
