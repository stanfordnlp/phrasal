...
public class SocketUtility {
  ...
  // ############# client side #########
  // connect to server
  public static ClientConnection open_connection_client(String hostname,
      int port) {
    ...
    try {
      ...
      // This method will block no more than timeoutMs If the timeout occurs,
      // SocketTimeoutException is thrown.
      int timeoutMs = 3000; // 2 seconds
      res.socket.connect(sockaddr, timeoutMs);
      res.socket.setKeepAlive(true);
      // file
      res.in = new BufferedReader(new InputStreamReader(
          res.socket.getInputStream()));
      res.out = new PrintWriter(new OutputStreamWriter(
          res.socket.getOutputStream()));
      // res.data_in = new DataInputStream( new BufferedInputStream(
      // res.socket.getInputStream())) ;
  ...
  public static class ClientConnection {
    ...
    public String exe_request(String line_out) {
      ...
      try {
        ...
        out.flush();
        line_res = in.readLine(); // TODO block function, big bug, the server
                                  // may close the section (e.g., the server
                                  // thread is dead due to out of memory(which
                                  // is possible due to cache) )
