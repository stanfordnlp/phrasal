...
public class FeatureSetBank {
  ...
  public static FeatureSetBank load(String filename, int max)
      throws IOException {
    if (max == -1)
    ...
    try {
      if ((line = file.readLine()) == null) {
        ...
        throw new RuntimeException(
            "in FeatureSetBank, data and hyp accessing has to be linear.");
      }
      for (; line != null; line = file.readLine()) {
        ...
        while (true) {
          ...
          if (split_i != -1)
            fields_i = fields1.substring(start, split_i);
          else
            fields_i = fields1.substring(start);
