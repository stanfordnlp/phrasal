...
public class BallseptronLearner extends AbstractOneOfManyClassifier {
  public enum   OptConstraint { oneZero, marginRescale, slackScale };
  ...
  public static final OptConstraint DEFAULT_OPT_CONSTRAINT =
    OptConstraint.marginRescale;
    ...
    if (optConstraintProp != null) {
      ...
      for (OptConstraint val : OptConstraint.values()) {
        if (val.toString().equals(optConstraintProp)) {
           optConstraint = val; hasMatch = true;
        }
      }
      if (!hasMatch) {
  ...
  public BallseptronLearner(double radius, double lrate, int maxEpochs,
     ...
     this.radius = radius; this.lrate = lrate; this.maxEpochs = maxEpochs;
  ...
  private double weightUpdate(CompactHypothesisList chl,
                              int worstViolator) {
     ...
     if (margin > effectiveRadius) return margin;

     for (int i = 0; i < actualPt.length; i++) {
       wts[actualIndicies[i]] +=
        eLrate * (actualPt[i] - effectiveRadius*wts[actualIndicies[i]]/wtLen);}
     for (int i = 0; i < worstVPt.length; i++) {
       wts[worstVIn[i]] -=
          eLrate * (worstVPt[i] + effectiveRadius*wts[worstVIn[i]]/wtLen); }
  ...
  @Override
    ...
    Arrays.fill(wts, 1.0);
    int sz = lchl.size(); double lastAvgMargin = Double.NEGATIVE_INFINITY;
    double runningDelta = 0;
    eLrate = lrate;
    for (int epoch = 0; epoch < maxEpochs; epoch++) {
      // primitive learning rate schedule
      if (epoch > 0 && (epoch % 500) == 0) eLrate *= 0.10;
      double avgMargin = 0; double avgPosMargin = 0;
      int correct = 0;
      for (CompactHypothesisList chl : lchl) {
         ...
         avgMargin += margin;
         if (margin > 0) { avgPosMargin += margin; }
      }
      ...
      System.out.printf(
         ...
         "Accuracy: %.3f (%d/%d)\n",
            ...
            avgPosMargin/sz, correct/(double)sz,
            correct, sz);
      if (runningDelta < Double.POSITIVE_INFINITY) {
         runningDelta *= 0.75; runningDelta += marginDelta;
      } else runningDelta = marginDelta;
