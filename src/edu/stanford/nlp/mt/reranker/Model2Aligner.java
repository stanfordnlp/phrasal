...
/**
 * Creates IBM Model 2 alignments between source and target language sentences
 * based on a GIZA++ translation probability table.
 * 
 * Displacement formula is based on William's 224n project, and the default
 * parameters are tuned so as to minimize AER on the French-English corpus. You
 * may wish to tune those yourself if you want better alignments.
 * 
 * Although we retain the lexicalized lexical NULL probabilities from the Model
 * 1 class, we do have a fixed dispalcement NULL probability.
 **/
public class Model2Aligner extends Model1Aligner {
  ...
  /*
   * these two have been tuned to minimize AER on some French-English corpus as
   * part of William's 224n project
   */
  ...
  public Model2Aligner() throws Exception {
    super();
  } /* java sucks */
  ...
  private double bucketScore(double posE, double posF, double lenE, double lenF) {
    ...
    if (bucketNumber > numBuckets - 1)
      ...
      score = (numBuckets - bucketNumber) * bucketSlope;
  ...
  public double displacementProb(String source, String target, int sourcePos,
    ...
    if (targetPos == -1)
      return nullDisplacementProb; /* aligning to null */
    ...
    for (int i = 0; i < targetLen; i++)
      norm += bucketScore(i, sourcePos, targetLen, sourceLen);
