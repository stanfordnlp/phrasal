...
public class TreeToSpans {
  ...
  public static List<String> printSpans(Tree t, int startIdx) {
    ...
    if (t.isLeaf() || t.isPreTerminal()) return spans;
    ...
    for (int i = 1; i < kids.length; i++) {
      ...
      Tree kid = kids[i];
      if (DEBUG) System.err.printf("DEBUG: kid = %s\n",kid);
    ...
    if (kids.length >=2) {
      ...
      sb.append(StringUtils.join(iStrs,","))
        .append(",")
        .append(StringUtils.join(label,","))
        .append(",")
        .append(t.nodeString());
      spans.add(sb.toString());
      //System.out.printf("%s,%s,%s\n", StringUtils.join(iStrs,","), StringUtils.join(label,","), t.nodeString());
    }
