...
public class FilterNGramsByCoOccurrences {
  ...
  public static boolean canCoOccur(Map<IString, BitSet> wordSents,
      String[] words) {
    ...
    for (String word : words) {
      ...
      if (sentIds == null)
        return false;
      if (intersect == null)
        intersect = (BitSet) sentIds.clone();
      else
        intersect.and(sentIds);
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (!type.equals("raw") && !type.equals("arpa")) {
      err.printf("Unsupported type: %s\n", type);
      exit(-1);
    }
        ...
        if (type.equals("raw")) {
          ...
          if (line.startsWith("\\") || line.equals("")) {
            if (line.endsWith("-grams:")) {
              String strId = line.substring(1,
                  line.length() - "-grams:".length());
