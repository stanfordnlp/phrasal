...
/*
 ...
 [Trivial changes (make class variables non-static, static methods static,
 etc.) done at Stanford.]
...
public class TERcalc {
  ...
  public void setRefLen(List reflens) {
    ...
    if (reflens == null || reflens.isEmpty()) {
      ref_len = -1.0;
      return;
    }
    ...
    for (int i = 0; i < reflens.size(); ++i) {
      String reflen = (String) reflens.get(i);
      if (reflen.length() == 0)
        ref_len += 0.;
      else
  ...
  public TERalignment TER(Comparable[] hyp, Comparable[] ref, TERcost costfunc) {
    ...
    to_return.allshifts = (TERshift[]) allshifts.toArray(new TERshift[allshifts
        .size()]);
  ...
  private Map BuildWordMatches(Comparable[] hyp, Comparable[] ref) {
    ...
    hwhash.addAll(Arrays.asList(hyp));
  ...
  private Object[] CalcBestShift(Comparable[] cur, Comparable[] hyp,
      Comparable[] ref, Map rloc, TERalignment med_align, TERcost costfunc) {
    /*
    ...
    TERshift[][] poss_shifts = GatherAllPossShifts(cur, ref, rloc, med_align,
        herr, rerr, ralign, costfunc);
    ...
    for (int i = poss_shifts.length - 1; i >= 0; i--) {
      if (DEBUG)
        System.out.println("Considering shift of length " + i + " ("
            + poss_shifts[i].length + ")");
      ...
      for (int s = 0; s < poss_shifts[i].length; s++) {
        ...
        if (DEBUG) {
          System.out.println("Gain for " + curshift + " is " + gain
              + ". (result: [" + TERalignment.join(" ", shiftarr) + "]");
        ...
        if ((gain > 0) || ((cur_best_shift_cost == 0) && (gain == 0))) {
          ...
          if (DEBUG)
            System.out.println("Tmp Choosing shift: " + cur_best_shift
                + " gives:\n" + cur_best_align + "\n");
        }
  ...
  private TERshift[][] GatherAllPossShifts(Comparable[] hyp, Comparable[] ref,
      Map rloc, TERalignment align, boolean[] herr, boolean[] rerr,
      int[] ralign, TERcost costfunc) {
    ...
    for (int start = 0; start < hyp.length; start++) {
      ...
      boolean ok = false;
      Iterator mti = ((Set) rloc.get(hyplist.subList(start, start + 1)))
          .iterator();
      ...
      ok = true;
      for (int end = start; (ok && (end < hyp.length) && (end < start
          + MAX_SHIFT_SIZE)); end++) {
        ...
        while (movetoit.hasNext()) {
          ...
          /*
           * check to see if there are any errors in either string (only move if
           * this is the case!)
           */
          ...
          for (int roff = -1; roff <= (end - start); roff++) {
            ...
            if ((roff == -1) && (moveto == 0)) {
              if (DEBUG)
                System.out.println("Consider making " + start + "..." + end
                    + " moveto: " + moveto + " roff: " + roff
                    + " ralign[mt+roff]: " + -1);
    ...
    for (int i = 0; i < to_return.length; i++) {
      to_return[i] = (TERshift[]) allshifts[i]
          .toArray(new TERshift[allshifts[i].size()]);
    }
    return to_return;
  }
  ...
  private Object[] PerformShift(Comparable[] words, int start, int end,
      int newloc) {
    int c = 0;
    ...
    if (DEBUG) {
      if (hypSpans != null) {
        System.out.println("word length: " + words.length + " span length: "
            + hypSpans.length);
      } else {
    ...
    if (newloc == -1) {
      ...
      for (int i = end + 1; (i < words.length)
          && (i <= (end + (newloc - start))); i++) {
        nwords[c++] = words[i];
        if (hypSpans != null)
          spans[c - 1] = hypSpans[i];
      }
  ...
  private static boolean spanIntersection(String refSpan, String hypSpan) {
    ...
    return (Integer.valueOf(rSpans[1]) >= Integer.valueOf(hSpans[0]) && Integer
        .valueOf(rSpans[0]) <= Integer.valueOf(hSpans[1]));
  }
  ...
  private static final int MAX_SHIFT_SIZE = 10; // this is genuinely a constant;
