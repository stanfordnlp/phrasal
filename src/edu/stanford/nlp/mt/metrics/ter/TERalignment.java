...
public class TERalignment {
  ...
  public String toMoreString() {
    ...
    s += String.format("TER Score: %1$6.2f (%2$5.1f/%3$5.1f)\n",
        100 * this.score(), this.numEdits, this.numWords);
  ...
  public static void performShiftArray(HashMap hwords, int start, int end,
      int moveto, int capacity) {
    ...
    if (moveto == -1) {
      ...
      copyHashWords(hwords, nhwords, moveto + 1, start - 1, end - start
          + moveto + 2);
      copyHashWords(hwords, nhwords, end + 1, capacity, end + 1);
    } else if (moveto > end) {
      ...
      copyHashWords(hwords, nhwords, end + moveto - start + 1, capacity, end
          + moveto - start + 1);
    }
    hwords.clear();
  ...
  private String prtShift(Comparable[] ref, TERshift[] allshifts) {
    ...
    if (allshifts != null) {
      for (int i = 0; i < allshifts.length; ++i) {
        ...
        to_return += "\nShift " + allshifts[i].shifted + " " + dist + " words "
            + direction;
  ...
  private String getPraStr(Comparable[] ref, Comparable[] aftershift,
    ...
    if (allshifts != null) {
      for (int i = 0; i < allshifts.length; ++i) {
         ...
         * ostart +"," + oend +","+ odest + "," + align_info.size());
         */
        if (slen > 0) {
    ...
    if (alignment != null) {
      for (int i = 0; i < alignment.length; ++i) {
        ...
        case 'S':
        case 'T':
          l = Math.max(ref_wd.length(),
              Math.max(hyp_wd.length(), Math.max(1, shift_in_str.length())));
          hstr += " " + hyp_wd;
          rstr += " " + ref_wd;
  ...
  private static void copyHashWords(HashMap ohwords, HashMap nhwords,
