...
/**
 * Find a linear combination of automatic metrics that best correlates with
 * human assessments. Metrics currently supported: BLEU, NIST, BLEU precision,
 * NIST precision, BLEU ngram precision (1..4), BLEU brevity penalty (matched
 * against closest reference), NIST brevity penalty (matched against average
 * reference length), and TER. For TER, it is possible to tune the cost of each
 * edit type.
 * 
...
public class LinearCombinationMetricOptimizer implements Function {
  ...
  private static IString TER = new IString("ter"), BLEU = new IString("bleu"),
      LBLEU = new IString("lbleu"), BLEU_BP = new IString("bleu_bp"),
      LBLEU_BP = new IString("lbleu_bp"), BLEU_P = new IString("bleu_prec"),
      LBLEU_P = new IString("lbleu_prec"), NIST = new IString("nist"),
      NIST_BP = new IString("nist_bp"), EXTERNAL = new IString("external"),
      NIST_P = new IString("nist_prec"), BLEU_1 = new IString("bleu_1gram"),
      BLEU_2 = new IString("bleu_2gram"), BLEU_3 = new IString("bleu_3gram"),
      BLEU_4 = new IString("bleu_4gram"), NIST_1 = new IString("nist_1gram"),
      NIST_2 = new IString("nist_2gram"), NIST_3 = new IString("nist_3gram"),
      NIST_4 = new IString("nist_4gram");
  ...
  // Double.parseDouble(System.getProperty(LEARN_RATE_PROPERTY, "0.3"));
  // public static final String PERSEVERENCE_PROPERTY = "Perseverence";
  // public static final int PERSEVERENCE =
  // Integer.parseInt(System.getProperty(PERSEVERENCE_PROPERTY, "5"));
  // public static final String MAX_ITERATIONS_PROPERTY = "MaxIterations";
  // public static final int MAX_ITERATIONS =
  // Integer.parseInt(System.getProperty(MAX_ITERATIONS_PROPERTY, "1000000"));
  public static final String STARTING_POINTS_PROPERTY = "StartingPoints";
  public static final int STARTING_POINTS = Integer.parseInt(System
      .getProperty(STARTING_POINTS_PROPERTY, "10"));
  public static final String SIMPLEX_GROWTH_PROPERTY = "SimplexGrows";
  public static final int SIMPLEX_GROWTH = Integer.parseInt(System.getProperty(
      SIMPLEX_GROWTH_PROPERTY, "5"));

  static public void main(String[] args) throws Exception {
    Properties prop = StringUtils.argsToProperties(args);
    System.err.println("properties: " + prop.toString());
    LinearCombinationMetricOptimizer hcm = new LinearCombinationMetricOptimizer(
        prop);
    hcm.maximize();
  }
  ...
  public LinearCombinationMetricOptimizer(Properties prop) throws Exception {
    ...
    if (metrics == null || refPrefix == null || hypFile == null
        || humanScoreFile == null) {
      System.err
    ...
    if (f.exists()) {
      ...
      for (int i = 0;; i++) {
        ...
        if (!f.exists())
          break;
        List<Sequence<IString>> lines = IOTools
            .slurpIStringSequences(refPrefix);
        lines = permute(lines);
  ...
  public void maximize() {
    ...
    if (w.length > 0 && tuneMetrics) {
      ...
      for (int i = 0; i < STARTING_POINTS; ++i) {
        for (int j = 0; j < SIMPLEX_GROWTH; ++j) {
          ...
          System.err.printf("Step %d/%d...\nStart at %s\n", i + 1,
              STARTING_POINTS, Arrays.toString(w));
      ...
      if (tuneCosts) {
        int i = bw.length - 1;
        System.out.printf("TER costs: sub=%.3f ins=%.3f del=%.3f shift=%.3f\n",
  ...
  public void createMetricScores() {
    ...
    while (i + sz <= numInstances) {
      ...
      // Create hypotheses:
      BLEUMetric<IString, String>.BLEUIncrementalMetric bleu = new BLEUMetric<IString, String>(
          refs1, true).getIncrementalMetric();
      NISTMetric<IString, String>.NISTIncrementalMetric nist = new NISTMetric<IString, String>(
          refs1).getIncrementalMetric();
      TERMetric<IString, String>.TERIncrementalMetric ter = new TERMetric<IString, String>(
          refs1).getIncrementalMetric();
      for (int j = 0; j < sz; ++j) {
        bleu.add(new ScoredFeaturizedTranslation<IString, String>(hyps.get(i
            + j), null, 0));
        nist.add(new ScoredFeaturizedTranslation<IString, String>(hyps.get(i
            + j), null, 0));
        ter.add(new ScoredFeaturizedTranslation<IString, String>(hyps
            .get(i + j), null, 0));
      }
      ...
      if (verbose) {
        System.err.printf("index %d (line %d) human=%.3f", ii, i,
            humanScores[ii]);
  ...
  /**
   * Computes Pearson correlation coefficient between two _standardized_
   * vectors.
   */
  ...
  void printWorseCorrelationCases(double[] combinedScores) {
    ...
    for (Pair<Double, Integer> aL : l) {
      ...
      System.err
          .printf(
              "index=%d human=%.3f metric=%.3f (standardized scores: human=%.3f metric=%.3f diff=%.3f):\n",
              ii, humanScores[ii], combinedScores[ii], humanScoresStd[ii],
              combinedScoresStd[ii], humanScoresStd[ii] - combinedScoresStd[ii]);
      for (int j = 0; j < windowSize; ++j) {
        int line = ii * windowSize + j;
        System.err.printf("  ref(%d)=%s\n", line, refs.get(line).get(0)
            .toString());
  ...
  /**
   * Transform input cost (any real value) into value that works reasonably well
   * with TER (between .05 and 1).
   */
