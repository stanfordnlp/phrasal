 ...
 * SymmetricalWordAlignment to limit the possible English word alignment choice,
 * and then use IBM Model1 score to pick the best one Notice it's not an
 * "IBMModel1Aligner", because it first constrains the possibilities to only the
...
public class IBMModel1InformedWordAligner implements WordAligner {
  ...
  public IBMModel1InformedWordAligner(String filename) {
    try {
      ...
      System.err
          .println("loading IBM Model 1 for IBMModel1InformedWordAligner from file: "
              + filename);
  ...
  public int getAlignedEnglishIndex(WordAlignment sp, int foreignIdx) {
    // private int getAlignedEnglishWordIndexFromSet(IString chWord,
    // Set<Integer> englishWordsIndices, Sequence<IString> e) {
    ...
    for (int eidx : englishWordsIndices) {
      ...
      if (max < score) {
        max_idx = eidx;
        if (DETAILED_DEBUG)
          System.err.println("max_idx=" + max_idx);
        max = score;
        if (DETAILED_DEBUG)
    ...
    if (DETAILED_DEBUG) {
      if (max_idx >= 0)
        System.err.println("enWord=" + e.get(max_idx));
    }
    return max_idx;
  }
}
