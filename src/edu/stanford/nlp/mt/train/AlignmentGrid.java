...
public class AlignmentGrid {
  ...
  private final List<AlignmentTemplateInstance> alTempList = new ArrayList<AlignmentTemplateInstance>();
   ...
   */
  public List<AlignmentTemplateInstance> getAlTemps() {
    return alTempList;
  }
  ...
  public void init(int eSize, int fSize) {
    ...
    if (eSize >= MAX_SENT_LEN || fSize >= MAX_SENT_LEN)
      throw new UnsupportedOperationException("Sentence too long: fsize="
          + fSize + " esize=" + eSize);
  ...
  public AlGridCell<AlignmentTemplateInstance> cellAt(int fi, int ei) {
    return alGridCells[fi][ei];
  }
  ...
  public void addAlTemp(AlignmentTemplateInstance alTemp, boolean isConsistent) {
    int e1 = alTemp.eStartPos(), e2 = alTemp.eEndPos(), f1 = alTemp.fStartPos(), f2 = alTemp
        .fEndPos();
    if (isConsistent) {
  ...
  /**
   * Add coordinates of an alignment template to the grid. Allows to cut down
   * memory usage, since one doesn't need to allocate each alignment template.
   */
  ...
  public static RelativePos relativePos(AlignmentTemplateInstance alTemp,
      int f, int e) {
    int e1 = alTemp.eStartPos(), e2 = alTemp.eEndPos(), f1 = alTemp.fStartPos(), f2 = alTemp
        .fEndPos();
    ...
    if (f < f1) {
      if (e < e1)
        return RelativePos.NW;
      if (e > e2)
        return RelativePos.SW;
      return RelativePos.W;
    }
    if (f > f2) {
      if (e < e1)
        return RelativePos.NE;
      if (e > e2)
        return RelativePos.SE;
      return RelativePos.N;
    }
    if (e < e1)
      return RelativePos.N;
    if (e > e2)
      return RelativePos.S;
    return RelativePos.I;
  }

  /**
   * Display alTemp and word alignments in the alignment grid. The alTemp is
   * displayed as a big filled square, and individual word alignments are shown
   * as crosses ("x"). The grid also displays contextual alignment templates
   * that are consistent with alTemp (consistent means both E and F spans do not
   * overlap). For instance, the "1" symbol indicates the location of the
   * bottom-left corner of any alignment template found north-west relative to
   * altemp. The same goes for "2", "3", and "4" for the three other directions.
   */
  public void printAlTempInGrid(String id, AlignmentTemplateInstance alTemp,
      PrintStream out) {
    ...
    for (int ei = 0; ei < eSize; ++ei) {
      ...
      for (int fi = 0; fi < fSize; ++fi) {
        ...
        if (alTemp != null) {
          ...
          if (pos == RelativePos.NW && alGridCells[fi][ei].hasBottomRight())
            phraseSym = "1";
          if (pos == RelativePos.SW && alGridCells[fi][ei].hasTopRight())
            phraseSym = "2";
          if (pos == RelativePos.NE && alGridCells[fi][ei].hasBottomLeft())
            phraseSym = "3";
          if (pos == RelativePos.SE && alGridCells[fi][ei].hasTopLeft())
            phraseSym = "4";
   ...
   */
  public static void printDecoderGrid(Featurizable<IString, String> f,
      PrintStream out) {
    if (!f.done)
    ...
    alGrid.sent = sent;
    alGrid.printAlTempInGrid("Alignment grid for decoded sentence: " + eSeq,
        null, out);
  }
