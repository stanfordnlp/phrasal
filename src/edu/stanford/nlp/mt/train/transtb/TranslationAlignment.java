...
public class TranslationAlignment {
  ...
  private int[][] trimMatrixRow(int[][] m, int newRowLength) {
    if (newRowLength != m.length - 1)
      throw new RuntimeException("new row length should be just one less");
  ...
  public TreeSet<Integer> mapChineseToEnglish_FillGap(TreeSet<Integer> enRange) {
    ...
    for (MutablePair<Integer, Integer> gap : gaps) {
      for (int eni = gap.first; eni <= gap.second; eni++) {
        ...
        boolean add = true;
        // It doesn't matter if the word aligns to NULL: matrix_[eni+1][0] can
        // be whatever
  ...
  public TranslationAlignment(String[] source, String[] translation,
      int[][] matrix) {
  ...
  public TranslationAlignment(String dataStr) {
    String regex = "<source_raw>(.*)</source_raw>\\n"
        + "<source>(.*)</source>\\n"
    ...
    if (matcher.find()) {
      ...
      for (int r_i = 0; r_i < rows.length; r_i++) {
        ...
        for (int e_i = 0; e_i < elements.length; e_i++) {
          ...
          if (element > 2)
            throw new RuntimeException("In alignment file: Bigger than 2?");
