...
public class EnglishTreeReader extends AbstractTreeReader {
  ...
  public EnglishTreeReader() {
    ...
    treeprint_ = new TreePrint("words,penn,typedDependencies",
        "removeTopBracket", tlpp_.treebankLanguagePack());
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 1; i <= 325; i++) {
      String name = String.format(dirName + "/chtb_%03d.mrg.gz", i);
...
class NMLandPOSTreeTransformer implements TreeTransformer {
  ...
  static Tree sToApostropheS(Tree tree) {
    List<Tree> leaves = tree.getLeaves();
    for (Tree leaf : leaves) {
      if (leaf.value().equals("s") && leaf.parent(tree).value().equals("POS")) {
        leaf.setValue("'s");
      }
    }
    return tree;
  ...
  static Tree mergeApostropheS(Tree tree) {
    ...
    while (needProcess) {
      ...
      List<Tree> leaves = tree.getLeaves();
      if (apostropheIdx < 0)
        throw new RuntimeException("Tree=" + tree.pennString());
      Tree tag1 = leaves.get(apostropheIdx).parent(tree);
      Tree tag2 = leaves.get(apostropheIdx + 1).parent(tree);
      if (tag1.value().equals("POS") && tag1.parent(tree) == tag2.parent(tree)) {
        ...
        for (int i = 0; i < oldChildren.length; i++) {
          if (i == apostropheIdx2) {
            if (!oldChildren[i].value().equals("POS"))
              throw new RuntimeException("");
            if (!oldChildren[i].firstChild().value().equals("'"))
              throw new RuntimeException("");
