...
public class DTUInstance extends AlignmentTemplateInstance {
   ...
   */
  public DTUInstance(WordAlignment sent, CoverageSet fs, CoverageSet es,
      boolean fContiguous, boolean eContiguous) {
    init(sent, fs, es, fContiguous, eContiguous);
  }

  public void init(WordAlignment sent, CoverageSet fs, CoverageSet es,
      boolean fContiguous, boolean eContiguous) {
    ...
    this.eStartPos = e1;
    // Init phrases:
    f = fContiguous ? sent.f().subsequence(f1, f2 + 1)
        : DiscontinuousSubSequences.subsequence(sent.f(), fs, fAlign);
    e = eContiguous ? sent.e().subsequence(e1, e2 + 1)
        : DiscontinuousSubSequences.subsequence(sent.e(), es, eAlign);
    fSet = fs;
    eSet = es;
