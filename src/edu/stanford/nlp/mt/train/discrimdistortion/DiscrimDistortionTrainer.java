...
public final class DiscrimDistortionTrainer {
  ...
  private static String usage() {
    ...
    classUsage
        .append(" -f  <num>   : Expected number of features (for mem pre-allocation)\n");
    classUsage
        .append(" -t  <num>   : Number of threads to use in feature extraction\n");
    classUsage.append(" -l          : Feature: source sentence length\n");
    classUsage.append(" -p          : Feature: relative sentence position\n");
    classUsage
        .append(" -s          : Feature: source POS tag (source file must be tagged with delimiter #)\n");
    classUsage
        .append(" -w <thresh> : Feature: word (cutoff threshold for vocabulary)\n");
    ...
    classUsage
        .append(" -r thresh   : Restrict training to abs(thresh) distortion movement\n");
  ...
  private final static OptionParser op = new OptionParser(
      "aodnx:r:ve:w:plst:f:m:");
  ...
  public static void main(String[] args) {
    ...
    DiscrimDistortionController controller = new DiscrimDistortionController(
        sourceFile, targetFile, alignFile, modelName);
    controller.setVerbose(VERBOSE);
    controller.setFeatureFlags(USE_WORD, USE_TAG, USE_POSITION, USE_SLEN,
        USE_CONTEXT, USE_ARC_TAG);
