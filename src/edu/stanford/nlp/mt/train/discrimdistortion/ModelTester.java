  ...
  private static String prettyPrint(DistortionModel model, Datum d) {
    ...
    for (DistortionModel.Feature feat : model.featureIndex) {
      if (feat == DistortionModel.Feature.Word
          && model.wordIndex.contains((int) d.get(featPtr)))
        sb.append(String.format(" %s",
            model.wordIndex.get((int) d.get(featPtr))));
  ...
  private static void evalDatums(DistortionModel model, String evalFile) {
    ...
    try {
      while (reader.ready()) {
        ...
        PrintStream ps = IOTools.getWriterFromFile(evalFile + ".out."
            + Integer.toString(reader.getLineNumber()));

        StringTokenizer st = new StringTokenizer(input);
        String word = null, tag = null, rTag = null, lTag = null, arcTag = null;
        int slen = 0;
        int featIdx = (model.featureIndex
            .contains(DistortionModel.Feature.Word)) ? 0 : 1;
