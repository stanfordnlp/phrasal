...
/**
 * Same as MosesPhraseExtractor, but restricts phrases according to dependencies
...
public class DependencyPhraseExtractor extends MosesPhraseExtractor {
  ...
  private static final boolean DEBUG = System
      .getProperty("debugDependencyPhraseExtractor") != null;
  private static final boolean EXTRACT_MODIFIER_PHRASES = System
      .getProperty("extractModifierPhrases") != null;

  private static final int NO_ID = -2;
  private static final int ROOT_ID = -1;

  public DependencyPhraseExtractor(Properties prop, AlignmentTemplates alTemps,
  ...
  public void setSentenceInfo(WordAlignment sent, String infoStr) {
    ...
    if (DEBUG) {
      ...
      for (int src = 0; src < deps.size(); src++) {
        ...
        System.err.printf("%d(%s)-%d(%s)\n", src, sent.e().get(src), tgt,
            (tgt >= 0) ? sent.e().get(tgt) : "<root>");
      }
    }
  ...
  private boolean ignorePhrase(WordAlignment sent, int f1, int f2, int e1,
    ...
    for (int si = e1; si <= e2; ++si) {
      ...
      // sent.e().get(si), ti, (ti>=0) ? sent.e().get(ti) : "");
      if (e1 <= ti && ti <= e2)
        continue;
      if (headIdx == NO_ID) {
        headIdx = ti;
    ...
    if (headAttachCount <= 0)
      throw new RuntimeException(String.format(
          "Head word %d without dependents\n", headIdx));

    if (headAttachCount > 1 && !EXTRACT_MODIFIER_PHRASES) {
      if (DEBUG)
        System.err.printf("ignore modifier phrase: %s\n",
            sent.e().subsequence(e1, e2 + 1));
      return true;
    }
    return false;
