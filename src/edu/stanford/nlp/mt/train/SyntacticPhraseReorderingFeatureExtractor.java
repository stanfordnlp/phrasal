...
public class SyntacticPhraseReorderingFeatureExtractor extends AbstractFeatureExtractor {
    ...
    for (AlignmentTemplateInstance t : allAlTemps) {
      ...
      if (DEBUG) System.err.printf("Processing t : %s\n", t.toString(true));
          if (e1 < min_E_for_F_range[f1][f2]) {
            ...
            if (DEBUG) System.err.printf("min_E_for_F_range[%d][%d] -> %d\n",f1, f2, e1);
          }
          if (e2 > max_E_for_F_range[f1][f2]) {
            max_E_for_F_range[f1][f2] = e2;
            alTemp_max_E_for_F_range[f1][f2] = t;
            if (DEBUG) System.err.printf("max_E_for_F_range[%d][%d] -> %d\n",f1, f2, e2);
          }
    }
      ...
      for(int i = 0; i < fsize; i++) {
        for(int j = 0; j < fsize; j++) {
          if (min_E_for_F_range[i][j] <= max_E_for_F_range[i][j]) {
            if (DEBUG) System.err.printf("Precompute: f[%d-%d] --> e[%d-%d]\n", i,j,min_E_for_F_range[i][j], max_E_for_F_range[i][j]);
            StringBuilder fseq = new StringBuilder();
            StringBuilder eseq = new StringBuilder();
            
            Sequence<IString> f = alTemp_min_E_for_F_range[i][j].getSentencePair().f();
            Sequence<IString> e = alTemp_min_E_for_F_range[i][j].getSentencePair().e();
  ...
  public IntPair extendPhraseRage(IntPair phraseRange, 
                                  int begin_phrase, int end_phrase) {
    if (phraseRange.getSource() > phraseRange.getTarget()) {
      ...
      System.err.printf("PB: mod f range [%d-%d] --> e range [%d-%d]\n",
                        begin_phrase-1, end_phrase,
                        phraseRange.getSource(), phraseRange.getTarget());
      phraseRange = englishRangeForForeignRange(begin_phrase,end_phrase+1);
      System.err.printf("PB: mod f range [%d-%d] --> e range [%d-%d]\n",
                        begin_phrase, end_phrase+1,
                        phraseRange.getSource(), phraseRange.getTarget());
      phraseRange = englishRangeForForeignRange(begin_phrase-1,end_phrase+1);
      System.err.printf("PB: mod f range [%d-%d] --> e range [%d-%d]\n",
                        begin_phrase-1, end_phrase+1,
                        phraseRange.getSource(), phraseRange.getTarget());
    }
    return phraseRange;
  }
  ...
  @Override
    ...
    System.err.println("PICHUAN: in SyntacticPhraseReorderingFeatureExtractor.java");
    System.err.println("------------------------------------------------");
    ...
    for(Map.Entry<IntQuadruple,String> e : boundaries.entrySet()) {
      ...
      IntPair wholePhraseRange 
        = englishRangeForForeignRange(begin_firstPhrase,end_secondPhrase);
      IntPair firstPhraseRange 
        = englishRangeForForeignRange(begin_firstPhrase,end_firstPhrase);
      IntPair secondPhraseRange 
        = englishRangeForForeignRange(begin_secondPhrase,end_secondPhrase);
      System.err.println("PB:"+ranges+"\t"+str);
      System.err.printf("PB: f0 range [%d-%d] --> e0 range [%d-%d]\n",
                        begin_firstPhrase, end_secondPhrase, 
                        wholePhraseRange.getSource(), wholePhraseRange.getTarget());

      //wholePhraseRange  = extendPhraseRage(wholePhraseRange,begin_firstPhrase,end_secondPhrase);

      System.err.printf("PB: f1 range [%d-%d] --> e1 range [%d-%d]\n",
                        begin_firstPhrase, end_firstPhrase, 
                        firstPhraseRange.getSource(), firstPhraseRange.getTarget());

      //firstPhraseRange  = extendPhraseRage(firstPhraseRange,begin_firstPhrase,end_firstPhrase);

      System.err.printf("PB: f2 range [%d-%d] --> e2 range [%d-%d]\n",
                        begin_secondPhrase, end_secondPhrase, 
                        secondPhraseRange.getSource(), secondPhraseRange.getTarget());

      //secondPhraseRange = extendPhraseRage(secondPhraseRange,begin_secondPhrase,end_secondPhrase);
      ...
      String type = checkPhraseDistortionType(firstPhraseRange, secondPhraseRange);
      //if (type.equals("OTHER")) {
      System.err.printf("DEBUG: type=%s, %s\t%s\n",type,firstPhraseRange,secondPhraseRange);
  ...
  String checkPhraseDistortionType(IntPair firstPhraseRange, IntPair secondPhraseRange) {
    ...
    if (begin_firstPhrase > end_firstPhrase || begin_secondPhrase > end_secondPhrase) {
      return "BAD";
    }
    ...
    if (begin_firstPhrase < begin_secondPhrase && end_firstPhrase < end_secondPhrase && end_firstPhrase >= begin_secondPhrase) {
      return "MONOTONE-OVERLAP";
    }
    if (begin_firstPhrase > begin_secondPhrase && end_firstPhrase > end_secondPhrase && end_secondPhrase >= begin_firstPhrase) {
      return "SWAP-OVERLAP";
    }
  ...
  TwoDimensionalCounter<String, Boolean> goodnessCounter
    = new TwoDimensionalCounter<String, Boolean>();

  TwoDimensionalCounter<String, String> distortionTypeCounter
  ...
  @Override
    ...
    for(String k1 : goodnessCounter.firstKeySet()) {
      ...
      System.out.printf("%s\t%.2f%%\t%.2f%%\t%.1f\t%.1f\n", k1, pTrue, pFalse, counter.getCount(true), counter.getCount(false));
    }
    System.out.println("---------------------------------------------------------------");
