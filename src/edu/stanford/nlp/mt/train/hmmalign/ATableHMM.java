...
public class ATableHMM extends ATable {
  ...
  public void initialize(ATable a1) {
    ...
    for (int jump = 0; jump <= MAX_LENGTH + 1; jump++) {
      initialCounts[jump] = a.getProbHMM(jump > MAX_LENGTH ? 2 * MAX_LENGTH + 1
          : jump, 0, MAX_LENGTH);
    }// jump
  ...
  public boolean checkOK(int len) {
    ...
    for (int i_prev = 1; i_prev <= 2 * len; i_prev++) {
      ...
      if (Math.abs(total - 1) > .001) {
        System.out.println(" not ok in iPrev " + i_prev + " total is " + total
            + " len " + len);
        return false;
      }
    }
  ...
  /*
   * calculate the kl divergence between this table and a in terms of the jump
   * probabilities
   */
  @Override
