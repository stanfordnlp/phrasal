 ...
 */
public class WordPairDistortionStatisticsCollector implements
    StatisticsCollector {
  ...
  public void collect(WordAlignment sent) {
    ...
    /*
     * this array records what is the one E that aligns to each f by the
     * WordAligner
     */
    ...
    for (int curr = 0; curr < f.size() - 1; curr++) {
       ...
       * System.err.println("f = "+f); System.err.println("e = "+e);
       * System.err.println("a = "+sent);
       * System.err.printf("f[%d]=%s --> e[%d]=%s\n", curr, f.get(curr), eCurr,
       * e.get(eCurr)); System.err.printf("f[%d]=%s --> e[%d]=%s\n", next,
       * f.get(next), eNext, e.get(eNext)); }
       */

      // String distortionType =
      // edu.stanford.nlp.mt.ExperimentalFeaturizers.WordLevelDiscrimDistortionFeaturizer.getDistortionType(distance);
      String distortionType = WordLevelDiscrimDistortionFeaturizer
          .getDistortionTypeAll(distance);
  ...
  public void postProcess() {
    System.err
        .println("----------postProcess step for WordPairDistortionStatisticsCollector----------");
    System.err.println("0) Settings");
    System.err.println("0.1) the WordAligner used is : "
        + waligner.getClass().getName());
    System.err.println();
    ...
    System.err
        .println("1.3) Word Pairs not counted   = " + wordPair_notAligned);
    System.err.println("1.4) Word Pairs counted       = " + wordPair_aligned);
    System.err.printf("1.5) Word Pairs total         = %d\n", wordPair_aligned
        + wordPair_notAligned);
    System.err.println("1.6) Foreign words that are aligned = "
        + fWords_aligned);
    System.err.printf(
        "1.7) Foreign words that are NOT aligned = %d , perc = %.3f%%\n",
        fWords_notAligned, (double) fWords_notAligned
            / (fWords_notAligned + fWords_aligned) * 100);
    ...
    System.err.println("1.11) distortionTypeCounter #type = "
        + distortionTypeCounter.size());
    System.err.println("1.12) distortionTypeCounter totalCounts = "
        + distortionTypeCounter.totalCount());
    System.err
        .println("1.10) distortionTypeCounter = " + distortionTypeCounter);
    System.err.println();
    ...
    System.err.printf("3.1) INTERSECTED = %d (%.3f)\n",
        ...
        (double) ShortestDistanceWordAligner.INTERSECTED
            / ShortestDistanceWordAligner.TOTAL * 100);
  ...
  public static void main(String[] args) {
    ...
    WordPairDistortionStatisticsCollector c = WordPairDistortionStatisticsCollector
        .load(serializedFileName);
    double totalSecs = (System.currentTimeMillis() - startTimeMillis) / 1000.0;
    ...
    System.err.println("distortionTypeCounter.size="
        + c.distortionTypeCounter.size());
    System.err.println("distortionTypeCounter=" + c.distortionTypeCounter);
    System.err.println("distortionTypeCounter.all="
        + c.distortionTypeCounter.totalCount());
    ...
    for (String distance : c.distortionTypeCounter.keySet()) {
      ...
      newDistortionTypeCounter.incrementCount(
          WordLevelDiscrimDistortionFeaturizer.getDistortionType(dis), count);
    }
    ...
    // int numKeys =
    // edu.stanford.nlp.mt.ExperimentalFeaturizers.WordLevelDiscrimDistortionFeaturizer.getNumDistortionType();
    /*
     * for(Map.Entry<String,Counter<String>> e : c.counterF1F2.entrySet()) {
     * System.err.println(e.getKey()); //Distribution<String> d =
     * Distribution.goodTuringSmoothedCounter(e.getValue(), numKeys);
     ...
     * System.err.println(e.getKey()); Distribution<String> d =
     * Distribution.getDistribution(e.getValue()); System.err.println(d);
     * System.err.println("============================================"); }
     */
  }
}
