...
public class SloppyBeam<S extends State<S>> implements Beam<S> {
  ...
  static public final int DEBUG_LEVEL = Integer.valueOf(System.getProperty(
      DEBUG_PROPERTY, "0"));
  ...
  public SloppyBeam(int capacity, RecombinationFilter<S> filter) {
    if (DEBUG_LEVEL >= DEBUG_BASIC) {
      System.err.printf(
          "New Sloppy Beam - capacity: %d pruning multiplier: %d\n", capacity,
          MOSES_SLOPPY_PRUNING_CAPACITY_MULTIPLIER);
    }
  ...
  synchronized public S put(S hypothesis) {
    ...
    // prune if necessary
    if (recombinationHash.size() >= MOSES_SLOPPY_PRUNING_CAPACITY_MULTIPLIER
        * capacity) {
      pruneBeam();
    }
  ...
  public void pruneBeam() {
    if (DEBUG_LEVEL >= DEBUG_BASIC) {
      System.err
          .printf(
              "Pre-Prunning - beam size: %d capacity: %d recombination hash size: %d\n",
    ...
    worst = hypotheses.get(
        Math.max(0, Math.min(capacity - 1, hypotheses.size() - 1))).score();
    ...
    if (DEBUG_LEVEL >= DEBUG_BASIC) {
      ...
      System.err
          .printf(
              "Post-Prunning - beam size: %d capacity: %d recombination hash size: %d\n",
              size(), capacity, recombinationHash.size());
    }
  }
