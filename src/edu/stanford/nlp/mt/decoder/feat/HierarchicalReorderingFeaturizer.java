...
public class HierarchicalReorderingFeaturizer extends StatefulFeaturizer<IString,String> implements RichIncrementalFeaturizer<IString, String>, MSDFeaturizer<IString,String>, ClonedFeaturizer<IString,String> {
  ...
  public static class HierBlock {
    ...
    HierBlock(CoverageSet cs, Featurizable<IString, String> previousF, int stackSz)
    { this.cs = cs; this.previousF = previousF; this.stackSz = stackSz; }

    public int fStart() { int i = cs.nextSetBit(0); assert(i>=0); return i; }
    public int fEnd() { int i = cs.length()-1; assert(i>=0); return i; }
    public boolean isPrefix() { return cs.cardinality() == cs.length(); }
  }

  // local: should produce the same as the non-hierarchical reordering model in LexicalReorderingFeaturizer
  // hierarchical (default): as described in the 2008 EMNLP paper (runtime: O(1) for each call of listFeaturize())
  ...
  //   boundary conditions.
  // Note: hierarchical chokes on constructions that are not ITG binarizable, e.g.,
  //    ..x..
  //    ...x.
  //    x....
  //    .x...
  //    ....a
  // Here, 'a' should have monotone orientation, though hierarchical predicts discontinuous. backtrack and backtrackFast
  ...
  // Performance: backtrackFast is the fastest. The two others are about 2-5% slower.
	private enum ForwardOrientationComputation { local, hierarchical, backtrack, backtrackFast }
  private ForwardOrientationComputation forwardOrientationComputation = ForwardOrientationComputation.hierarchical;

  // local: should produce the same as the non-hierarchical reordering model in LexicalReorderingFeaturizer
  // hierarchical: predict discontinuous only if we know for sure orientation will be discontinuous
  ...
  //    predict Discontinuous otherwise)
	private enum BackwardOrientationComputation { local, hierarchical }
  ...
  private BitSet tmpCoverage = new BitSet();
    ...
    if (args.length >= 3) {
      ...
      if (args.length >= 4) {
        forwardOrientationComputation = ForwardOrientationComputation.valueOf(args[3]);
        if (args.length >= 5) {
          backwardOrientationComputation = BackwardOrientationComputation.valueOf(args[4]);
          if (args.length >= 6) {
    ...
    float[] scores = mlrt.getReorderingScores(f.hyp.translationOpt.abstractOption.id);
    float[] priorScores = (f.prior == null ? null : mlrt.getReorderingScores(f.prior.hyp.translationOpt.abstractOption.id));

    ReorderingTypes 
      forwardOrientation = ReorderingTypes.discontinuousWithPrevious,
