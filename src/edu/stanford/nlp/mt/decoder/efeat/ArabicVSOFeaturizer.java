...
public class ArabicVSOFeaturizer implements IncrementalFeaturizer<IString, String>, AlignmentFeaturizer {
  ...
  private boolean fireFeature(final Pair<Integer,Integer> subject,
    ...
    final int eVerbLeftBound = getLeftTargetSideBoundary(new Pair<Integer,Integer>(verbIdx,verbIdx), f);
    ...
    if(modeIndicator == 1) {
      if(subjCovered && (eVerbRightBound == eSubjRightBound || (eSubjRightBound + 1) == eVerbLeftBound))
        return true;
    } else {
  ...
  private int getLeftTargetSideBoundary(Pair<Integer,Integer> span, Featurizable<IString,String> f) {
    ...
    return (minLeftIndex == Integer.MAX_VALUE) ? Integer.MIN_VALUE : minLeftIndex;
  }
  ...
  private int getRightTargetSideBoundary(Pair<Integer,Integer> span, Featurizable<IString,String> f) {
    ...
    return (maxRightIndex == Integer.MIN_VALUE) ? Integer.MAX_VALUE : maxRightIndex;
  }
  ...
  public FeatureValue<String> featurize(Featurizable<IString,String> f) {
    ...
    if(lastSubject != currentSubject) {
      ...
      if(fireFeature(subject,verbIdx,f,VERBOSE)) {
        ...
        if(modeIndicator == 1) {
          if(f.prior == null)
            return new FeatureValue<String>(FEATURE_NAME, DEFAULT_BIN1_VALUE - (DEFAULT_BIN1_VALUE / (double) subjectSpans.size()));
          else
            return new FeatureValue<String>(FEATURE_NAME, -1 * DEFAULT_BIN1_VALUE / (double) subjectSpans.size());
          //          else if(currentSubject == 0)
//            return new FeatureValue<String>(FEATURE_NAME, FEATURE_VALUE - DEFAULT_BIN1_VALUE); //Back out the default penalty
//          else { //Need to check if this is the first time that the feature has fired
//            for(int i = currentSubject - 1; i >= 0; i--) {
//              Triple<Integer,Integer,Integer> prevSubjectGroup = subjectSpans.get(i);
//              final int vIdx = prevSubjectGroup.first();
//              Pair<Integer,Integer> subjSpan = new Pair<Integer,Integer>(prevSubjectGroup.second(),prevSubjectGroup.third());
//              if(fireFeature(subjSpan,vIdx,f,VERBOSE))
//                return new FeatureValue<String>(FEATURE_NAME, -1 * FEATURE_VALUE / (currentSubject + 2));                
//            }
//            //A first-timer....
