...
/**
 * Does nothing expect checking that each abstract option has an alignment available.
 *
...
public class DummyAlignmentFeaturizer implements IncrementalFeaturizer<IString, String>, AlignmentFeaturizer {
  ...
  @Override
    ...
    if (f.option.phraseTableName.equals(IdentityPhraseGenerator.PHRASE_TABLE_NAMES))
    ...
    if (!f.option.abstractOption.alignment.hasAlignment()) {
      throw new RuntimeException(String.format("Alignment missing for phrase:\n"+
         "Featurizer: %s\nOption: %s\nAbstract option: %s\n",
          f, f.option, f.option.abstractOption));
    }
		return null;
  }
