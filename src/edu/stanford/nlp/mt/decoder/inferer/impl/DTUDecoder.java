...
public class DTUDecoder<TK, FV> extends AbstractBeamInferer<TK, FV> {
  ...
  protected Beam<Hypothesis<TK, FV>> decode(Scorer<FV> scorer,
    ...
    Beam<Hypothesis<TK, FV>>[] beams = createBeamsForCoverageCounts(
        foreign.size() + 2, beamCapacity, filter, recombinationHistory);
    ...
    List<ConcreteTranslationOption<TK>> optionsWithoutGaps = new ArrayList<ConcreteTranslationOption<TK>>(), optionsWithGaps = new ArrayList<ConcreteTranslationOption<TK>>();
  ...
  public class DTUOptionGrid {
    ...
    public DTUOptionGrid(List<ConcreteTranslationOption<TK>> options,
        Sequence<TK> foreign) {
