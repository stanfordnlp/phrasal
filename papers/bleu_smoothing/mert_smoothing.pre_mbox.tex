...
%\begin{enumerate}
%  \item State original stat mt formulation \cite{brown1993}, $p(e|f) = p(f|e)p(e)$
...
Using n-best lists produced by a decoder to approximate ${\cal E}^n$ and ${\cal H}^n$, MERT searches for the weight vector ${\bf w}^*$ that minimizes the loss $\ell$. Letting $\tilde{\bf E}_{\bf w}$ denote the result of the $\argmax$ w.r.t\@. the n-best approximation of the hypothesis space, this search is expressed by equation (\ref{unsmoothmert}). That is, the objective function being optimized during learning is equivalent to the loss assigned by the automatic measure of translation quality, i.e. ${\cal O}({\bf w}) = \ell (\tilde {\bf E}_{\bf w})$.
...
\begin{algorithm}[tb]
\begin{algorithmic}
   \STATE {\bfseries Input:} ${\cal L}$, ${\bf w}$, ${\bf d}$, eval
   ...
   \FOR {$l \in {\cal L}$}
     ...
     \ENDFOR
     % \STATE find initial best at $\infty$
   ...
   \STATE {\bf return} ${\bf w}+(i_{best}-\epsilon) \cdot {\bf d}$
\caption{
\emcite{och2003MERT}'s line search method to find the global minimum in the loss, ${\ell}$, when starting at the point ${\bf w}$ and searching along the direction ${\bf d}$ using the candidate translations given in the collection of n-best lists ${\cal L}$.
...
%    \item $O(dnm^2)$ -  let $n$ denote the number of foreign sentences ${\bf f}$ in a data set, $d$ denote the number of parameters being searched, and letting $m$ denote the size of the n-best lists used for learning.
...
One candidate for performing such regularization is the continuous approximation of the MERT objective, ${\cal O} = \mathbb{E}_{p_{\bf w}}({\ell})$. \emcite{och2003MERT} claimed that this approximation achieved essentially equivalent performance to that obtained when using the true loss, ${\cal O} = \ell$. However, \emcite{zens07} found that ${\cal O} = \mathbb{E}_{p_{\bf w}}({\ell})$ achieved substantially better test set performance than ${\cal O} = \ell$, even though it performs slightly worse on the data used to train the parameters. Unfortunately, a straightforward implementation of the continuous approximation requires a loss that can be applied at the sentence level. If the evaluation metric of interest does not have this property (e.g. BLEU), a sentence level surrogate must be developed, with successful learning then being tied to how well the surrogate captures the critical properties of the true loss.  
