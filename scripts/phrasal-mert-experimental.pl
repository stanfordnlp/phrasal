	    ...
	    $jmert_log = "$work_dir/jmert.$iter.log";
   	  if ($iter >= $first_active_iter) {
	      unlink($next_iter_weights);
				...
				my $mertCMD = "java $java_flags mt.tune.UnsmoothedMERT -o simplex+cer~ -t 4 -p 20 -s $all_iter_weights $opt_type $iter_pcumulative_nbest $iter_nbest_list.gz $all_iter_weights $commaRefList $next_iter_weights > $jmert_log 2>&1";
