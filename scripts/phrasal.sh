...
function create-learn-curve {
  ...
  if [[ -n "$REPORTING_DIR" && -d $REPORTING_DIR ]]; then
      ...
      echo "<tr><td>$TUNE_SET_NAME</td><td>$DECODE_SET_NAME</td><td>$RUNNAME</td><td>$MAX_SCORE</td><td>$METRIC</td><td>$(whoami)</td><td>$(pwd)</td><td>$(date +"%Y-%m-%d %H:%M")</td><td>$RUN_MSG</td></tr>" >> $RESULTS_FILE
