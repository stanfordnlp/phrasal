...
if [ $model_opt -eq 1 ]; then
  echo "# RNNLM"
  execute_check $all_nbest_scores "~lmthang/rnnlm_ext/rnnlm -rnnlm $model -test $all_nbest_distinct_unk -nbest -debug 2 > $all_nbest_scores"
else
  echo "# NPLM"
...
date
echo "# Adding scores ..."
echo "$scriptDir/nbest_add_rnnlm_score.py $nbest_in $all_nbest_distinct $all_nbest_scores $nbest_out" 
execute_check $nbest_out "$scriptDir/nbest_add_rnnlm_score.py $nbest_in $all_nbest_distinct $all_nbest_scores $nbest_out"
